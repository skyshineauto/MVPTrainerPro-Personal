class MvpSoundModesProcessor extends AudioWorkletProcessor {
  constructor() {
    ...
    this.port.onmessage = (event) => {
      ...
      if (m.type === "ping") {
        this.port.postMessage({ type: "ready", engine: "r21" });
        return;
      }
  ...
  processDimension(mode, l, r) {
    const sr = typeof sampleRate === "number" && sampleRate > 0 ? sampleRate : 48000;
    const sideAlpha = 1 - Math.exp(-2 * Math.PI * 650 / sr);
    const spatialAlpha = 1 - Math.exp(-2 * Math.PI * 900 / sr);
    const state = this.dimensionState[mode];

    // R21 MVP DIMENSION MAX:
    // tuned for the biggest premium soundstage without touching the direct gain or EQ.
    // Bass stays centered; real stereo and generated upper-mid/high side space go much wider.
    const width = mode === "power" ? 2.45 : 1.75;
    const lowWidth = mode === "power" ? 0.03 : 0.15;
    const centerFocus = mode === "power" ? 1.05 : 1.025;
    const spatialMix = mode === "power" ? 0.74 : 0.42;
    const ap1Coeff = mode === "power" ? 0.75 : 0.60;
    const ap2Coeff = mode === "power" ? -0.60 : -0.47;
    ...
    // All state continues running while OFF, so the ON/OFF comparison is instant.
