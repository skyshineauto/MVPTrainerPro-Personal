class MvpSoundModesProcessor extends AudioWorkletProcessor {
  constructor() {
    ...
    this.port.onmessage = (event) => {
      ...
      if (m.type === "ping") {
        this.port.postMessage({ type: "ready", engine: "r19" });
        return;
      }
  ...
  resetDimensionState() {
    this.dimensionState.adaptive.lowSide = 0;
    this.dimensionState.power.lowSide = 0;
  }

  processDimension(mode, l, r) {
    const sr = typeof sampleRate === "number" && sampleRate > 0 ? sampleRate : 48000;
    const lowAlpha = 1 - Math.exp(-2 * Math.PI * 240 / sr);
    const state = this.dimensionState[mode];

    // R19 STRONG DIMENSION:
    // much wider mids/highs, tighter low-end width, and a small center-focus lift.
    const width = mode === "power" ? 1.65 : 1.35;
    const lowWidth = mode === "power" ? 0.10 : 0.30;
    const centerFocus = mode === "power" ? 1.06 : 1.03;

    const mid = (l + r) * 0.5;
    const side = (l - r) * 0.5;

    // Keep the crossover state warm even while Dimension is OFF so toggling is immediate.
    state.lowSide += (side - state.lowSide) * lowAlpha;
    if (!this.user.dimensionEnabled) return { l, r };

    const highSide = side - state.lowSide;
    const shapedSide = state.lowSide * lowWidth + highSide * width;
