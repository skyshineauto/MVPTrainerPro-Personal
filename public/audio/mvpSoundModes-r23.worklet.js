class MvpSoundModesProcessor extends AudioWorkletProcessor {
  constructor() {
    ...
    this.port.onmessage = (event) => {
      ...
      if (m.type === "ping") {
        this.port.postMessage({ type: "ready", engine: "r23" });
        return;
      }
  ...
  resetDimensionState() {
    for (const state of Object.values(this.dimensionState)) {
      ...
      state.ap2X = 0;
      state.ap2Y = 0;
      state.generatedAirLow = 0;
      state.sidePresenceLow = 0;
      state.sideAirLow = 0;
    }
  }
  ...
  processDimension(mode, l, r) {
    ...
    const generatedAirAlpha = 1 - Math.exp(-2 * Math.PI * 9500 / sr);
    const sidePresenceAlpha = 1 - Math.exp(-2 * Math.PI * 4000 / sr);
    const sideAirAlpha = 1 - Math.exp(-2 * Math.PI * 7000 / sr);
    const state = this.dimensionState[mode];

    // R23 MVP DIMENSION ULTRA SMOOTH:
    // Keep the full R22 width and spatial amount. Only smooth the SIDE tone.
    ...
    const sidePresenceScale = mode === "power" ? 0.92 : 0.95;
    const sideAirScale = mode === "power" ? 0.84 : 0.88;
    const generatedAirScale = mode === "power" ? 0.70 : 0.76;
    ...
    // Extra top-end trim only on the generated spatial layer above about 9.5 kHz.
    state.generatedAirLow += (generatedSide - state.generatedAirLow) * generatedAirAlpha;
    const smoothGeneratedSide =
      state.generatedAirLow +
      (generatedSide - state.generatedAirLow) * generatedAirScale;

    const rawSide =
      state.lowSide2 * lowWidth +
      highSide * width +
      smoothGeneratedSide;

    // Side-only presence/air smoothing. The MID/center never passes through this.
    state.sidePresenceLow += (rawSide - state.sidePresenceLow) * sidePresenceAlpha;
    state.sideAirLow += (rawSide - state.sideAirLow) * sideAirAlpha;
    const sidePresenceBand = state.sideAirLow - state.sidePresenceLow;
    const sideAirBand = rawSide - state.sideAirLow;
    const smoothSide =
      state.sidePresenceLow +
      sidePresenceBand * sidePresenceScale +
      sideAirBand * sideAirScale;

    // All states stay warm while OFF for an immediate A/B toggle.
    if (!this.user.dimensionEnabled) return { l, r };

    const focusedMid = mid * centerFocus;
    return {
      l: focusedMid + smoothSide,
      r: focusedMid - smoothSide,
    };
  }
