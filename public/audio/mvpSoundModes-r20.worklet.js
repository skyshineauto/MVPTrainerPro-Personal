class MvpSoundModesProcessor extends AudioWorkletProcessor {
  constructor() {
    ...
    this.port.onmessage = (event) => {
      ...
      if (m.type === "ping") {
        this.port.postMessage({ type: "ready", engine: "r20" });
        return;
      }
  ...
  processDimension(mode, l, r) {
    const sr = typeof sampleRate === "number" && sampleRate > 0 ? sampleRate : 48000;
    const sideAlpha = 1 - Math.exp(-2 * Math.PI * 600 / sr);
    const spatialAlpha = 1 - Math.exp(-2 * Math.PI * 1000 / sr);
    const state = this.dimensionState[mode];

    // R20 SPATIAL DIMENSION:
    // 1) keeps low-frequency side energy tight,
    // 2) strongly widens real stereo mids/highs,
    // 3) creates new phase-decorrelated side space from center-heavy upper mids/highs.
    const width = mode === "power" ? 2.05 : 1.60;
    const lowWidth = mode === "power" ? 0.05 : 0.20;
    const centerFocus = mode === "power" ? 1.04 : 1.02;
    const spatialMix = mode === "power" ? 0.55 : 0.32;
    const ap1Coeff = mode === "power" ? 0.68 : 0.55;
    const ap2Coeff = mode === "power" ? -0.52 : -0.42;

    const mid = (l + r) * 0.5;
    const side = (l - r) * 0.5;

    // Two-pole low-side detector keeps kick/bass centered without narrowing the whole mix.
    ...
    // Two cascaded high-pass stages keep generated space out of the bass and lower mids.
    ...
    // Two first-order all-pass stages create phase depth without an audible echo/delay.
    ...
    // Keep all spatial states warm while OFF so toggling is immediate and click-free.
