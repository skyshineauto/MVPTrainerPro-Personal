...
const MARKER = "MVP_R81_R2_EFFECTS_LOUDNESS_INTERACTION";
...
try {
  ...
  if (!player.includes("R80_R3_PROFILE_GAIN_KEY")) {
...
console.log("R81 mastering/effects test: PASS");`,
  ...
  console.error(`\nR81-R2 FAILED: ${error instanceof Error ? error.message : String(error)}`);
  console.error("Restoring all touched project files...");
  restore();
  console.error("Restore complete. Repo returned to its exact pre-R81-R2 state.");
  process.exitCode = 1;
}
