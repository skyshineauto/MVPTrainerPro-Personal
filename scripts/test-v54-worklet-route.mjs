...
function makeInput(){const N=128,inputL=new Float32Array(N),inputR=new Float32Array(N);for(let i=0;i<N;i++){const t=i/48000;inputL[i]=.25*Math.sin(2*Math.PI*110*t)+.15*Math.sin(2*Math.PI*3400*t);inputR[i]=.24*Math.sin(2*Math.PI*110*t+.1)+.14*Math.sin(2*Math.PI*3400*t+.3);}return{inputL,inputR};}
function block(){const {inputL,inputR}=makeInput(),outL=new Float32Array(128),outR=new Float32Array(128);p.process([[inputL,inputR]],[[outL,outR]]);return{inputL,inputR,outL,outR};}
let b=block(),diff=0;for(let i=0;i<128;i++)diff+=Math.abs(b.outL[i]-b.inputL[i]);if(diff<.01)throw Error('worklet output did not change');
// Authoritative route proof: muting the processor must produce digital silence, then recover.
p.port.send({type:'SET_PROOF_MUTE',enabled:true});const muteAck=[...p.port.messages].reverse().find(m=>m.type==='PROOF_MUTE_APPLIED');if(!muteAck?.enabled)throw Error('proof mute not acknowledged');
b=block();let mutedPeak=0;for(let i=0;i<128;i++)mutedPeak=Math.max(mutedPeak,Math.abs(b.outL[i]),Math.abs(b.outR[i]));if(mutedPeak>1e-9)throw Error('proof mute leaked audio '+mutedPeak);
p.port.send({type:'SET_PROOF_MUTE',enabled:false});b=block();let recovered=0;for(let i=0;i<128;i++)recovered=Math.max(recovered,Math.abs(b.outL[i]),Math.abs(b.outR[i]));if(recovered<1e-4)throw Error('route did not recover after proof mute');
...
console.log(`V5.4 Worklet route + proof mute: ${checked}/${checked} PASS`);console.log('ACK signature:',ack.signature.slice(0,80)+'...');
