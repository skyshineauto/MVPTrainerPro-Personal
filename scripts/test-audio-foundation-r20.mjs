...
const workletPath = process.argv[2] || "public/audio/mvpSoundModes-r20.worklet.js";
...
if (!Processor) throw new Error("R20 worklet did not register");
...
check("R20 has no compressor stage", !code.includes("compGain") && !code.includes("compressionDb") && !code.includes("thresholdDb"), {});
check("R20 has no limiter stage", !code.includes("limiterGain") && !code.includes("lookaheadFrames") && !code.includes("limiterReleaseCoeff"), {});
check("R20 direct gain remains explicit", code.includes("l *= settings.gain") && code.includes("r *= settings.gain"), {});
check("R20 Dimension is Mid/Side without delay", code.includes("const mid = (l + r) * 0.5") && code.includes("const side = (l - r) * 0.5") && !code.includes("delayTime"), {});
check("R20 Spatial Dimension constants are installed", code.includes('const width = mode === "power" ? 2.05 : 1.60') && code.includes('const spatialMix = mode === "power" ? 0.55 : 0.32') && code.includes('const ap1Coeff = mode === "power" ? 0.68 : 0.55') && code.includes('const ap2Coeff = mode === "power" ? -0.52 : -0.42'), {});
...

{
  ...
  check("MVP DIMENSION strongly expands existing high-frequency sides",ratio>1.90&&ratio<2.25,{sideRatio:+ratio.toFixed(3)});
}

{
  ...
  const ratio=sideRms(slice(onOut,start))/sideRms(slice(offOut,start));
  check("MVP DIMENSION keeps deep bass side energy tightly centered",ratio<0.45,{lowSideRatio:+ratio.toFixed(3)});
}

{
  ...
  check("MVP DIMENSION OFF leaves center-heavy material centered",sideRms(offSteady)<1e-7,{offSideRms:+sideRms(offSteady).toFixed(7)});
  check("MVP DIMENSION creates obvious new side space from center-heavy material",sideToMid>0.34,{sideToMid:+sideToMid.toFixed(3)});
  check("MVP DIMENSION preserves a strong center while creating space",Math.abs(centerChangeDb)<0.5,{centerChangeDb:+centerChangeDb.toFixed(3)});
}

{
  ...
  check("MVP DIMENSION does not synthesize stereo bass",sideToMid<0.02,{bassSideToMid:+sideToMid.toFixed(4)});
}
...
if (fs.existsSync("src/lib/musicPlayer.ts")) {
  ...
  check("player uses R20 Worklet route",source.includes("/audio/mvpSoundModes-r20.worklet.js"),{});
