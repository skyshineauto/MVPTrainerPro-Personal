...
const enginePath = path.join(root, "src", "lib", "audio", "mvpSoundV7Engine.ts");
const workletDir = path.join(root, "public", "audioV7");
const workletPath = path.join(workletDir, "mvpSoundV7.worklet.js");
...
if (!fs.existsSync(playerPath)) {
  fail("Run this from the MVPTrainerPro-Personal repo root.");
}

const originalPlayer = fs.readFileSync(playerPath, "utf8");

const oldImport = 'from "./audio/mvpStudioEngine";';

if (!originalPlayer.includes(oldImport)) {
  fail("Current mvpStudioEngine import was not found.");
}
...
const patchedPlayer = originalPlayer.replace(
  oldImport,
  'from "./audio/mvpSoundV7Engine";'
);

if (patchedPlayer === originalPlayer) {
  fail("musicPlayer import replacement failed.");
}
...
try {
  ...
  fs.writeFileSync(workletPath, worklet, "utf8");
  ...
  if (result.status !== 0) {
    throw new Error("V7 AudioWorklet syntax check failed.");
  }
  ...
  console.log("");
  console.log("SUCCESS");
  console.log("MVP Sound V7 clean engine created.");
  console.log("Old audio engine files were NOT deleted.");
  console.log("musicPlayer now points only to mvpSoundV7Engine.");
  console.log("Production build passed.");
  console.log("");
  console.log("Changed:");
  console.log("  src/lib/musicPlayer.ts");
  console.log("  src/lib/audio/mvpSoundV7Engine.ts");
  console.log("  public/audioV7/mvpSoundV7.worklet.js");
} finally {
  if (!committed) {
    ...
    try {
      fs.rmSync(workletPath, { force: true });
    } catch {}

    console.error("");
    console.error("V7 install rolled back automatically.");
  }
}