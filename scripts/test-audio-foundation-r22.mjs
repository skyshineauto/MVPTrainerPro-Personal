...
const workletPath = process.argv[2] || "public/audio/mvpSoundModes-r22.worklet.js";
...
if (!Processor) throw new Error("R22 worklet did not register");
...
check("R22 has no compressor", !/\b(compGain|compressionDb|thresholdDb|ratio)\b/.test(code), {});
check("R22 has no limiter", !/\b(limiterGain|lookaheadFrames|limiterReleaseCoeff)\b/.test(code), {});
check("R22 keeps direct gain", code.includes("l *= settings.gain") && code.includes("r *= settings.gain"), {});
check(
  "R22 DIMENSION ULTRA constants installed",
  code.includes('const width = mode === "power" ? 2.90 : 2.00') &&
  code.includes('const spatialMix = mode === "power" ? 1.10 : 0.68') &&
  code.includes('const lowWidth = mode === "power" ? 0.02 : 0.12') &&
  code.includes('const centerFocus = mode === "power" ? 1.06 : 1.03'),
  {}
);
...

{
  ...
  check("DIMENSION ULTRA makes existing sides enormous", ratio > 2.65 && ratio < 3.20, { sideRatio:+ratio.toFixed(3) });
}

{
  ...
  check("DIMENSION ULTRA creates huge side space from center-heavy material", sideToMid > 0.62, { sideToMid:+sideToMid.toFixed(3) });
  check("DIMENSION ULTRA keeps the center strong", centerChangeDb > 0 && centerChangeDb < 0.7, { centerChangeDb:+centerChangeDb.toFixed(3) });
}

{
  ...
  check("DIMENSION ULTRA does not synthesize stereo bass", sideToMid < 0.01, { bassSideToMid:+sideToMid.toFixed(4) });
}

{
  ...
  const start = Math.round(sr*.5);
  const ratio = sideRms(slice(onOut,start)) / sideRms(slice(offOut,start));
  check("DIMENSION ULTRA keeps deep bass width controlled", ratio < 0.38, { lowSideRatio:+ratio.toFixed(3) });
}
...
if (fs.existsSync("src/lib/musicPlayer.ts")) {
  ...
  check("player uses R22 Worklet route", source.includes("/audio/mvpSoundModes-r22.worklet.js"), {});
...
if (fs.existsSync("src/features/music/MusicMiniPlayer.tsx")) {
  ...
  check("UI identifies MVP DIMENSION ULTRA", source.includes("MVP DIMENSION ULTRA"), {});
