...
const workletPath = process.argv[2] || "public/audio/mvpSoundModes-r18.worklet.js";
...
if (!Processor) throw new Error("R18 worklet did not register");
...
check("R18 has no compressor stage", !code.includes("compGain") && !code.includes("compressionDb") && !code.includes("thresholdDb"), {});
check("R18 has no limiter stage", !code.includes("limiterGain") && !code.includes("lookaheadFrames") && !code.includes("limiterReleaseCoeff"), {});
check("R18 direct gain remains explicit", code.includes("l *= settings.gain") && code.includes("r *= settings.gain"), {});
check("R18 Dimension is Mid/Side without delay", code.includes("const mid = (l + r) * 0.5") && code.includes("const side = (l - r) * 0.5") && !code.includes("delayTime"), {});
...

{
  ...
  check("MVP DIMENSION expands high-frequency sides",ratio>1.22&&ratio<1.34,{sideRatio:+ratio.toFixed(3)});
}

{
  ...
  const ratio=sideRms(slice(onOut,start))/sideRms(slice(offOut,start));
  check("MVP DIMENSION keeps low-end width tighter",ratio<0.72,{lowSideRatio:+ratio.toFixed(3)});
}
...
if (fs.existsSync("src/lib/musicPlayer.ts")) {
  ...
  check("player uses R18 Worklet route",source.includes("/audio/mvpSoundModes-r18.worklet.js"),{});
