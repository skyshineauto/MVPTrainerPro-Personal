...
import { WorkoutRoadmapRail3D, type WorkoutRoadmapRailItem } from "./WorkoutRoadmapRail3D";
...
export function WorkoutPlayerPage({ params }: any) {
  ...
  const roadmapAccentFor = (index: number) => roadmapExerciseAccents[Math.abs(index) % roadmapExerciseAccents.length];
  const roadmapRailItems: WorkoutRoadmapRailItem[] = items.map((row, index) => {
    const isCurrent = index === activeIdx;
    const isDone = !!row.completed_at;
    const isNext = index === nextIncompleteIndex && !isCurrent && !isDone;
    return {
      id: String(row.id),
      name: row.exercise?.name ?? row.exercise?.title ?? `Exercise ${index + 1}`,
      accent: roadmapAccentFor(index),
      state: isCurrent ? "current" : isDone ? "done" : isNext ? "next" : "remaining",
    };
  });
      ...
      <div className="tr-workoutSessionCard">
        <Card title={sessionLabel} tone="blue">
          <section
            ...
            style={{ "--tr-roadmap-progress": progressRatio, "--tr-roadmap-count": Math.max(items.length, 1), "--tr-rm-active": roadmapAccentFor(activeIdx) } as any}
          >
            ...
            <WorkoutRoadmapRail3D
              items={roadmapRailItems}
            ...
            <div className="tr-roadmapV2Body">
              <div
                className={`tr-roadmapV2Current ${current?.completed_at ? "is-complete" : ""}`}
                style={{ "--tr-rm-exercise": roadmapAccentFor(activeIdx) } as any}
              >
                <div className="tr-roadmapV2CurrentFx" aria-hidden="true" />
                <div className="tr-roadmapV2CurrentBanner" aria-label="Current exercise">
                    ...
                    {(() => {
                      ...
                      return (
                        <button
                          ...
                          style={{ "--tr-rm-exercise": roadmapAccentFor(index) } as any}
                        ...
                        >
                          ...
                          <span className="tr-roadmapV2AgendaCopy"><strong>{name}</strong><small>{visual.label} • PRIMARY TARGET</small></span>
...
{editing && (
  ...
  />
      ...
      <style>{`
        ...
        /* ============================================================
           R12.5E.7 — MODERN INDUSTRIAL LUXURY RENDERING RESET
           Satin graphite, dark glass, precision seams and controlled
           light. No neon-tube HUD language, no nested cheap cards.
           ============================================================ */
        .tr-roadmapV2{
          border-color:rgba(137,174,187,.20);
          background:
            radial-gradient(980px 330px at 22% -28%,rgba(181,218,229,.055),transparent 58%),
            radial-gradient(520px 250px at 103% 103%,rgba(197,112,65,.026),transparent 65%),
            linear-gradient(112deg,rgba(255,255,255,.020),transparent 17% 84%,rgba(255,255,255,.008)),
            linear-gradient(158deg,#0a1217 0%,#050b0f 54%,#020609 100%);
          box-shadow:
            inset 0 1px 0 rgba(235,248,251,.075),
            inset 0 -1px 0 rgba(0,0,0,.78),
            inset 20px 0 48px rgba(102,156,174,.018),
            inset -24px -10px 52px rgba(0,0,0,.26),
        ...
        .tr-roadmapV2::before{
          opacity:.42;
          background:
            linear-gradient(105deg,transparent 0 16%,rgba(244,252,253,.030) 20%,rgba(255,255,255,.055) 21.2%,rgba(244,252,253,.018) 23%,transparent 29% 100%),
            radial-gradient(600px 70px at 43% 0,rgba(231,247,250,.035),transparent 74%);
        }
        .tr-roadmapV2::after{opacity:.105;mix-blend-mode:soft-light}

        /* The R3F rail is the sculpted hardware centerpiece, not a light bar. */
        .tr-roadmapRail3D{height:128px;margin:2px -3px 7px;overflow:visible}
        .tr-roadmapRail3D::before{
          left:5%;right:5%;top:59%;height:24%;border-radius:50%;
          background:radial-gradient(ellipse at center,rgba(0,0,0,.42),rgba(0,0,0,.18) 47%,transparent 75%);
          filter:blur(10px);opacity:.72;
        }
        .tr-roadmapRail3DCanvas{inset:-2px -3px -4px;filter:drop-shadow(0 20px 17px rgba(0,0,0,.48))}
        .tr-roadmapRail3DHitGrid{left:10%;right:10%;top:17%;bottom:16%}
        .tr-roadmapRail3DHit:focus-visible{
          outline:1px solid rgba(206,235,242,.62);outline-offset:-11px;
          clip-path:polygon(18% 9%,82% 9%,96% 28%,96% 72%,82% 91%,18% 91%,4% 72%,4% 28%);
          filter:drop-shadow(0 0 7px rgba(130,204,224,.22));
        }
        .tr-roadmapRail3DFallback{
          left:4.5%;right:4.5%;top:50%;height:62px;padding:0 8%;
          clip-path:polygon(4% 14%,18% 4%,50% 0,82% 4%,96% 14%,100% 50%,96% 86%,82% 96%,50% 100%,18% 96%,4% 86%,0 50%);
          background:linear-gradient(180deg,#1b2429,#0c1216 48%,#05080a 100%);
          box-shadow:inset 0 1px 0 rgba(237,249,252,.11),inset 0 -1px 0 rgba(0,0,0,.85),0 15px 22px rgba(0,0,0,.40);
        }
        .tr-roadmapRail3DFallbackPort{
          width:34px;height:16px;border-radius:50%;clip-path:none;
          background:linear-gradient(180deg,#121a1f,#05090b);
          border:1px solid rgba(174,203,211,.18);box-shadow:inset 0 1px 0 rgba(255,255,255,.07);
          position:relative;
        }
        .tr-roadmapRail3DFallbackPort::after{content:"";position:absolute;left:28%;right:28%;top:44%;height:2px;border-radius:2px;background:var(--rail-accent);opacity:.30}
        .tr-roadmapRail3DFallbackPort.is-current::after{opacity:.95;box-shadow:0 0 8px color-mix(in srgb,var(--rail-accent) 30%,transparent)}
        .tr-roadmapRail3DFallbackPort.is-next::after{opacity:.62}.tr-roadmapRail3DFallbackPort.is-done::after{background:#76dfa9;opacity:.75}

        /* One continuous sculpted surface beneath the rail. */
        .tr-roadmapV2Body{
          border-top:1px solid rgba(170,205,217,.095);
          border-bottom:1px solid rgba(0,0,0,.56);
          background:
            radial-gradient(480px 260px at 14% 48%,color-mix(in srgb,var(--tr-rm-active,#49ddff) 5.5%,transparent),transparent 68%),
            linear-gradient(180deg,rgba(255,255,255,.012),transparent 13% 84%,rgba(0,0,0,.17));
          box-shadow:inset 0 1px 0 rgba(246,253,255,.018),inset 0 -20px 34px rgba(0,0,0,.16);
        }
        .tr-roadmapV2Body::before{
          left:46.8%;top:8%;bottom:8%;
          background:linear-gradient(180deg,transparent,rgba(193,220,229,.10) 14%,rgba(96,126,137,.085) 50%,rgba(193,220,229,.08) 86%,transparent);
          box-shadow:1px 0 0 rgba(0,0,0,.62);
        }
        .tr-roadmapV2Body::after{background:linear-gradient(90deg,transparent,rgba(237,249,251,.075) 21%,rgba(237,249,251,.018) 58%,transparent)}

        /* Current feature bay: no image box, no cyan slab, light behaves like a studio environment. */
        .tr-roadmapV2Current{min-height:236px;grid-template-columns:minmax(176px,45%) minmax(0,1fr);gap:8px;padding:8px 18px 8px 2px}
        .tr-roadmapV2Current::before{
          left:-4%;top:12%;width:55%;height:76%;border-radius:50%;
          background:
            radial-gradient(ellipse at 50% 53%,rgba(212,235,241,.045) 0 12%,color-mix(in srgb,var(--tr-rm-exercise) 7.5%,transparent) 32%,rgba(0,0,0,.055) 56%,transparent 73%);
          filter:blur(7px);opacity:.90;
        }
        .tr-roadmapV2Current::after{
          left:7%;width:34%;bottom:14px;height:10px;
          background:radial-gradient(ellipse at center,rgba(180,210,219,.105),rgba(0,0,0,.28) 44%,transparent 72%);
          filter:blur(6px);opacity:.70;
        }
        .tr-roadmapV2CurrentFx{opacity:.42}
        .tr-roadmapV2CurrentFx::before{
          left:-22%;top:-92%;width:64%;height:250%;transform:rotate(12deg);
          background:linear-gradient(90deg,transparent,rgba(236,249,251,.016),rgba(255,255,255,.055),rgba(236,249,251,.014),transparent);
          filter:blur(8px);animation:tr-roadmapV2-glass-sweep 11s ease-in-out infinite;
        }
        .tr-roadmapV2CurrentVisual{height:218px}
        .tr-roadmapV2CurrentVisual::before{
          width:88%;height:82%;
          background:radial-gradient(ellipse at center,rgba(225,242,246,.035),color-mix(in srgb,var(--tr-rm-exercise) 4.8%,transparent) 42%,transparent 70%);
          filter:blur(7px);opacity:.90;
        }
        .tr-roadmapV2CurrentVisual::after{
          left:18%;right:18%;bottom:14px;height:1px;
          background:linear-gradient(90deg,transparent,rgba(204,229,235,.20),color-mix(in srgb,var(--tr-rm-exercise) 28%,rgba(204,229,235,.15)),transparent);
          box-shadow:0 4px 13px rgba(0,0,0,.42);opacity:.72;
        }
        .tr-roadmapV2VisualHalo{display:none}
        .tr-roadmapV2CurrentVisual img{
          max-width:218px;max-height:218px;
          filter:drop-shadow(0 18px 16px rgba(0,0,0,.56)) drop-shadow(0 0 9px color-mix(in srgb,var(--tr-rm-exercise) 9%,transparent));
        }
        .tr-roadmapV2CurrentCopy{gap:6px}
        .tr-roadmapV2CurrentIndex{font-size:24px;color:color-mix(in srgb,var(--tr-rm-exercise) 54%,#dce7ea);text-shadow:none}
        .tr-roadmapV2CurrentCopy>strong{font-size:clamp(30px,2.85vw,39px);text-shadow:0 4px 14px rgba(0,0,0,.58)}
        .tr-roadmapV2MuscleLabel{font-size:9.3px;color:rgba(215,230,235,.80)}
        .tr-roadmapV2CurrentState{font-size:8.2px;text-shadow:none;color:color-mix(in srgb,var(--tr-rm-exercise) 52%,#dfe9ec)}

        /* Agenda lanes are engraved into the surface, not boxed cards. */
        .tr-roadmapV2Agenda{gap:7px;padding:9px 1px 7px 18px}
        .tr-roadmapV2Agenda::before{
          background:
            radial-gradient(420px 190px at 12% 12%,rgba(184,217,227,.028),transparent 70%),
            linear-gradient(180deg,rgba(255,255,255,.006),transparent 25%);
          opacity:1;
        }
        .tr-roadmapV2GroupHead{min-height:29px;padding:0 8px 0 7px;border-bottom-color:rgba(159,193,204,.10)!important}
        .tr-roadmapV2GroupHead>span{font-size:11.2px;color:rgba(218,232,237,.84);text-shadow:none}
        .tr-roadmapV2GroupHead>small{font-size:8.6px;color:rgba(192,210,216,.70)}
        .tr-roadmapV2Group.is-next .tr-roadmapV2GroupHead>span{color:#e7c98f;text-shadow:none}
        .tr-roadmapV2Group.is-done .tr-roadmapV2GroupHead>span{color:#85ddb0;text-shadow:none}
        .tr-roadmapV2AgendaRow{
          min-height:63px;grid-template-columns:64px minmax(0,1fr) 18px;gap:11px;padding:5px 9px 5px 8px;
          transition:transform .16s ease,filter .16s ease,background .16s ease;
        }
        .tr-roadmapV2AgendaRow::before{
          top:13px;bottom:13px;width:1.5px;border-radius:0;
          background:linear-gradient(180deg,transparent,color-mix(in srgb,var(--tr-rm-exercise) 68%,#cfdfe3 10%) 28% 72%,transparent);
          box-shadow:0 0 6px color-mix(in srgb,var(--tr-rm-exercise) 12%,transparent);
        }
        .tr-roadmapV2AgendaRow::after{
          left:8px;right:1px;
          background:linear-gradient(90deg,rgba(170,199,208,.11),rgba(110,141,151,.065) 43%,transparent 96%);
          box-shadow:0 1px 0 rgba(0,0,0,.66);
        }
        .tr-roadmapV2AgendaRow:hover,.tr-roadmapV2AgendaRow:focus-visible{
          transform:translateX(2px);filter:brightness(1.045);outline:none;
          background:radial-gradient(320px 80px at 12% 50%,color-mix(in srgb,var(--tr-rm-exercise) 4.5%,transparent),transparent 72%)!important;
        }
        .tr-roadmapV2AgendaRow.is-next{
          filter:none;
          background:radial-gradient(380px 90px at 13% 50%,rgba(210,165,90,.060),transparent 74%)!important;
        }
        .tr-roadmapV2AgendaRow.is-next::after{background:linear-gradient(90deg,rgba(213,181,126,.16),rgba(130,107,72,.06) 53%,transparent 96%)}
        .tr-roadmapV2AgendaRow.is-done{grid-template-columns:23px 64px minmax(0,1fr) 18px}
        .tr-roadmapV2AgendaVisual{width:62px;height:55px}
        .tr-roadmapV2AgendaVisual::before{width:55px;height:42px;background:radial-gradient(ellipse,rgba(214,235,241,.025),color-mix(in srgb,var(--tr-rm-exercise) 4.5%,transparent) 48%,transparent 72%);filter:blur(4px)}
        .tr-roadmapV2AgendaVisual img{max-width:60px;max-height:54px;filter:drop-shadow(0 9px 9px rgba(0,0,0,.48))}
        .tr-roadmapV2AgendaCopy{gap:3px}.tr-roadmapV2AgendaCopy>strong{font-size:16.7px;text-shadow:0 2px 7px rgba(0,0,0,.48)}
        .tr-roadmapV2AgendaCopy>small{font-size:8.8px;color:rgba(207,224,230,.76)}
        .tr-roadmapV2AgendaArrow{font-size:19px;color:rgba(199,218,224,.62);filter:none}
        .tr-roadmapV2DoneCheck{border-color:rgba(116,221,169,.38);background:radial-gradient(circle at 40% 28%,rgba(231,255,243,.12),rgba(39,126,83,.10) 48%,rgba(6,23,15,.82));box-shadow:inset 0 1px 0 rgba(255,255,255,.08),0 4px 10px rgba(0,0,0,.28)}

        @media (max-width:980px) and (min-width:721px){
          .tr-roadmapRail3D{height:116px}
          .tr-roadmapV2Current{grid-template-columns:minmax(158px,43%) minmax(0,1fr);min-height:222px}
          .tr-roadmapV2CurrentVisual{height:198px}.tr-roadmapV2CurrentVisual img{max-width:198px;max-height:198px}
        }
        @media (max-width:720px){
          .tr-roadmapV2{background:radial-gradient(500px 180px at 24% -20%,rgba(190,221,230,.045),transparent 62%),linear-gradient(160deg,#0a1217,#04090d 60%,#020609)}
          .tr-roadmapRail3D{height:92px;margin:0 -5px 3px}.tr-roadmapRail3DCanvas{inset:-1px -2px -3px}.tr-roadmapRail3DHitGrid{left:10%;right:10%;top:13%;bottom:13%}
          .tr-roadmapV2Current{min-height:122px;grid-template-columns:112px minmax(0,1fr);gap:6px;padding:3px 5px 6px 0}
          .tr-roadmapV2Current::before{left:-8%;top:8%;width:50%;height:82%;filter:blur(6px)}
          .tr-roadmapV2CurrentVisual{height:114px}.tr-roadmapV2CurrentVisual img{max-width:114px;max-height:114px}
          .tr-roadmapV2CurrentCopy{gap:3px}.tr-roadmapV2CurrentCopy>strong{font-size:clamp(22px,6.5vw,27px)}.tr-roadmapV2MuscleLabel{font-size:7.1px}.tr-roadmapV2CurrentState{font-size:6.6px}
          .tr-roadmapV2Agenda{gap:3px;padding:5px 0 1px}.tr-roadmapV2GroupHead{min-height:25px}.tr-roadmapV2GroupHead>span{font-size:10.2px}.tr-roadmapV2GroupHead>small{font-size:7.6px}
          .tr-roadmapV2AgendaRow{min-height:51px;grid-template-columns:49px minmax(0,1fr) 12px;gap:7px;padding:2px 4px 2px 3px}.tr-roadmapV2AgendaRow.is-done{grid-template-columns:18px 49px minmax(0,1fr) 12px;gap:6px}.tr-roadmapV2AgendaVisual{width:47px;height:45px}.tr-roadmapV2AgendaVisual img{max-width:46px;max-height:44px}.tr-roadmapV2AgendaCopy>strong{font-size:14.2px}.tr-roadmapV2AgendaCopy>small{font-size:7.45px}
        }
        @media (max-width:390px){
          .tr-roadmapRail3D{height:87px}.tr-roadmapV2Current{grid-template-columns:103px minmax(0,1fr);min-height:116px}.tr-roadmapV2CurrentVisual{height:106px}.tr-roadmapV2CurrentVisual img{max-width:106px;max-height:106px}
        }
        @media (max-width:345px){
          .tr-roadmapRail3D{height:82px}.tr-roadmapV2Current{grid-template-columns:94px minmax(0,1fr);min-height:108px}.tr-roadmapV2CurrentVisual{height:98px}.tr-roadmapV2CurrentVisual img{max-width:98px;max-height:98px}
        }



        /* ============================================================
           R12.5E.9 — PREMIUM REFERENCE POLISH
           Locks the approved E8 layout and adds only the final material,
           reflection, legibility and depth refinements requested.
           ============================================================ */
        .tr-roadmapV2{
          border-color:rgba(177,211,222,.28);
          box-shadow:
            inset 0 1px 0 rgba(242,251,253,.17),
            inset 0 -1px 0 rgba(255,158,65,.13),
            inset 0 0 0 1px rgba(0,0,0,.38),
            inset 24px 0 58px rgba(31,137,174,.038),
            inset -30px -18px 64px rgba(0,0,0,.37),
            0 24px 52px rgba(0,0,0,.44),
            0 0 0 1px rgba(0,0,0,.40);
        }
        .tr-roadmapV2::before{
          background:
            linear-gradient(111deg,transparent 0 49%,rgba(231,247,251,.035) 56%,rgba(255,255,255,.10) 60%,rgba(219,239,245,.025) 67%,transparent 74%),
            radial-gradient(760px 128px at 48% -7%,rgba(223,244,249,.07),transparent 70%),
            linear-gradient(180deg,rgba(255,255,255,.018),transparent 22%),
            repeating-linear-gradient(90deg,rgba(255,255,255,.006) 0 1px,transparent 1px 14px);
          opacity:.90;
        }
        .tr-roadmapV2::after{
          background:
            linear-gradient(180deg,rgba(255,255,255,.045),rgba(255,255,255,.012) 9%,transparent 23% 78%,rgba(0,0,0,.29)),
            radial-gradient(ellipse at 50% 102%,rgba(255,145,48,.035),transparent 56%);
          box-shadow:inset 0 0 68px rgba(0,0,0,.20),inset 0 18px 34px rgba(214,239,246,.014);
        }

        .tr-roadmapRailPremium{height:116px;margin-top:2px;margin-bottom:4px}
        .tr-roadmapRailPremiumSvg{
          filter:drop-shadow(0 15px 15px rgba(0,0,0,.43)) drop-shadow(0 1px 0 rgba(214,238,245,.09));
        }
        .tr-roadmapRailPremium::after{
          content:"";position:absolute;z-index:2;left:9%;right:9%;top:23%;height:13%;pointer-events:none;border-radius:50%;
          background:linear-gradient(90deg,transparent,rgba(234,249,253,.045) 18%,rgba(255,255,255,.09) 48%,rgba(231,247,251,.038) 78%,transparent);
          filter:blur(3px);opacity:.60;mix-blend-mode:screen;
        }

        .tr-roadmapV2Body{
          background:
            radial-gradient(560px 300px at 17% 48%,color-mix(in srgb,var(--tr-rm-active,#28d9ff) 11%,transparent),transparent 67%),
            radial-gradient(410px 170px at 72% -2%,rgba(234,248,252,.028),transparent 72%),
            linear-gradient(113deg,rgba(255,255,255,.020),transparent 25% 70%,rgba(255,255,255,.007)),
            linear-gradient(180deg,rgba(255,255,255,.014),transparent 20% 82%,rgba(0,0,0,.23));
          box-shadow:inset 0 1px 0 rgba(246,253,255,.035),inset 0 -20px 32px rgba(0,0,0,.21);
        }

        .tr-roadmapV2Current::before{
          background:
            radial-gradient(ellipse at 50% 51%,color-mix(in srgb,var(--tr-rm-exercise) 16%,transparent) 0 17%,color-mix(in srgb,var(--tr-rm-exercise) 6.5%,transparent) 39%,rgba(0,0,0,.07) 62%,transparent 75%);
          opacity:.98;
        }
        .tr-roadmapV2Current::after{
          background:radial-gradient(ellipse at center,color-mix(in srgb,var(--tr-rm-exercise) 27%,rgba(174,214,226,.19)),rgba(0,0,0,.46) 47%,transparent 74%);
          filter:blur(5px);opacity:.92;
          box-shadow:0 7px 21px rgba(0,0,0,.43),0 0 17px color-mix(in srgb,var(--tr-rm-exercise) 8%,transparent);
        }
        .tr-roadmapV2CurrentVisual::before{
          border-color:color-mix(in srgb,var(--tr-rm-exercise) 39%,rgba(220,239,245,.20));
          background:radial-gradient(ellipse at center,color-mix(in srgb,var(--tr-rm-exercise) 15%,transparent),rgba(2,8,12,.32) 57%,transparent 75%);
          box-shadow:
            0 0 17px color-mix(in srgb,var(--tr-rm-exercise) 12%,transparent),
            0 10px 22px rgba(0,0,0,.41),
            inset 0 1px 0 rgba(239,250,253,.11),
            inset 0 -2px 5px rgba(0,0,0,.28);
          opacity:.94;
        }
        .tr-roadmapV2CurrentVisual::after{
          opacity:.92;
          box-shadow:0 0 14px color-mix(in srgb,var(--tr-rm-exercise) 23%,transparent),0 4px 10px rgba(0,0,0,.35);
        }
        .tr-roadmapV2CurrentVisual img{
          filter:
            drop-shadow(0 22px 20px rgba(0,0,0,.62))
            drop-shadow(0 0 12px color-mix(in srgb,var(--tr-rm-exercise) 17%,transparent))
            drop-shadow(0 1px 1px rgba(231,247,251,.09));
        }
        .tr-roadmapV2MuscleLabel{color:rgba(223,237,242,.86);text-shadow:0 2px 8px rgba(0,0,0,.42)}
        .tr-roadmapV2CurrentState{color:color-mix(in srgb,var(--tr-rm-exercise) 79%,#e7f4f7)}

        .tr-roadmapV2Agenda{
          background:
            radial-gradient(470px 230px at 17% 2%,rgba(208,238,247,.035),transparent 69%),
            linear-gradient(180deg,rgba(255,255,255,.012),transparent 25%);
        }
        .tr-roadmapV2GroupHead>span{color:rgba(226,239,243,.89)}
        .tr-roadmapV2GroupHead>small{color:rgba(203,219,225,.77)}
        .tr-roadmapV2AgendaRow{
          border-color:rgba(165,199,211,.19);
          background:
            linear-gradient(180deg,rgba(242,251,253,.045),rgba(255,255,255,.010) 20%,transparent 42%),
            radial-gradient(270px 105px at 7% 50%,color-mix(in srgb,var(--tr-rm-exercise) 7%,transparent),transparent 73%),
            linear-gradient(105deg,color-mix(in srgb,var(--tr-rm-exercise) 4.8%,rgba(13,29,38,.90)),rgba(7,16,22,.86) 43%,rgba(2,8,12,.96) 100%)!important;
          box-shadow:
            inset 0 1px 0 rgba(239,250,253,.10),
            inset 0 -1px 0 rgba(0,0,0,.68),
            inset 15px 0 30px color-mix(in srgb,var(--tr-rm-exercise) 3.5%,transparent),
            0 9px 18px rgba(0,0,0,.23);
        }
        .tr-roadmapV2AgendaRow::after{
          background:linear-gradient(90deg,rgba(244,252,254,.15),rgba(222,241,247,.035) 55%,transparent);
        }
        .tr-roadmapV2AgendaRow.is-next{
          border-color:color-mix(in srgb,var(--tr-rm-exercise) 42%,rgba(223,193,135,.25));
          background:
            linear-gradient(180deg,rgba(255,250,237,.065),rgba(255,255,255,.012) 22%,transparent 45%),
            radial-gradient(300px 115px at 7% 50%,color-mix(in srgb,var(--tr-rm-exercise) 13%,transparent),transparent 72%),
            linear-gradient(105deg,color-mix(in srgb,var(--tr-rm-exercise) 8.5%,rgba(29,31,27,.94)),rgba(18,20,18,.89) 45%,rgba(4,9,10,.97) 100%)!important;
          box-shadow:
            inset 0 1px 0 rgba(255,248,229,.14),
            inset 0 -1px 0 rgba(67,43,13,.48),
            inset 20px 0 38px color-mix(in srgb,var(--tr-rm-exercise) 8%,transparent),
            0 12px 24px rgba(0,0,0,.29),
            0 0 24px color-mix(in srgb,var(--tr-rm-exercise) 5%,transparent);
        }
        .tr-roadmapV2AgendaVisual img{
          filter:drop-shadow(0 11px 10px rgba(0,0,0,.56)) drop-shadow(0 0 7px color-mix(in srgb,var(--tr-rm-exercise) 13%,transparent));
        }
        .tr-roadmapV2AgendaCopy>strong{color:#f8fbfc;text-shadow:0 3px 10px rgba(0,0,0,.58)}
        .tr-roadmapV2AgendaCopy>small{color:color-mix(in srgb,var(--tr-rm-exercise) 47%,rgba(220,235,240,.86))}

        @media (max-width:980px) and (min-width:721px){
          .tr-roadmapRailPremium{height:108px}
        }
        @media (max-width:720px){
          .tr-roadmapRailPremium{height:82px;margin-top:1px;margin-bottom:2px}
          .tr-roadmapV2MuscleLabel{color:rgba(226,239,243,.88)}
          .tr-roadmapV2AgendaCopy>small{color:color-mix(in srgb,var(--tr-rm-exercise) 45%,rgba(221,236,241,.84))}
          .tr-roadmapV2AgendaRow{box-shadow:inset 0 1px 0 rgba(239,250,253,.09),inset 0 -1px 0 rgba(0,0,0,.66),inset 11px 0 24px color-mix(in srgb,var(--tr-rm-exercise) 3%,transparent),0 7px 14px rgba(0,0,0,.20)}
          .tr-roadmapV2AgendaRow.is-next{box-shadow:inset 0 1px 0 rgba(255,248,229,.12),inset 0 -1px 0 rgba(67,43,13,.42),inset 14px 0 28px color-mix(in srgb,var(--tr-rm-exercise) 7%,transparent),0 9px 17px rgba(0,0,0,.24),0 0 16px color-mix(in srgb,var(--tr-rm-exercise) 4%,transparent)}
        }
        @media (max-width:390px){.tr-roadmapRailPremium{height:76px}}
        @media (max-width:345px){.tr-roadmapRailPremium{height:72px}}
        ...
        /* ============================================================
           R12.5E.5 — ENGINEERED COLOR-COORDINATED EXERCISE RAIL
           No rail text labels. Each exercise carries one accent color
           through the rail, Current preview, and agenda. State is
           expressed by hardware treatment, luminance, motion and check.
           ============================================================ */
        .tr-roadmapV2{
          --tr-rm-rail-metal:#07141b;
          --tr-rm-rail-edge:#244653;
          --tr-rm-rail-highlight:#82bdd0;
          padding-bottom:13px;
        }

        /* Precision chassis: two structural rails, a recessed bus, and segmented energy filaments. */
        .tr-roadmapV2Rail{height:66px;margin:2px 7px 10px;isolation:isolate}
        .tr-roadmapV2RailBed{
          left:4.5%;right:4.5%;top:31px;height:20px;border-radius:7px;overflow:visible;
          border:1px solid rgba(105,169,192,.18);
          background:
            linear-gradient(180deg,rgba(171,220,237,.095) 0 1px,transparent 1px 4px,rgba(0,0,0,.72) 4px 7px,rgba(22,52,63,.62) 7px 12px,rgba(1,7,10,.95) 12px 16px,rgba(91,149,169,.07) 16px 17px,rgba(0,0,0,.88) 17px 100%),
            linear-gradient(90deg,#02080b,#102833 13%,#07151c 50%,#102833 87%,#02080b);
          box-shadow:
            inset 0 2px 0 rgba(255,255,255,.035),
            inset 0 -3px 5px rgba(0,0,0,.84),
            inset 0 0 0 3px rgba(0,0,0,.20),
            0 1px 0 rgba(157,215,235,.045),
            0 7px 16px rgba(0,0,0,.34),
            0 0 22px rgba(76,184,220,.035);
        }
        .tr-roadmapV2RailBed::before{
          content:"";position:absolute;left:1.2%;right:1.2%;top:50%;height:5px;transform:translateY(-50%);border-radius:2px;
          border-top:1px solid rgba(177,226,240,.10);border-bottom:1px solid rgba(0,0,0,.78);
          background:linear-gradient(180deg,rgba(18,48,60,.95),rgba(3,12,17,.98));
          box-shadow:inset 0 1px 2px rgba(255,255,255,.035),inset 0 -1px 2px rgba(0,0,0,.9);
        }
        .tr-roadmapV2RailBed::after{
          content:"";position:absolute;left:2%;right:2%;top:3px;height:1px;border-radius:99px;
          background:linear-gradient(90deg,transparent,rgba(179,226,240,.20) 16%,rgba(179,226,240,.08) 50%,rgba(179,226,240,.20) 84%,transparent);
          opacity:.9;pointer-events:none;
        }
        .tr-roadmapV2Segment{
          top:31px;height:3px;z-index:3;border-radius:1px;
          background:linear-gradient(90deg,var(--tr-rm-seg-a),var(--tr-rm-seg-b));
          box-shadow:0 0 7px color-mix(in srgb,var(--tr-rm-seg-a) 22%,transparent),0 0 7px color-mix(in srgb,var(--tr-rm-seg-b) 22%,transparent);
          filter:saturate(.85) brightness(.62);opacity:.66;
          transition:filter .25s ease,opacity .25s ease,box-shadow .25s ease;
        }
        .tr-roadmapV2Segment::after{
          content:"";position:absolute;inset:-2px 0;border-top:1px solid rgba(255,255,255,.025);border-bottom:1px solid rgba(0,0,0,.25);pointer-events:none;
        }
        .tr-roadmapV2Segment.is-current{filter:saturate(1.15) brightness(1.18);opacity:1;box-shadow:0 0 10px color-mix(in srgb,var(--tr-rm-seg-a) 45%,transparent),0 0 13px color-mix(in srgb,var(--tr-rm-seg-b) 45%,transparent)}
        .tr-roadmapV2Segment.is-next{filter:saturate(1) brightness(.90);opacity:.92}
        .tr-roadmapV2Segment.is-done{filter:saturate(.95) brightness(.88);opacity:.86}
        .tr-roadmapV2Segment.is-remaining{filter:saturate(.72) brightness(.48);opacity:.56}

        /* Nodes are precision docking modules, not circles. */
        .tr-roadmapV2Node{top:31px;width:56px;height:44px;transform:translate(-50%,-21px);display:grid;place-items:center;gap:0;overflow:visible}
        .tr-roadmapV2NodeFace{
          position:relative;width:48px;height:34px;border:0;border-radius:0;clip-path:polygon(12% 0,88% 0,100% 22%,100% 78%,88% 100%,12% 100%,0 78%,0 22%);
          background:
            linear-gradient(180deg,rgba(255,255,255,.13),transparent 12%),
            linear-gradient(90deg,rgba(255,255,255,.04),transparent 24% 76%,rgba(255,255,255,.035)),
            linear-gradient(180deg,#18313b 0%,#09181f 34%,#02090d 72%,#10242d 100%);
          box-shadow:
            0 0 0 1px color-mix(in srgb,var(--tr-rm-exercise) 46%,#213944),
            0 0 0 4px rgba(0,5,8,.94),
            0 0 0 5px rgba(123,184,205,.05),
            inset 0 1px 0 rgba(255,255,255,.13),
            inset 0 -5px 9px rgba(0,0,0,.62),
            0 7px 13px rgba(0,0,0,.48),
            0 0 12px color-mix(in srgb,var(--tr-rm-exercise) 12%,transparent);
          transition:transform .18s ease,filter .2s ease,box-shadow .22s ease;
        }
        .tr-roadmapV2NodeFace::before{
          content:"";position:absolute;left:5px;right:5px;top:5px;height:4px;border-radius:1px;
          background:linear-gradient(90deg,transparent,color-mix(in srgb,var(--tr-rm-exercise) 70%,white 8%) 18%,color-mix(in srgb,var(--tr-rm-exercise) 94%,white 4%) 50%,color-mix(in srgb,var(--tr-rm-exercise) 70%,white 8%) 82%,transparent);
          box-shadow:0 0 8px color-mix(in srgb,var(--tr-rm-exercise) 28%,transparent);opacity:.74;
        }
        .tr-roadmapV2NodeFace::after{
          content:"";position:absolute;inset:9px 8px 6px;clip-path:polygon(10% 0,90% 0,100% 25%,100% 75%,90% 100%,10% 100%,0 75%,0 25%);
          background:linear-gradient(180deg,color-mix(in srgb,var(--tr-rm-exercise) 29%,#18313b),#061016 58%,#02080b);
          box-shadow:inset 0 1px 0 rgba(255,255,255,.10),inset 0 -2px 4px rgba(0,0,0,.7);
        }
        .tr-roadmapV2NodeLens{
          position:relative;z-index:2;width:15px;height:6px;border-radius:2px;display:block;border:0;
          background:linear-gradient(90deg,color-mix(in srgb,var(--tr-rm-exercise) 28%,#061017),var(--tr-rm-exercise),color-mix(in srgb,var(--tr-rm-exercise) 28%,#061017));
          box-shadow:0 0 8px color-mix(in srgb,var(--tr-rm-exercise) 36%,transparent),inset 0 1px 0 rgba(255,255,255,.30);
        }
        .tr-roadmapV2Node:hover .tr-roadmapV2NodeFace,.tr-roadmapV2Node:focus-visible .tr-roadmapV2NodeFace{transform:translateY(-1px);filter:brightness(1.12);outline:none}
        .tr-roadmapV2Node:focus-visible{outline:none}

        .tr-roadmapV2Node:not(.is-current):not(.is-next):not(.is-done) .tr-roadmapV2NodeFace{
          filter:saturate(.72) brightness(.82);
          box-shadow:0 0 0 1px color-mix(in srgb,var(--tr-rm-exercise) 34%,#334b55),0 0 0 4px rgba(0,5,8,.94),inset 0 1px 0 rgba(255,255,255,.10),inset 0 -5px 9px rgba(0,0,0,.62),0 7px 13px rgba(0,0,0,.48),0 0 10px color-mix(in srgb,var(--tr-rm-exercise) 11%,transparent);
        }
        .tr-roadmapV2Node.is-next .tr-roadmapV2NodeFace{
          filter:saturate(1.05) brightness(1.02);
          box-shadow:0 0 0 1px color-mix(in srgb,var(--tr-rm-exercise) 78%,white 4%),0 0 0 4px rgba(0,5,8,.95),0 0 0 6px color-mix(in srgb,var(--tr-rm-exercise) 9%,transparent),inset 0 1px 0 rgba(255,255,255,.16),inset 0 -5px 9px rgba(0,0,0,.60),0 8px 14px rgba(0,0,0,.50),0 0 18px color-mix(in srgb,var(--tr-rm-exercise) 27%,transparent);
        }
        .tr-roadmapV2Node.is-current .tr-roadmapV2NodeFace{
          width:54px;height:38px;filter:saturate(1.18) brightness(1.12);
          background:linear-gradient(180deg,rgba(255,255,255,.18),transparent 13%),linear-gradient(180deg,color-mix(in srgb,var(--tr-rm-exercise) 28%,#132d38),#06131a 45%,#02080c 76%,color-mix(in srgb,var(--tr-rm-exercise) 14%,#0b2029));
          box-shadow:0 0 0 1px color-mix(in srgb,var(--tr-rm-exercise) 92%,white 7%),0 0 0 4px rgba(0,6,9,.96),0 0 0 7px color-mix(in srgb,var(--tr-rm-exercise) 13%,transparent),inset 0 1px 0 rgba(255,255,255,.21),inset 0 -6px 10px rgba(0,0,0,.57),0 9px 16px rgba(0,0,0,.52),0 0 26px color-mix(in srgb,var(--tr-rm-exercise) 48%,transparent);
          animation:tr-roadmapV2-engine-core 2.8s ease-in-out infinite;
        }
        .tr-roadmapV2Node.is-current .tr-roadmapV2NodeLens{width:19px;height:7px;background:linear-gradient(90deg,color-mix(in srgb,var(--tr-rm-exercise) 35%,#061017),color-mix(in srgb,var(--tr-rm-exercise) 86%,white 14%),color-mix(in srgb,var(--tr-rm-exercise) 35%,#061017));box-shadow:0 0 10px color-mix(in srgb,var(--tr-rm-exercise) 62%,transparent),0 0 18px color-mix(in srgb,var(--tr-rm-exercise) 34%,transparent),inset 0 1px 0 rgba(255,255,255,.48)}
        .tr-roadmapV2Node.is-done .tr-roadmapV2NodeFace{
          filter:none;background:linear-gradient(180deg,rgba(255,255,255,.14),transparent 14%),linear-gradient(180deg,color-mix(in srgb,var(--tr-rm-exercise) 18%,#173329),#071a14 47%,#02100b 78%);
          box-shadow:0 0 0 1px color-mix(in srgb,var(--tr-rm-exercise) 58%,#6be7aa),0 0 0 4px rgba(0,7,7,.95),inset 0 1px 0 rgba(255,255,255,.17),inset 0 -5px 9px rgba(0,0,0,.58),0 8px 14px rgba(0,0,0,.48),0 0 17px rgba(76,224,153,.18);
        }
        .tr-roadmapV2Node.is-done b{position:relative;z-index:3;color:#eafff4;font-size:16px;line-height:1;text-shadow:0 0 8px rgba(73,229,157,.55)}

        /* Color travels with the exercise into the Current zone. No nested image panel. */
        .tr-roadmapV2Body{
          position:relative;grid-template-columns:minmax(340px,.43fr) minmax(0,.57fr);gap:0;align-items:stretch;
          border-top:1px solid rgba(116,190,217,.12);
          background:linear-gradient(180deg,rgba(255,255,255,.012),transparent 17%),linear-gradient(90deg,rgba(17,50,64,.10),transparent 46%,rgba(0,0,0,.10));
          box-shadow:inset 0 1px 0 rgba(255,255,255,.016),inset 0 20px 45px rgba(33,150,190,.018);
        }
        .tr-roadmapV2Body::before{content:none}
        .tr-roadmapV2Current{
          position:relative;align-self:stretch;min-height:226px;grid-template-columns:minmax(166px,.46fr) minmax(0,.54fr);gap:13px;padding:17px 18px 16px 8px;border:0;border-right:1px solid rgba(120,190,215,.11);border-radius:0;
          background:
            radial-gradient(330px 240px at 21% 49%,color-mix(in srgb,var(--tr-rm-exercise) 18%,transparent),transparent 67%),
            radial-gradient(270px 170px at 98% 108%,rgba(255,157,54,.045),transparent 70%),
            linear-gradient(100deg,rgba(255,255,255,.014),transparent 38%);
          box-shadow:inset -22px 0 34px rgba(0,0,0,.11);overflow:hidden;
        }
        .tr-roadmapV2Current::before{content:"";position:absolute;left:7%;right:41%;top:0;height:2px;background:linear-gradient(90deg,transparent,var(--tr-rm-exercise) 24%,color-mix(in srgb,var(--tr-rm-exercise) 72%,white 28%) 52%,var(--tr-rm-exercise) 78%,transparent);box-shadow:0 0 14px color-mix(in srgb,var(--tr-rm-exercise) 34%,transparent)}
        .tr-roadmapV2Current::after{content:"";position:absolute;inset:0;pointer-events:none;background:linear-gradient(112deg,transparent 4% 34%,rgba(190,242,255,.025) 45%,transparent 56%),radial-gradient(240px 95px at 21% 90%,color-mix(in srgb,var(--tr-rm-exercise) 8%,transparent),transparent 72%);box-shadow:inset 20px 0 28px rgba(0,0,0,.03),inset -20px 0 34px rgba(0,0,0,.15)}
        .tr-roadmapV2CurrentFx{background:linear-gradient(112deg,transparent 8%,color-mix(in srgb,var(--tr-rm-exercise) 7%,white 3%) 42%,transparent 61%)}
        .tr-roadmapV2CurrentVisual{height:194px;border-radius:0;background:none;filter:drop-shadow(0 15px 20px rgba(0,0,0,.40))}
        .tr-roadmapV2CurrentVisual::before{width:88%;height:88%;background:radial-gradient(circle,color-mix(in srgb,var(--tr-rm-exercise) 16%,transparent),transparent 68%);filter:blur(12px)}
        .tr-roadmapV2CurrentVisual::after{left:11%;right:11%;bottom:1px;height:2px;background:linear-gradient(90deg,transparent,var(--tr-rm-exercise),transparent);box-shadow:0 0 15px color-mix(in srgb,var(--tr-rm-exercise) 32%,transparent)}
        .tr-roadmapV2VisualHalo{width:76%;border-color:color-mix(in srgb,var(--tr-rm-exercise) 11%,transparent);box-shadow:0 0 30px color-mix(in srgb,var(--tr-rm-exercise) 8%,transparent),inset 0 0 22px color-mix(in srgb,var(--tr-rm-exercise) 5%,transparent)}
        .tr-roadmapV2CurrentVisual img{max-width:190px;max-height:190px;filter:drop-shadow(0 10px 12px rgba(0,0,0,.52)) drop-shadow(0 0 14px color-mix(in srgb,var(--tr-rm-exercise) 14%,transparent))}
        .tr-roadmapV2CurrentCopy{gap:6px}.tr-roadmapV2CurrentIndex{color:color-mix(in srgb,var(--tr-rm-exercise) 74%,#c5f7ff);text-shadow:0 0 14px color-mix(in srgb,var(--tr-rm-exercise) 18%,transparent)}
        .tr-roadmapV2CurrentCopy>strong{font-size:clamp(26px,2.6vw,35px)}
        .tr-roadmapV2CurrentState{color:color-mix(in srgb,var(--tr-rm-exercise) 78%,white 18%)}

        /* Agenda rows share each exercise color through an engineered side bus and local surface wash. */
        .tr-roadmapV2Agenda{align-self:stretch;gap:6px;padding:9px 0 4px 16px;background:linear-gradient(90deg,rgba(0,0,0,.08),transparent 18%)}
        .tr-roadmapV2Group{gap:2px}.tr-roadmapV2GroupHead{min-height:25px;padding:0 7px;border-bottom:1px solid rgba(111,178,203,.10)}
        .tr-roadmapV2GroupHead>span{font-size:9px;letter-spacing:.14em}.tr-roadmapV2GroupHead>small{font-size:7.6px}
        .tr-roadmapV2AgendaRow{
          min-height:54px;grid-template-columns:56px minmax(0,1fr) 16px;gap:10px;padding:4px 9px 4px 8px;border:0;border-radius:0;border-bottom:1px solid rgba(107,170,194,.075);
          background:
            linear-gradient(90deg,color-mix(in srgb,var(--tr-rm-exercise) 9%,transparent),transparent 40%),
            linear-gradient(180deg,rgba(255,255,255,.012),transparent 48%);
          box-shadow:inset 0 1px 0 rgba(255,255,255,.012);
        }
        .tr-roadmapV2AgendaRow::before{left:0;top:8px;bottom:8px;width:3px;background:var(--tr-rm-exercise);box-shadow:0 0 12px color-mix(in srgb,var(--tr-rm-exercise) 35%,transparent)}
        .tr-roadmapV2AgendaRow::after{content:"";position:absolute;left:7px;right:26%;top:0;height:1px;background:linear-gradient(90deg,color-mix(in srgb,var(--tr-rm-exercise) 24%,transparent),transparent 78%);opacity:.60;pointer-events:none}
        .tr-roadmapV2AgendaRow:hover,.tr-roadmapV2AgendaRow:focus-visible{background:linear-gradient(90deg,color-mix(in srgb,var(--tr-rm-exercise) 15%,transparent),rgba(18,55,68,.028) 65%,transparent);transform:translateX(2px)}
        .tr-roadmapV2AgendaRow.is-next{background:linear-gradient(90deg,color-mix(in srgb,var(--tr-rm-exercise) 14%,transparent),transparent 48%),linear-gradient(180deg,rgba(255,255,255,.018),transparent 48%)}
        .tr-roadmapV2AgendaRow.is-done{grid-template-columns:22px 56px minmax(0,1fr) 16px;background:linear-gradient(90deg,rgba(72,222,151,.055),color-mix(in srgb,var(--tr-rm-exercise) 6%,transparent) 34%,transparent 66%)}
        .tr-roadmapV2AgendaVisual{width:54px;height:48px;border-radius:0;background:radial-gradient(circle at 50% 50%,color-mix(in srgb,var(--tr-rm-exercise) 12%,transparent),transparent 69%);filter:drop-shadow(0 7px 9px rgba(0,0,0,.31))}
        .tr-roadmapV2AgendaVisual img{max-width:52px;max-height:47px;filter:drop-shadow(0 6px 8px rgba(0,0,0,.43)) drop-shadow(0 0 10px color-mix(in srgb,var(--tr-rm-exercise) 10%,transparent))}
        .tr-roadmapV2AgendaRow.is-next .tr-roadmapV2AgendaVisual,.tr-roadmapV2AgendaRow.is-done .tr-roadmapV2AgendaVisual{background:radial-gradient(circle at 50% 50%,color-mix(in srgb,var(--tr-rm-exercise) 15%,transparent),transparent 69%)}
        .tr-roadmapV2AgendaCopy>strong{font-size:15px}.tr-roadmapV2AgendaCopy>small{font-size:8px;letter-spacing:.07em;color:rgba(202,224,233,.74)}
        .tr-roadmapV2AgendaRow.is-next .tr-roadmapV2AgendaCopy>small{color:color-mix(in srgb,var(--tr-rm-exercise) 58%,#f7f4e9)}
        .tr-roadmapV2DoneCheck{position:relative;z-index:2;margin-left:1px}

        @keyframes tr-roadmapV2-engine-core{
          0%,100%{filter:saturate(1.12) brightness(1.08);transform:translateY(0);}
          50%{filter:saturate(1.24) brightness(1.18);transform:translateY(-.5px);}
        }

        @media (max-width:900px) and (min-width:721px){
          .tr-roadmapV2Body{grid-template-columns:minmax(300px,.43fr) minmax(0,.57fr)}
          .tr-roadmapV2Current{grid-template-columns:150px minmax(0,1fr);min-height:214px;padding-left:7px;padding-right:14px}.tr-roadmapV2CurrentVisual{height:178px}.tr-roadmapV2CurrentVisual img{max-width:174px;max-height:174px}
          .tr-roadmapV2Agenda{padding-left:13px}.tr-roadmapV2AgendaCopy>strong{font-size:14.2px}.tr-roadmapV2AgendaCopy>small{font-size:7.6px}
        }

        @media (max-width:720px){
          .tr-roadmapV2{padding:10px 9px 10px}.tr-roadmapV2Header{margin-bottom:2px}
          .tr-roadmapV2Rail{height:54px;margin:2px 1px 7px}.tr-roadmapV2RailBed{left:6%;right:6%;top:27px;height:16px;border-radius:6px}.tr-roadmapV2Segment{top:27px;height:3px}
          .tr-roadmapV2Node{top:27px;width:44px;height:38px;transform:translate(-50%,-19px)}.tr-roadmapV2NodeFace{width:38px;height:28px}.tr-roadmapV2Node.is-current .tr-roadmapV2NodeFace{width:44px;height:32px}.tr-roadmapV2NodeLens{width:12px;height:5px}.tr-roadmapV2Node.is-current .tr-roadmapV2NodeLens{width:16px;height:6px}.tr-roadmapV2Node.is-done b{font-size:14px}
          .tr-roadmapV2Body{grid-template-columns:1fr;gap:0;border-top-color:rgba(117,194,222,.11);background:linear-gradient(180deg,rgba(255,255,255,.01),transparent 19%)}
          .tr-roadmapV2Current{min-height:112px;grid-template-columns:104px minmax(0,1fr);gap:8px;padding:7px 7px 8px 1px;border-right:0;border-bottom:1px solid rgba(116,191,218,.11)}
          .tr-roadmapV2Current::before{left:4%;right:62%}.tr-roadmapV2CurrentVisual{height:99px}.tr-roadmapV2CurrentVisual img{max-width:101px;max-height:101px}.tr-roadmapV2CurrentCopy{gap:3px}.tr-roadmapV2CurrentIndex{font-size:18px}.tr-roadmapV2CurrentIndex small{font-size:8px}.tr-roadmapV2CurrentCopy>strong{font-size:clamp(21px,6.1vw,25px)}.tr-roadmapV2MuscleLabel{font-size:7px}.tr-roadmapV2CurrentState{font-size:6.5px}
          .tr-roadmapV2Agenda{gap:3px;padding:5px 0 1px}.tr-roadmapV2Group{gap:0}.tr-roadmapV2GroupHead{min-height:23px;padding:0 5px}.tr-roadmapV2GroupHead>span{font-size:9.6px}.tr-roadmapV2GroupHead>small{font-size:7.4px}
          .tr-roadmapV2AgendaRow{min-height:48px;grid-template-columns:46px minmax(0,1fr) 12px;gap:7px;padding:2px 5px}.tr-roadmapV2AgendaRow.is-done{grid-template-columns:17px 46px minmax(0,1fr) 12px;gap:6px}.tr-roadmapV2AgendaVisual{width:44px;height:42px}.tr-roadmapV2AgendaVisual img{max-width:43px;max-height:41px}.tr-roadmapV2DoneCheck{width:17px;height:17px;font-size:8.5px}.tr-roadmapV2AgendaCopy>strong{font-size:13.6px}.tr-roadmapV2AgendaCopy>small{font-size:7.2px}.tr-roadmapV2AgendaArrow{font-size:15px}
        }

        @media (max-width:390px){
          .tr-roadmapV2Rail{height:52px}.tr-roadmapV2Node{width:40px}.tr-roadmapV2NodeFace{width:35px;height:27px}.tr-roadmapV2Node.is-current .tr-roadmapV2NodeFace{width:41px;height:31px}
          .tr-roadmapV2Current{grid-template-columns:94px minmax(0,1fr);min-height:106px;gap:7px}.tr-roadmapV2CurrentVisual{height:92px}.tr-roadmapV2CurrentVisual img{max-width:94px;max-height:94px}.tr-roadmapV2CurrentCopy>strong{font-size:20px}.tr-roadmapV2MuscleLabel{font-size:6.5px}.tr-roadmapV2CurrentState{font-size:6.1px}
          .tr-roadmapV2AgendaRow{grid-template-columns:43px minmax(0,1fr) 11px;gap:6px;min-height:46px}.tr-roadmapV2AgendaRow.is-done{grid-template-columns:16px 43px minmax(0,1fr) 11px;gap:5px}.tr-roadmapV2AgendaVisual{width:41px;height:40px}.tr-roadmapV2AgendaVisual img{max-width:40px;max-height:39px}.tr-roadmapV2AgendaCopy>strong{font-size:13px}.tr-roadmapV2AgendaCopy>small{font-size:6.8px}
        }

        @media (max-width:345px){
          .tr-roadmapV2Rail{height:50px}.tr-roadmapV2Node{width:37px}.tr-roadmapV2NodeFace{width:33px;height:25px}.tr-roadmapV2Node.is-current .tr-roadmapV2NodeFace{width:38px;height:29px}.tr-roadmapV2NodeLens{width:10px}.tr-roadmapV2Node.is-current .tr-roadmapV2NodeLens{width:14px}
          .tr-roadmapV2Current{grid-template-columns:86px minmax(0,1fr);min-height:100px;gap:6px}.tr-roadmapV2CurrentVisual{height:84px}.tr-roadmapV2CurrentVisual img{max-width:87px;max-height:87px}.tr-roadmapV2CurrentCopy>strong{font-size:18.5px}.tr-roadmapV2MuscleLabel{font-size:6px}.tr-roadmapV2CurrentState{font-size:5.8px}
          .tr-roadmapV2GroupHead>span{font-size:9px}.tr-roadmapV2AgendaRow{grid-template-columns:40px minmax(0,1fr) 10px;gap:5px;min-height:44px}.tr-roadmapV2AgendaRow.is-done{grid-template-columns:15px 40px minmax(0,1fr) 10px;gap:4px}.tr-roadmapV2AgendaVisual{width:38px;height:37px}.tr-roadmapV2AgendaVisual img{max-width:37px;max-height:36px}.tr-roadmapV2AgendaCopy>strong{font-size:12.3px}.tr-roadmapV2AgendaCopy>small{font-size:6.4px}.tr-roadmapV2AgendaArrow{font-size:14px}
        }


        /* ============================================================
           R12.5E.6 — R3F RENDERED ROADMAP / ONE SCULPTED SURFACE
           Real Three.js rail hardware with physically lit graphite,
           glass status ports, emissive energy, bloom and responsive
           camera framing. HTML remains sharp and accessible.
           ============================================================ */
        .tr-roadmapV2{
          position:relative;
          overflow:hidden;
          padding:14px 15px 15px;
          border-color:rgba(89,191,226,.24);
          background:
            radial-gradient(920px 340px at 18% -18%,rgba(30,184,233,.105),transparent 58%),
            radial-gradient(620px 300px at 98% 104%,rgba(255,139,57,.045),transparent 64%),
            linear-gradient(112deg,rgba(255,255,255,.026),transparent 19% 81%,rgba(255,255,255,.012)),
            linear-gradient(160deg,rgba(7,24,33,.985),rgba(2,10,15,.997) 58%,rgba(1,6,10,.999));
          box-shadow:
            inset 0 1px 0 rgba(220,249,255,.10),
            inset 0 -1px 0 rgba(0,0,0,.72),
            inset 18px 0 42px rgba(0,125,176,.028),
            inset -24px 0 46px rgba(0,0,0,.26),
            0 18px 40px rgba(0,0,0,.34),
            0 0 42px rgba(29,178,225,.055);
          backdrop-filter:blur(16px) saturate(118%);
        }
        .tr-roadmapV2::before{
          content:"";position:absolute;inset:0;pointer-events:none;opacity:.56;animation:none;
          background:
            linear-gradient(106deg,transparent 0 13%,rgba(222,249,255,.045) 18%,transparent 24% 100%),
            radial-gradient(520px 80px at 35% 0,rgba(211,248,255,.055),transparent 76%);
        ...
        .tr-roadmapV2::after{
          ...
          background-size:96px 96px;mix-blend-mode:soft-light;
        }
        .tr-roadmapV2Header,.tr-roadmapRail3D,.tr-roadmapV2Body{position:relative;z-index:2}
        .tr-roadmapV2Header{margin-bottom:2px;gap:18px}
        .tr-roadmapV2Title>span{font-size:8.2px;color:rgba(193,221,232,.72)}
        .tr-roadmapV2TitleLine>strong{font-size:16.5px;letter-spacing:.012em}
        .tr-roadmapV2Edit{height:25px;min-width:53px;border-radius:8px;background:linear-gradient(180deg,rgba(25,58,74,.62),rgba(4,16,23,.86));border-color:rgba(83,200,238,.30);box-shadow:inset 0 1px 0 rgba(255,255,255,.07),0 7px 16px rgba(0,0,0,.20),0 0 14px rgba(48,196,239,.045)}
        .tr-roadmapV2Count{min-width:80px;gap:3px}
        .tr-roadmapV2Count>strong{font-size:31px;letter-spacing:-.055em}
        .tr-roadmapV2Count>strong>span{font-size:17px;margin-left:2px}
        .tr-roadmapV2Count>small{font-size:7.8px;color:rgba(226,239,244,.82);letter-spacing:.14em}

        /* Real R3F / Three.js engineered status rail. */
        .tr-roadmapRail3D{height:112px;margin:1px -2px 9px;isolation:isolate;overflow:visible}
        .tr-roadmapRail3D::before{
          content:"";position:absolute;left:4%;right:4%;top:52%;height:38%;transform:translateY(-50%);pointer-events:none;border-radius:50%;
          background:radial-gradient(ellipse at center,rgba(37,171,218,.09),rgba(4,22,31,.025) 48%,transparent 74%);filter:blur(11px);opacity:.78;
        }
        .tr-roadmapRail3DCanvas{position:absolute;inset:-9px -4px -8px;pointer-events:none;filter:drop-shadow(0 16px 18px rgba(0,0,0,.36))}
        .tr-roadmapRail3DCanvas canvas{display:block!important;width:100%!important;height:100%!important;background:transparent!important}
        .tr-roadmapRail3DHitGrid{position:absolute;z-index:4;left:8.7%;right:8.7%;top:20%;bottom:18%;display:grid;grid-template-columns:repeat(var(--tr-roadmap-rail-count),minmax(0,1fr));align-items:stretch;pointer-events:none}
        .tr-roadmapRail3DHit{appearance:none;display:block;width:100%;height:100%;border:0;background:transparent;padding:0;margin:0;cursor:pointer;pointer-events:auto;border-radius:0;outline:none}
        .tr-roadmapRail3DHit:focus-visible{outline:1px solid rgba(147,235,255,.72);outline-offset:-8px;clip-path:polygon(16% 2%,84% 2%,98% 24%,98% 76%,84% 98%,16% 98%,2% 76%,2% 24%);filter:drop-shadow(0 0 8px rgba(83,218,255,.45))}
        .tr-roadmapRail3DFallback{position:absolute;left:5%;right:5%;top:50%;height:44px;transform:translateY(-50%);display:flex;align-items:center;justify-content:space-between;padding:0 5%;clip-path:polygon(2% 0,98% 0,100% 22%,100% 78%,98% 100%,2% 100%,0 78%,0 22%);background:linear-gradient(180deg,#17313b,#061118 55%,#02070a);box-shadow:inset 0 1px 0 rgba(255,255,255,.1),0 10px 20px rgba(0,0,0,.35)}
        .tr-roadmapRail3DFallbackPort{width:24px;height:24px;clip-path:polygon(50% 0,88% 18%,100% 50%,88% 82%,50% 100%,12% 82%,0 50%,12% 18%);background:radial-gradient(circle,var(--rail-accent) 0 12%,#0a1a22 42%,#02080c 76%);box-shadow:0 0 12px color-mix(in srgb,var(--rail-accent) 28%,transparent)}

        /* One continuous sculpted interior. No card-inside-card surfaces. */
        .tr-roadmapV2Body{
          display:grid;grid-template-columns:minmax(365px,.47fr) minmax(0,.53fr);gap:0;align-items:stretch;overflow:hidden;
          border-top:1px solid rgba(108,191,219,.13);
          border-bottom:1px solid rgba(0,0,0,.42);
          background:
            radial-gradient(420px 290px at 17% 48%,color-mix(in srgb,var(--tr-rm-active,#49ddff) 10%,transparent),transparent 70%),
            radial-gradient(520px 240px at 91% 24%,rgba(65,155,191,.045),transparent 74%),
            linear-gradient(180deg,rgba(255,255,255,.017),transparent 15% 84%,rgba(0,0,0,.18));
          box-shadow:inset 0 1px 0 rgba(255,255,255,.025),inset 0 -16px 30px rgba(0,0,0,.12);
        }
        .tr-roadmapV2Body::before{
          content:"";position:absolute;left:46.8%;top:9%;bottom:9%;width:1px;z-index:0;pointer-events:none;
          background:linear-gradient(180deg,transparent,rgba(138,209,232,.16) 16%,rgba(52,124,149,.10) 50%,rgba(138,209,232,.12) 84%,transparent);
          box-shadow:1px 0 0 rgba(0,0,0,.46);
        }
        .tr-roadmapV2Body::after{
          content:"";position:absolute;left:1.5%;right:1.5%;top:0;height:1px;z-index:0;pointer-events:none;
          background:linear-gradient(90deg,transparent,rgba(221,250,255,.13) 22%,rgba(221,250,255,.045) 58%,transparent);
        }
        .tr-roadmapV2Current{
          align-self:stretch;min-height:238px;display:grid;grid-template-columns:minmax(170px,45%) minmax(0,1fr);align-items:center;gap:13px;padding:10px 19px 11px 4px;
          border:0!important;border-radius:0!important;background:none!important;box-shadow:none!important;overflow:visible;
        }
        .tr-roadmapV2Current::before{
          content:"";position:absolute;left:-7%;top:7%;width:58%;height:86%;border-radius:50%;pointer-events:none;
          background:radial-gradient(ellipse at 52% 48%,color-mix(in srgb,var(--tr-rm-exercise) 22%,transparent),color-mix(in srgb,var(--tr-rm-exercise) 7%,transparent) 38%,transparent 70%);filter:blur(9px);opacity:.96;
        }
        .tr-roadmapV2Current::after{
          content:"";position:absolute;left:4%;width:39%;bottom:12px;height:14px;border-radius:50%;pointer-events:none;
          background:radial-gradient(ellipse at center,color-mix(in srgb,var(--tr-rm-exercise) 29%,transparent),rgba(0,0,0,.24) 45%,transparent 73%);filter:blur(6px);opacity:.78;
        }
        .tr-roadmapV2CurrentFx{position:absolute;inset:0;overflow:hidden;pointer-events:none;opacity:.66;background:none!important}
        .tr-roadmapV2CurrentFx::before{
          content:"";position:absolute;left:-14%;top:-80%;width:72%;height:230%;transform:rotate(13deg);
          background:linear-gradient(90deg,transparent,rgba(213,248,255,.038),rgba(255,255,255,.08),rgba(213,248,255,.025),transparent);filter:blur(7px);animation:tr-roadmapV2-glass-sweep 8.5s ease-in-out infinite;
        }
        .tr-roadmapV2CurrentVisual{position:relative;z-index:2;height:216px;display:grid;place-items:center;overflow:visible;border:0!important;border-radius:0!important;background:none!important;box-shadow:none!important;filter:none!important}
        .tr-roadmapV2CurrentVisual::before{content:"";position:absolute;width:92%;height:92%;border-radius:50%;background:radial-gradient(circle,color-mix(in srgb,var(--tr-rm-exercise) 16%,transparent),color-mix(in srgb,var(--tr-rm-exercise) 5%,transparent) 48%,transparent 71%);filter:blur(8px);opacity:.9}
        .tr-roadmapV2CurrentVisual::after{content:"";position:absolute;left:13%;right:13%;bottom:16px;height:2px;border-radius:50%;background:linear-gradient(90deg,transparent,color-mix(in srgb,var(--tr-rm-exercise) 65%,white 8%),transparent);box-shadow:0 0 14px color-mix(in srgb,var(--tr-rm-exercise) 32%,transparent);opacity:.7}
        .tr-roadmapV2VisualHalo{position:absolute;width:80%;aspect-ratio:1;border:0!important;border-radius:50%;background:radial-gradient(circle at 50% 52%,transparent 0 48%,color-mix(in srgb,var(--tr-rm-exercise) 7%,transparent) 62%,transparent 75%);box-shadow:none!important;opacity:.9}
        .tr-roadmapV2CurrentVisual img{position:relative;z-index:3;max-width:216px;max-height:216px;object-fit:contain;filter:drop-shadow(0 16px 15px rgba(0,0,0,.50)) drop-shadow(0 0 18px color-mix(in srgb,var(--tr-rm-exercise) 17%,transparent))}
        .tr-roadmapV2CurrentCopy{position:relative;z-index:3;align-content:center;gap:7px;min-width:0}
        .tr-roadmapV2CurrentIndex{font-size:23px;line-height:1;color:color-mix(in srgb,var(--tr-rm-exercise) 78%,#d6f9ff);text-shadow:0 0 15px color-mix(in srgb,var(--tr-rm-exercise) 18%,transparent)}
        .tr-roadmapV2CurrentIndex small{font-size:11px;color:rgba(210,229,237,.68)}
        .tr-roadmapV2CurrentCopy>strong{font-size:clamp(29px,2.7vw,38px);line-height:.98;letter-spacing:-.025em;text-wrap:balance;text-shadow:0 4px 15px rgba(0,0,0,.48)}
        .tr-roadmapV2MuscleLabel{font-size:9px;line-height:1.2;color:rgba(205,230,239,.78);letter-spacing:.10em}
        .tr-roadmapV2CurrentState{font-size:8px;color:color-mix(in srgb,var(--tr-rm-exercise) 78%,white 18%);letter-spacing:.16em;text-shadow:0 0 10px color-mix(in srgb,var(--tr-rm-exercise) 18%,transparent)}

        /* Agenda is etched into the same surface, never stacked cards. */
        .tr-roadmapV2Agenda{position:relative;z-index:2;align-self:stretch;display:grid;align-content:start;gap:8px;padding:10px 2px 8px 19px;background:none!important;border:0!important;box-shadow:none!important}
        .tr-roadmapV2Agenda::before{content:"";position:absolute;inset:0;pointer-events:none;background:radial-gradient(360px 170px at 18% 15%,rgba(61,161,197,.055),transparent 72%),linear-gradient(180deg,rgba(255,255,255,.009),transparent 28%);opacity:.88}
        .tr-roadmapV2Group{position:relative;z-index:1;display:grid;gap:2px}
        .tr-roadmapV2GroupHead{min-height:28px;display:flex;align-items:center;justify-content:space-between;padding:0 8px 0 6px;border:0!important;border-bottom:1px solid rgba(112,181,207,.10)!important;background:none!important;box-shadow:none!important}
        .tr-roadmapV2GroupHead>span{font-size:10.2px;line-height:1;font-weight:1100;letter-spacing:.16em;color:rgba(205,229,238,.82);text-shadow:0 2px 5px rgba(0,0,0,.65)}
        .tr-roadmapV2GroupHead>small{font-size:8.1px;color:rgba(193,215,224,.67)}
        .tr-roadmapV2Group.is-next .tr-roadmapV2GroupHead>span{color:#f3d598;text-shadow:0 0 13px rgba(238,194,115,.21)}
        .tr-roadmapV2Group.is-done .tr-roadmapV2GroupHead>span{color:#8af1bd;text-shadow:0 0 12px rgba(70,225,150,.19)}
        .tr-roadmapV2AgendaRow{
          position:relative;min-height:62px;grid-template-columns:62px minmax(0,1fr) 18px;gap:11px;padding:5px 9px 5px 8px;
          border:0!important;border-radius:0!important;background:none!important;box-shadow:none!important;overflow:visible;
          transition:transform .18s ease,filter .18s ease;
        }
        .tr-roadmapV2AgendaRow::before{content:"";position:absolute;left:0;top:11px;bottom:11px;width:2px;border-radius:2px;background:linear-gradient(180deg,color-mix(in srgb,var(--tr-rm-exercise) 32%,transparent),var(--tr-rm-exercise),color-mix(in srgb,var(--tr-rm-exercise) 24%,transparent));box-shadow:0 0 11px color-mix(in srgb,var(--tr-rm-exercise) 27%,transparent)}
        .tr-roadmapV2AgendaRow::after{content:"";position:absolute;left:8px;right:2px;bottom:0;height:1px;background:linear-gradient(90deg,color-mix(in srgb,var(--tr-rm-exercise) 17%,rgba(133,193,214,.13)),rgba(122,182,203,.075) 42%,transparent 94%);box-shadow:0 1px 0 rgba(0,0,0,.55)}
        .tr-roadmapV2AgendaRow:hover,.tr-roadmapV2AgendaRow:focus-visible{transform:translateX(3px);filter:brightness(1.08);outline:none}
        .tr-roadmapV2AgendaRow.is-next{filter:saturate(1.04)}
        .tr-roadmapV2AgendaRow.is-next::after{background:linear-gradient(90deg,color-mix(in srgb,var(--tr-rm-exercise) 30%,rgba(247,209,144,.20)),rgba(142,113,66,.08) 58%,transparent 95%)}
        .tr-roadmapV2AgendaRow.is-done{grid-template-columns:23px 62px minmax(0,1fr) 18px;gap:9px}
        .tr-roadmapV2AgendaVisual{width:60px;height:54px;display:grid;place-items:center;border:0!important;border-radius:0!important;background:none!important;box-shadow:none!important;filter:none!important}
        .tr-roadmapV2AgendaVisual::before{content:"";position:absolute;width:54px;height:44px;border-radius:50%;background:radial-gradient(ellipse,color-mix(in srgb,var(--tr-rm-exercise) 12%,transparent),transparent 70%);filter:blur(5px);opacity:.9}
        .tr-roadmapV2AgendaVisual img{position:relative;z-index:2;max-width:58px;max-height:53px;object-fit:contain;filter:drop-shadow(0 8px 8px rgba(0,0,0,.42)) drop-shadow(0 0 9px color-mix(in srgb,var(--tr-rm-exercise) 10%,transparent))}
        .tr-roadmapV2AgendaCopy{gap:3px}.tr-roadmapV2AgendaCopy>strong{font-size:16.2px;line-height:1.04;color:#f7fbfd;text-shadow:0 2px 7px rgba(0,0,0,.48)}
        .tr-roadmapV2AgendaCopy>small{font-size:8.4px;line-height:1.15;color:rgba(205,225,233,.75);letter-spacing:.07em}
        .tr-roadmapV2AgendaArrow{font-size:20px;color:rgba(201,231,240,.66);filter:drop-shadow(0 0 6px rgba(69,190,226,.10))}
        .tr-roadmapV2DoneCheck{position:relative;width:22px;height:22px;display:grid;place-items:center;margin:0;border:1px solid rgba(88,236,163,.47);border-radius:50%;background:radial-gradient(circle at 40% 30%,rgba(210,255,233,.19),rgba(39,144,93,.16) 48%,rgba(4,33,20,.88));color:#9af3c5;font-size:11px;font-weight:1100;box-shadow:inset 0 1px 0 rgba(255,255,255,.12),0 0 14px rgba(67,225,148,.17)}

        @keyframes tr-roadmapV2-glass-sweep{
          0%,16%{transform:translateX(-32%) rotate(13deg);opacity:.02}
          46%{opacity:.54}
          72%,100%{transform:translateX(138%) rotate(13deg);opacity:.02}
        }

        @media (max-width:980px) and (min-width:721px){
          .tr-roadmapRail3D{height:104px}
          .tr-roadmapV2Body{grid-template-columns:minmax(330px,.46fr) minmax(0,.54fr)}
          .tr-roadmapV2Current{grid-template-columns:minmax(152px,43%) minmax(0,1fr);min-height:220px;padding-right:14px}.tr-roadmapV2CurrentVisual{height:194px}.tr-roadmapV2CurrentVisual img{max-width:194px;max-height:194px}.tr-roadmapV2CurrentCopy>strong{font-size:30px}
          .tr-roadmapV2Agenda{padding-left:16px}.tr-roadmapV2AgendaRow{min-height:59px;grid-template-columns:58px minmax(0,1fr) 17px}.tr-roadmapV2AgendaRow.is-done{grid-template-columns:22px 58px minmax(0,1fr) 17px}.tr-roadmapV2AgendaVisual{width:56px;height:51px}.tr-roadmapV2AgendaVisual img{max-width:55px;max-height:50px}.tr-roadmapV2AgendaCopy>strong{font-size:15px}
        }

        @media (max-width:720px){
          .tr-roadmapV2{padding:10px 9px 10px;border-radius:17px}
          .tr-roadmapV2Header{margin-bottom:0;gap:8px}.tr-roadmapV2Title>span{font-size:6.8px}.tr-roadmapV2TitleLine{gap:5px}.tr-roadmapV2TitleLine>strong{font-size:12.2px}.tr-roadmapV2Edit{height:23px;min-width:46px;padding:0 5px}.tr-roadmapV2Edit span{font-size:6px}.tr-roadmapV2Edit svg{width:10px;height:10px}
          .tr-roadmapV2Count{min-width:60px;gap:2px}.tr-roadmapV2Count>strong{font-size:23px}.tr-roadmapV2Count>strong>span{font-size:13px}.tr-roadmapV2Count>small{font-size:6.6px;letter-spacing:.12em}
          .tr-roadmapRail3D{height:80px;margin:0 -5px 4px}.tr-roadmapRail3DCanvas{inset:-6px -2px -5px}.tr-roadmapRail3DHitGrid{left:8.2%;right:8.2%;top:18%;bottom:15%}
          .tr-roadmapV2Body{grid-template-columns:1fr;gap:0;border-top-color:rgba(116,193,220,.12);overflow:hidden}
          .tr-roadmapV2Body::before{display:none}
          .tr-roadmapV2Current{min-height:126px;grid-template-columns:112px minmax(0,1fr);gap:8px;padding:4px 6px 7px 0;border-bottom:1px solid rgba(112,184,209,.10)!important}
          .tr-roadmapV2Current::before{left:-12%;top:3%;width:55%;height:93%;filter:blur(8px)}.tr-roadmapV2Current::after{left:1%;width:34%;bottom:7px;height:11px}
          .tr-roadmapV2CurrentVisual{height:116px}.tr-roadmapV2CurrentVisual img{max-width:116px;max-height:116px}.tr-roadmapV2CurrentVisual::after{left:12%;right:12%;bottom:8px}
          .tr-roadmapV2CurrentCopy{gap:3px}.tr-roadmapV2CurrentIndex{font-size:18px}.tr-roadmapV2CurrentIndex small{font-size:8px}.tr-roadmapV2CurrentCopy>strong{font-size:clamp(22px,6.4vw,27px);line-height:.98}.tr-roadmapV2MuscleLabel{font-size:7px}.tr-roadmapV2CurrentState{font-size:6.5px}
          .tr-roadmapV2Agenda{gap:4px;padding:5px 0 1px}.tr-roadmapV2Group{gap:1px}.tr-roadmapV2GroupHead{min-height:24px;padding:0 4px}.tr-roadmapV2GroupHead>span{font-size:9.6px}.tr-roadmapV2GroupHead>small{font-size:7.4px}
          .tr-roadmapV2AgendaRow{min-height:50px;grid-template-columns:48px minmax(0,1fr) 12px;gap:7px;padding:2px 4px 2px 3px}.tr-roadmapV2AgendaRow.is-done{grid-template-columns:18px 48px minmax(0,1fr) 12px;gap:6px}.tr-roadmapV2AgendaRow::before{top:8px;bottom:8px}.tr-roadmapV2AgendaVisual{width:46px;height:44px}.tr-roadmapV2AgendaVisual::before{width:42px;height:36px}.tr-roadmapV2AgendaVisual img{max-width:45px;max-height:43px}.tr-roadmapV2AgendaCopy>strong{font-size:14px}.tr-roadmapV2AgendaCopy>small{font-size:7.3px}.tr-roadmapV2AgendaArrow{font-size:16px}.tr-roadmapV2DoneCheck{width:18px;height:18px;font-size:9px}
        }
        @media (max-width:390px){
          .tr-roadmapV2TitleLine>strong{font-size:11.4px}.tr-roadmapV2Count{min-width:55px}.tr-roadmapV2Count>strong{font-size:21px}.tr-roadmapV2Count>strong>span{font-size:12px}.tr-roadmapV2Count>small{font-size:6.2px}
          .tr-roadmapRail3D{height:75px;margin-bottom:2px}.tr-roadmapV2Current{grid-template-columns:103px minmax(0,1fr);min-height:119px;gap:6px}.tr-roadmapV2CurrentVisual{height:108px}.tr-roadmapV2CurrentVisual img{max-width:108px;max-height:108px}.tr-roadmapV2CurrentCopy>strong{font-size:21px}.tr-roadmapV2MuscleLabel{font-size:6.5px}.tr-roadmapV2CurrentState{font-size:6.1px}
          .tr-roadmapV2GroupHead>span{font-size:9.1px}.tr-roadmapV2AgendaRow{grid-template-columns:45px minmax(0,1fr) 11px;gap:6px;min-height:48px}.tr-roadmapV2AgendaRow.is-done{grid-template-columns:17px 45px minmax(0,1fr) 11px;gap:5px}.tr-roadmapV2AgendaVisual{width:43px;height:42px}.tr-roadmapV2AgendaVisual img{max-width:42px;max-height:41px}.tr-roadmapV2AgendaCopy>strong{font-size:13.4px}.tr-roadmapV2AgendaCopy>small{font-size:6.9px}
        }
        @media (max-width:345px){
          .tr-roadmapV2{padding-left:7px;padding-right:7px}.tr-roadmapV2TitleLine>strong{font-size:10.6px}.tr-roadmapV2Edit{min-width:42px}.tr-roadmapV2Count{min-width:51px}.tr-roadmapV2Count>strong{font-size:19.5px}.tr-roadmapV2Count>strong>span{font-size:11px}.tr-roadmapV2Count>small{font-size:5.8px}
          .tr-roadmapRail3D{height:70px;margin-left:-6px;margin-right:-6px}.tr-roadmapV2Current{grid-template-columns:94px minmax(0,1fr);min-height:110px;gap:5px}.tr-roadmapV2CurrentVisual{height:100px}.tr-roadmapV2CurrentVisual img{max-width:100px;max-height:100px}.tr-roadmapV2CurrentCopy>strong{font-size:19.2px}.tr-roadmapV2MuscleLabel{font-size:6px}.tr-roadmapV2CurrentState{font-size:5.8px}
          .tr-roadmapV2GroupHead>span{font-size:8.7px}.tr-roadmapV2AgendaRow{grid-template-columns:42px minmax(0,1fr) 10px;gap:5px;min-height:46px}.tr-roadmapV2AgendaRow.is-done{grid-template-columns:16px 42px minmax(0,1fr) 10px;gap:4px}.tr-roadmapV2AgendaVisual{width:40px;height:40px}.tr-roadmapV2AgendaVisual img{max-width:39px;max-height:39px}.tr-roadmapV2AgendaCopy>strong{font-size:12.7px}.tr-roadmapV2AgendaCopy>small{font-size:6.5px}.tr-roadmapV2AgendaArrow{font-size:14px}
        }
        ...
        /* ============================================================
           R12.5E.8 — PREMIUM REFERENCE ROADMAP
           Reference-match sculpted rail + dedicated exercise identity
           colors. SVG hardware rendering, one surface, no box-in-box.
           ============================================================ */
        .tr-roadmapV2{
          --tr-rm-cyan:#28d9ff;
          --tr-rm-gold:#ffae35;
          border:1px solid rgba(168,202,214,.24);
          border-radius:20px;
          background:
            radial-gradient(780px 300px at 18% 30%,color-mix(in srgb,var(--tr-rm-active,#28d9ff) 8%,transparent),transparent 66%),
            radial-gradient(480px 250px at 101% 102%,rgba(255,146,56,.055),transparent 68%),
            linear-gradient(110deg,rgba(255,255,255,.028),transparent 22% 76%,rgba(255,255,255,.012)),
            linear-gradient(160deg,#101a21 0%,#071016 46%,#02070b 100%);
          box-shadow:
            inset 0 1px 0 rgba(236,249,253,.14),
            inset 0 -1px 0 rgba(255,154,60,.12),
            inset 0 0 0 1px rgba(0,0,0,.36),
            inset 22px 0 52px rgba(28,129,165,.035),
            inset -28px -16px 58px rgba(0,0,0,.34),
        ...
        .tr-roadmapV2::before{
          opacity:.82;
          background:
            linear-gradient(112deg,transparent 0 54%,rgba(228,245,250,.055) 60%,rgba(255,255,255,.09) 62%,rgba(210,234,241,.018) 69%,transparent 76%),
            radial-gradient(620px 120px at 45% -8%,rgba(214,240,248,.055),transparent 72%),
            repeating-linear-gradient(90deg,rgba(255,255,255,.007) 0 1px,transparent 1px 13px);
          mix-blend-mode:screen;
        }
        .tr-roadmapV2::after{
          opacity:1;
          background:linear-gradient(180deg,rgba(255,255,255,.035),transparent 16%,transparent 84%,rgba(0,0,0,.26));
          box-shadow:inset 0 0 56px rgba(0,0,0,.18);
        }

        .tr-roadmapV2Header{margin-bottom:0;padding:1px 1px 0;gap:14px}
        .tr-roadmapV2Title>span{font-size:8.4px;letter-spacing:.16em;color:rgba(177,205,216,.64)}
        .tr-roadmapV2TitleLine{gap:9px}.tr-roadmapV2TitleLine>strong{font-size:16.4px;letter-spacing:-.012em}
        .tr-roadmapV2Edit{
          height:27px;min-width:58px;padding:0 8px;border-radius:9px;
          border:1px solid rgba(92,205,239,.29);
          background:linear-gradient(180deg,rgba(29,56,68,.66),rgba(7,20,28,.82));
          box-shadow:inset 0 1px 0 rgba(225,248,255,.12),0 6px 14px rgba(0,0,0,.24);
        }
        .tr-roadmapV2Edit:hover{border-color:rgba(101,221,255,.48);box-shadow:inset 0 1px 0 rgba(238,251,255,.16),0 0 16px rgba(35,200,245,.10),0 6px 14px rgba(0,0,0,.28)}
        .tr-roadmapV2Count{min-width:75px;gap:0;align-self:center}
        .tr-roadmapV2Count>strong{font-size:29px;line-height:.92;letter-spacing:-.055em;color:#35d8f5;text-shadow:0 0 13px rgba(40,217,255,.20)}
        .tr-roadmapV2Count>strong>span{font-size:17px;margin-left:2px;color:rgba(232,244,248,.86);letter-spacing:-.04em}
        .tr-roadmapV2Count>small{margin-top:4px;font-size:8px;line-height:1;letter-spacing:.13em;color:rgba(218,232,237,.76)}

        /* Sculpted precision rail: one manufactured assembly, not blobs on a line. */
        .tr-roadmapRailPremium{height:112px;margin:3px -5px 3px;overflow:visible;isolation:isolate}
        .tr-roadmapRailPremium::before{
          content:"";position:absolute;z-index:0;left:5%;right:5%;bottom:8px;height:24px;border-radius:50%;
          background:radial-gradient(ellipse at center,rgba(0,0,0,.54),rgba(0,0,0,.22) 50%,transparent 78%);filter:blur(8px);opacity:.82;
        }
        .tr-roadmapRailPremiumSvg{position:absolute;z-index:1;inset:0;width:100%;height:100%;display:block;overflow:visible;filter:drop-shadow(0 13px 13px rgba(0,0,0,.40))}
        .tr-roadmapRailPremiumNode{transform-box:fill-box;transform-origin:center}
        .tr-roadmapRailPremiumNode.is-current{animation:tr-rm8-current 3.4s ease-in-out infinite}
        .tr-roadmapRailPremiumNode.is-next{animation:tr-rm8-next 4.6s ease-in-out infinite}
        @keyframes tr-rm8-current{0%,100%{filter:brightness(1)}50%{filter:brightness(1.10)}}
        @keyframes tr-rm8-next{0%,100%{filter:brightness(.98)}50%{filter:brightness(1.055)}}
        .tr-roadmapRailPremiumCore.is-current{animation:tr-rm8-core 2.6s ease-in-out infinite}
        @keyframes tr-rm8-core{0%,100%{opacity:.86}50%{opacity:1}}
        .tr-roadmapRail3DHitGrid{left:5.7%;right:5.7%;top:17%;bottom:17%;z-index:5;display:grid;grid-template-columns:repeat(var(--tr-roadmap-rail-count),minmax(0,1fr));pointer-events:none}
        .tr-roadmapRail3DHit{pointer-events:auto;border-radius:50%;cursor:pointer}
        .tr-roadmapRail3DHit:focus-visible{outline:2px solid rgba(211,243,251,.72);outline-offset:-17px;border-radius:50%;filter:drop-shadow(0 0 12px rgba(42,211,255,.30))}

        /* Main body remains one surface; depth comes from light and material, not nested boxes. */
        .tr-roadmapV2Body{
          grid-template-columns:minmax(0,46%) minmax(0,54%);gap:0;align-items:stretch;
          border-top:1px solid rgba(185,216,226,.11);
          border-bottom:0;
          background:
            radial-gradient(520px 280px at 17% 49%,color-mix(in srgb,var(--tr-rm-active,#28d9ff) 10%,transparent),transparent 68%),
            radial-gradient(420px 220px at 76% 8%,rgba(255,255,255,.018),transparent 70%),
            linear-gradient(180deg,rgba(255,255,255,.014),transparent 18% 82%,rgba(0,0,0,.20));
          box-shadow:inset 0 1px 0 rgba(246,253,255,.024),inset 0 -18px 28px rgba(0,0,0,.18);
        }
        .tr-roadmapV2Body::before{left:46%;top:8%;bottom:8%;width:1px;background:linear-gradient(180deg,transparent,rgba(186,218,229,.14) 18%,rgba(86,120,133,.11) 50%,rgba(186,218,229,.10) 82%,transparent);box-shadow:1px 0 0 rgba(0,0,0,.56)}
        .tr-roadmapV2Body::after{display:none}

        /* Current hero bay. No inner image rectangle. */
        .tr-roadmapV2Current{min-height:252px;grid-template-columns:minmax(188px,48%) minmax(0,1fr);gap:8px;padding:10px 18px 10px 4px;align-items:center}
        .tr-roadmapV2Current::before{
          left:-5%;top:4%;width:62%;height:91%;border-radius:50%;
          background:
            radial-gradient(ellipse at 50% 52%,color-mix(in srgb,var(--tr-rm-exercise) 13%,transparent) 0 18%,color-mix(in srgb,var(--tr-rm-exercise) 5%,transparent) 40%,rgba(0,0,0,.07) 61%,transparent 74%);
          filter:blur(8px);opacity:.92;
        }
        .tr-roadmapV2Current::after{
          left:8%;bottom:15px;width:34%;height:14px;border-radius:50%;
          background:radial-gradient(ellipse at center,color-mix(in srgb,var(--tr-rm-exercise) 22%,rgba(163,205,219,.18)),rgba(0,0,0,.43) 48%,transparent 73%);
          filter:blur(5px);opacity:.82;
          box-shadow:0 6px 18px rgba(0,0,0,.40);
        }
        .tr-roadmapV2CurrentFx{opacity:.68}
        .tr-roadmapV2CurrentFx::before{left:-25%;top:-85%;width:62%;height:240%;background:linear-gradient(90deg,transparent,rgba(224,247,253,.012),rgba(255,255,255,.055),rgba(231,248,252,.012),transparent);filter:blur(8px);animation:tr-roadmapV2-glass-sweep 13s ease-in-out infinite}
        .tr-roadmapV2CurrentVisual{height:236px;overflow:visible}
        .tr-roadmapV2CurrentVisual::before{
          content:"";position:absolute;left:50%;bottom:9px;width:78%;height:18px;transform:translateX(-50%);border-radius:50%;
          border:1px solid color-mix(in srgb,var(--tr-rm-exercise) 33%,rgba(212,234,241,.18));
          background:radial-gradient(ellipse at center,color-mix(in srgb,var(--tr-rm-exercise) 11%,transparent),rgba(2,8,12,.28) 58%,transparent 74%);
          box-shadow:0 0 13px color-mix(in srgb,var(--tr-rm-exercise) 10%,transparent),0 9px 18px rgba(0,0,0,.35),inset 0 1px 0 rgba(232,248,252,.08);
          filter:none;opacity:.82;
        }
        .tr-roadmapV2CurrentVisual::after{left:27%;right:27%;bottom:15px;height:1px;background:linear-gradient(90deg,transparent,color-mix(in srgb,var(--tr-rm-exercise) 55%,#d7edf3),transparent);box-shadow:0 0 11px color-mix(in srgb,var(--tr-rm-exercise) 17%,transparent);opacity:.75}
        .tr-roadmapV2CurrentVisual img{max-width:228px;max-height:228px;filter:drop-shadow(0 20px 18px rgba(0,0,0,.58)) drop-shadow(0 0 10px color-mix(in srgb,var(--tr-rm-exercise) 14%,transparent))}
        .tr-roadmapV2CurrentCopy{gap:6px;padding-right:3px}.tr-roadmapV2CurrentIndex{font-size:27px;color:color-mix(in srgb,var(--tr-rm-exercise) 70%,#d9edf2)}.tr-roadmapV2CurrentIndex small{font-size:12px;color:rgba(202,220,227,.64)}
        .tr-roadmapV2CurrentCopy>strong{font-size:clamp(33px,3vw,43px);line-height:.96;letter-spacing:-.035em;color:#f7fafb;text-shadow:0 5px 16px rgba(0,0,0,.60)}
        .tr-roadmapV2MuscleLabel{font-size:10px;letter-spacing:.08em;color:rgba(215,232,238,.76)}
        .tr-roadmapV2CurrentState{font-size:9px;letter-spacing:.14em;color:color-mix(in srgb,var(--tr-rm-exercise) 72%,#dceef3);text-shadow:0 0 11px color-mix(in srgb,var(--tr-rm-exercise) 16%,transparent)}

        /* Premium glass agenda modules, sculpted with highlights and dedicated exercise color. */
        .tr-roadmapV2Agenda{gap:8px;padding:11px 2px 9px 20px}
        .tr-roadmapV2Agenda::before{background:radial-gradient(430px 230px at 16% 8%,rgba(195,229,240,.032),transparent 70%),linear-gradient(180deg,rgba(255,255,255,.009),transparent 26%);opacity:1}
        .tr-roadmapV2Group{gap:5px}.tr-roadmapV2GroupHead{min-height:29px;padding:0 8px 0 6px;border-bottom:1px solid rgba(170,202,213,.10)!important}
        .tr-roadmapV2GroupHead>span{font-size:11.5px;letter-spacing:.13em;color:rgba(219,234,239,.82)}.tr-roadmapV2GroupHead>small{font-size:9px;color:rgba(192,211,218,.68)}
        .tr-roadmapV2Group.is-next .tr-roadmapV2GroupHead>span{color:#efc980}.tr-roadmapV2Group.is-done .tr-roadmapV2GroupHead>span{color:#86e0b1}
        .tr-roadmapV2AgendaRow{
          min-height:62px;grid-template-columns:64px minmax(0,1fr) 19px;gap:12px;padding:5px 10px 5px 8px;
          border:1px solid rgba(151,190,204,.16);border-radius:13px;
          background:
            linear-gradient(105deg,color-mix(in srgb,var(--tr-rm-exercise) 4.5%,rgba(12,28,37,.88)),rgba(7,16,22,.82) 39%,rgba(2,8,12,.94) 100%)!important;
          box-shadow:
            inset 0 1px 0 rgba(231,248,253,.08),
            inset 0 -1px 0 rgba(0,0,0,.64),
            inset 15px 0 28px color-mix(in srgb,var(--tr-rm-exercise) 3%,transparent),
            0 8px 16px rgba(0,0,0,.20);
          overflow:hidden;
          transition:transform .16s ease,filter .16s ease,border-color .16s ease,box-shadow .16s ease;
        }
        .tr-roadmapV2AgendaRow::before{left:0;top:12px;bottom:12px;width:2px;border-radius:0 3px 3px 0;background:linear-gradient(180deg,transparent,var(--tr-rm-exercise) 24% 76%,transparent);box-shadow:0 0 8px color-mix(in srgb,var(--tr-rm-exercise) 28%,transparent)}
        .tr-roadmapV2AgendaRow::after{content:"";position:absolute;left:10px;right:16px;top:0;height:1px;background:linear-gradient(90deg,rgba(234,249,253,.11),rgba(220,240,246,.025) 54%,transparent);box-shadow:none}
        .tr-roadmapV2AgendaRow:hover,.tr-roadmapV2AgendaRow:focus-visible{transform:translateY(-1px);filter:brightness(1.06);border-color:color-mix(in srgb,var(--tr-rm-exercise) 34%,rgba(167,201,213,.18));box-shadow:inset 0 1px 0 rgba(241,252,255,.11),inset 15px 0 30px color-mix(in srgb,var(--tr-rm-exercise) 6%,transparent),0 11px 20px rgba(0,0,0,.28),0 0 18px color-mix(in srgb,var(--tr-rm-exercise) 5%,transparent);outline:none}
        .tr-roadmapV2AgendaRow.is-next{min-height:69px;border-color:color-mix(in srgb,var(--tr-rm-exercise) 36%,rgba(219,187,125,.22));background:linear-gradient(105deg,color-mix(in srgb,var(--tr-rm-exercise) 8%,rgba(27,31,28,.92)),rgba(21,22,19,.86) 44%,rgba(5,9,10,.96) 100%)!important;box-shadow:inset 0 1px 0 rgba(255,244,218,.10),inset 18px 0 34px color-mix(in srgb,var(--tr-rm-exercise) 7%,transparent),0 10px 20px rgba(0,0,0,.25),0 0 20px color-mix(in srgb,var(--tr-rm-exercise) 4%,transparent)}
        .tr-roadmapV2AgendaRow.is-done{grid-template-columns:24px 64px minmax(0,1fr) 19px;border-color:rgba(115,221,169,.20)}
        .tr-roadmapV2AgendaVisual{width:62px;height:55px;overflow:visible}.tr-roadmapV2AgendaVisual::before{width:56px;height:44px;background:radial-gradient(ellipse,color-mix(in srgb,var(--tr-rm-exercise) 10%,transparent),transparent 69%);filter:blur(5px);opacity:.86}.tr-roadmapV2AgendaVisual img{max-width:61px;max-height:55px;filter:drop-shadow(0 10px 9px rgba(0,0,0,.52)) drop-shadow(0 0 5px color-mix(in srgb,var(--tr-rm-exercise) 10%,transparent))}
        .tr-roadmapV2AgendaCopy{gap:3px}.tr-roadmapV2AgendaCopy>strong{font-size:17.4px;line-height:1.04;letter-spacing:-.018em;color:#f4f8f9;text-shadow:0 3px 9px rgba(0,0,0,.52)}.tr-roadmapV2AgendaCopy>small{font-size:9px;line-height:1.05;letter-spacing:.085em;color:color-mix(in srgb,var(--tr-rm-exercise) 42%,rgba(210,228,234,.78))}
        .tr-roadmapV2AgendaArrow{font-size:21px;color:color-mix(in srgb,var(--tr-rm-exercise) 52%,rgba(220,238,244,.70));text-shadow:0 0 8px color-mix(in srgb,var(--tr-rm-exercise) 13%,transparent)}
        .tr-roadmapV2DoneCheck{width:20px;height:20px;font-size:11px;color:#a0f0c4;border:1px solid rgba(119,226,171,.45);background:radial-gradient(circle at 35% 28%,rgba(229,255,241,.17),rgba(28,104,68,.16) 52%,rgba(3,15,9,.92));box-shadow:inset 0 1px 0 rgba(255,255,255,.10),0 0 10px rgba(75,210,140,.13),0 5px 10px rgba(0,0,0,.28)}

        @media (max-width:980px) and (min-width:721px){
          .tr-roadmapRailPremium{height:104px}.tr-roadmapV2Current{grid-template-columns:minmax(165px,45%) minmax(0,1fr);min-height:230px}.tr-roadmapV2CurrentVisual{height:210px}.tr-roadmapV2CurrentVisual img{max-width:204px;max-height:204px}.tr-roadmapV2AgendaRow{grid-template-columns:58px minmax(0,1fr) 17px}.tr-roadmapV2AgendaRow.is-done{grid-template-columns:22px 58px minmax(0,1fr) 17px}.tr-roadmapV2AgendaVisual{width:56px;height:51px}.tr-roadmapV2AgendaVisual img{max-width:55px;max-height:50px}
        }
        @media (max-width:720px){
          .tr-roadmapV2{padding:11px 10px 11px;border-radius:18px;background:radial-gradient(520px 210px at 22% 26%,color-mix(in srgb,var(--tr-rm-active,#28d9ff) 8%,transparent),transparent 68%),linear-gradient(160deg,#101a21,#061016 58%,#02070b)}
          .tr-roadmapV2Header{gap:7px;padding:0}.tr-roadmapV2Title>span{font-size:7px}.tr-roadmapV2TitleLine{gap:6px}.tr-roadmapV2TitleLine>strong{font-size:13px}.tr-roadmapV2Edit{height:24px;min-width:49px;padding:0 5px;border-radius:7px}.tr-roadmapV2Edit svg{width:10px;height:10px}.tr-roadmapV2Edit span{font-size:6.4px}.tr-roadmapV2Count{min-width:58px}.tr-roadmapV2Count>strong{font-size:24px}.tr-roadmapV2Count>strong>span{font-size:14px;margin-left:1px}.tr-roadmapV2Count>small{font-size:7px;margin-top:3px}
          .tr-roadmapRailPremium{height:80px;margin:1px -7px 1px}.tr-roadmapRail3DHitGrid{left:5.5%;right:5.5%;top:12%;bottom:12%}.tr-roadmapRail3DHit:focus-visible{outline-offset:-11px}
          .tr-roadmapV2Body{grid-template-columns:1fr;gap:0}.tr-roadmapV2Body::before{display:none}
          .tr-roadmapV2Current{min-height:126px;grid-template-columns:116px minmax(0,1fr);gap:7px;padding:4px 5px 6px 0;border-bottom:1px solid rgba(169,203,214,.11)}.tr-roadmapV2Current::before{left:-9%;top:4%;width:58%;height:94%;filter:blur(7px)}.tr-roadmapV2Current::after{left:5%;width:31%;bottom:8px;height:10px}.tr-roadmapV2CurrentVisual{height:119px}.tr-roadmapV2CurrentVisual::before{width:82%;height:13px;bottom:5px}.tr-roadmapV2CurrentVisual img{max-width:116px;max-height:116px}.tr-roadmapV2CurrentCopy{gap:3px}.tr-roadmapV2CurrentIndex{font-size:20px}.tr-roadmapV2CurrentIndex small{font-size:9px}.tr-roadmapV2CurrentCopy>strong{font-size:clamp(23px,6.6vw,29px);line-height:.98}.tr-roadmapV2MuscleLabel{font-size:7.2px}.tr-roadmapV2CurrentState{font-size:6.6px}
          .tr-roadmapV2Agenda{gap:5px;padding:7px 0 1px}.tr-roadmapV2Group{gap:4px}.tr-roadmapV2GroupHead{min-height:24px;padding-left:6px}.tr-roadmapV2GroupHead>span{font-size:9.2px}.tr-roadmapV2GroupHead>small{font-size:7.4px}
          .tr-roadmapV2AgendaRow{min-height:53px;grid-template-columns:50px minmax(0,1fr) 14px;gap:8px;padding:3px 6px 3px 5px;border-radius:11px}.tr-roadmapV2AgendaRow.is-next{min-height:58px}.tr-roadmapV2AgendaRow.is-done{grid-template-columns:18px 50px minmax(0,1fr) 14px;gap:6px}.tr-roadmapV2AgendaVisual{width:48px;height:46px}.tr-roadmapV2AgendaVisual img{max-width:47px;max-height:45px}.tr-roadmapV2AgendaCopy>strong{font-size:14.4px}.tr-roadmapV2AgendaCopy>small{font-size:7.3px}.tr-roadmapV2AgendaArrow{font-size:18px}.tr-roadmapV2DoneCheck{width:17px;height:17px;font-size:9px}
        }
        @media (max-width:390px){
          .tr-roadmapV2{padding-left:8px;padding-right:8px}.tr-roadmapV2TitleLine>strong{font-size:12px}.tr-roadmapV2Edit{min-width:45px}.tr-roadmapV2Count{min-width:53px}.tr-roadmapV2Count>strong{font-size:22px}.tr-roadmapV2Count>strong>span{font-size:13px}.tr-roadmapRailPremium{height:74px;margin-left:-7px;margin-right:-7px}.tr-roadmapV2Current{grid-template-columns:104px minmax(0,1fr);min-height:116px}.tr-roadmapV2CurrentVisual{height:108px}.tr-roadmapV2CurrentVisual img{max-width:105px;max-height:105px}.tr-roadmapV2CurrentCopy>strong{font-size:21px}.tr-roadmapV2AgendaRow{grid-template-columns:46px minmax(0,1fr) 13px;gap:7px}.tr-roadmapV2AgendaRow.is-done{grid-template-columns:17px 46px minmax(0,1fr) 13px}.tr-roadmapV2AgendaVisual{width:44px;height:43px}.tr-roadmapV2AgendaVisual img{max-width:43px;max-height:42px}.tr-roadmapV2AgendaCopy>strong{font-size:13.4px}.tr-roadmapV2AgendaCopy>small{font-size:6.9px}
        }
        @media (max-width:345px){
          .tr-roadmapV2TitleLine>strong{font-size:11px}.tr-roadmapV2Edit{min-width:42px}.tr-roadmapV2Count{min-width:50px}.tr-roadmapV2Count>strong{font-size:21px}.tr-roadmapV2Count>small{font-size:6.5px}.tr-roadmapRailPremium{height:70px}.tr-roadmapV2Current{grid-template-columns:96px minmax(0,1fr);min-height:109px}.tr-roadmapV2CurrentVisual{height:101px}.tr-roadmapV2CurrentVisual img{max-width:98px;max-height:98px}.tr-roadmapV2CurrentCopy>strong{font-size:19px}.tr-roadmapV2MuscleLabel{font-size:6.4px}.tr-roadmapV2CurrentState{font-size:6px}.tr-roadmapV2AgendaRow{grid-template-columns:42px minmax(0,1fr) 12px;gap:6px}.tr-roadmapV2AgendaRow.is-done{grid-template-columns:16px 42px minmax(0,1fr) 12px}.tr-roadmapV2AgendaVisual{width:40px;height:40px}.tr-roadmapV2AgendaVisual img{max-width:39px;max-height:39px}.tr-roadmapV2AgendaCopy>strong{font-size:12.4px}.tr-roadmapV2AgendaCopy>small{font-size:6.4px}
        }

        @media (prefers-reduced-motion:reduce){
          .tr-siToggle,.tr-siWeekFill{transition:none!important}
          .tr-roadmapV2::before,.tr-roadmapV2CurrentFx,.tr-roadmapV2Node.is-current .tr-roadmapV2NodeFace,.tr-roadmapV2CurrentFx::before{animation:none!important}
        }
      `}</style>
    </div>
  );
