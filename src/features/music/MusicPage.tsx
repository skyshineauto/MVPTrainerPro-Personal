...
type SmartIntensity = "recovery" | "balanced" | "high";

const PLAYLISTS_CHANGED_EVENT =
  "mvp:music-playlists-changed";
...
function trackDuration(track: MusicTrack) {
  return Math.max(
    120,
    Number(track.duration_seconds || 210)
  );
}

function smartMixScore(
  track: MusicTrack,
  ...
  if (track.energy_level === targetEnergy) {
    score += 28;
  } else if (
    intensity === "balanced" &&
    ...
    score += 13;
  } else if (track.energy_level === "medium") {
    score += 9;
  }

  score += Math.max(
    0,
    8 - Number(track.play_count || 0) * 0.35
  );

  score -= Math.min(
    28,
    Number(track.skip_count || 0) * 4.5
  );
...
function buildSmartMix(
  ...
  const targetSeconds =
    Math.max(15, minutes) * 60;

  const candidates = tracks
    .map((track) => ({
      track,
      score: smartMixScore(track, intensity),
    }))
...
export function MusicPage({
  ...
  const orderSaveTimerRef =
    useRef<number | null>(null);
  const latestOrderRef =
    useRef<MusicTrack[]>([]);

  const player = useMusicPlayer();

  const [tab, setTab] =
    useState<MusicTab>("songs");
  ...
  const [smartMinutes, setSmartMinutes] =
    useState(60);
  const [
    smartIntensity,
    setSmartIntensity,
  ] = useState<SmartIntensity>("high");
  ...
  const totalSize = useMemo(
    () =>
      tracks.reduce(
        (sum, track) =>
          sum +
          Number(track.file_size_bytes || 0),
        0
      ),
    [tracks]
  );
  ...
  async function refreshPlaylists(
    ...
    window.dispatchEvent(
      new Event(PLAYLISTS_CHANGED_EVENT)
    );
  ...
  async function uploadFiles(
    ...
    try {
      ...
      setError(
        caught instanceof Error
          ? caught.message
          : "Could not upload the selected songs."
      );
    } finally {
      setUploading(false);
  ...
  async function deleteTrack(
    ...
    const confirmed = window.confirm(
      `Delete “${track.title}” from MVP Trainer?`
    );
  ...
  function scheduleTrackOrderSave(
    next: MusicTrack[]
  ) {
    latestOrderRef.current = next;
  ...
  async function addTrackToSelected(
    trackId: string
  ) {
    if (!selectedPlaylist) return;
  ...
  async function removeTrackFromSelected(
    trackId: string
  ) {
    if (!selectedPlaylist) return;
    ...
    try {
      await removeMusicPlaylistTrack(
        selectedPlaylist.id,
        trackId
      );
  ...
  async function movePlaylistTrack(
    index: number,
    direction: -1 | 1
  ) {
    if (!selectedPlaylist) return;
    ...
    if (
      target < 0 ||
      target >= selectedTrackIds.length
    ) {
      return;
    }
  ...
  async function buildAndPlaySmartMix() {
    if (!tracks.length) {
      setError(
        "Upload songs before building a Smart Mix."
      );
      return;
    }
    ...
    setError("");
    setMessage(
      "Building your Smart Mix…"
    );

    try {
      const mixTracks = buildSmartMix(
        tracks,
        smartMinutes,
        smartIntensity
      );
      ...
      setMessage(
        `Smart Mix ready: ${
          mixTracks.length
        } songs, about ${formatLongDuration(
          mixSeconds
        )}.`
      );
    } catch (caught: unknown) {
  ...
  return (
    <div className="tr-musicHubPage">
      <section className="tr-musicHubHero">
        <div>
          <span className="tr-musicHubEyebrow">
            PRIVATE WORKOUT AUDIO
          </span>

          <h1>
            Your music. Engineered for
            training.
          </h1>

          <p>
            Upload your own songs, build
            playlists, and let Smart Mix
            assemble a fresh queue from your
            favorites, energy tags, skips, and
            recent plays.
          </p>
        </div>
      ...
      <section
        className="tr-musicHubStats"
        aria-label="Music library summary"
      >
        <div>
      ...
      {message ? (
        <div className="tr-musicHubNotice tr-musicHubNotice--ok">
          {message}
        </div>
      ) : null}

      {error ? (
        <div className="tr-musicHubNotice tr-musicHubNotice--err">
          {error}
        </div>
      ) : null}
          ...
          {loading ? (
            ...
            <div className="tr-musicHubEmpty">
              {tracks.length
                ...
                : "Upload your first MP3, M4A, or WAV file."}
            </div>
          ) : (
              ...
              {playlists.map(
                (playlist) => {
                  ...
                  return (
                    ...
                    >
                      <span className="tr-playlistTileIcon">
                        ♫
                      </span>
                  ...
                  <div className="tr-playlistAvailableGrid">
                    {availableTracks.map(
                      (track) => (
                        <article
                          key={track.id}
                          className="tr-playlistAvailableCard"
                        >
                          <span>
                            <strong>
                              {track.title}
      ...
      {tab === "smart" ? (
        <section className="tr-smartMixPanel">
          <div className="tr-smartMixCopy">
            ...
            <h2>
              A fresh workout playlist in one
              press.
            </h2>

            <p>
              Smart Mix scores your uploaded
              songs using favorites, energy,
              play history, skips, recency,
              and artist variety. It updates
              one reusable Smart Mix playlist
          ...
          <div className="tr-smartMixControls">
            ...
            <button
              ...
              onClick={() =>
                void buildAndPlaySmartMix()
              }
            >
              {buildingSmart
                ? "BUILDING MIX…"
                : "✦ BUILD, SAVE & PLAY SMART MIX"}
            </button>

            <small>
              {tracks.length} uploaded songs
              available • {favoriteCount}{" "}
              favorites tagged
            </small>
          </div>
        </section>
