...
type EnrichmentState = { running: boolean; current: number; total: number; matched: number; review: number; notFound: number; label: string };
...
export function MusicPage({ navigate }: { navigate?: (to: string) => void }) {
  ...
  const [enrichment, setEnrichment] = useState<EnrichmentState>({ running: false, current: 0, total: 0, matched: 0, review: 0, notFound: 0, label: "" });
  ...
  async function enrichTracks(targets: MusicTrack[], artworkOnly = false) {
    ...
    if (!work.length) { setMessage(artworkOnly ? "Selected songs already have artwork. Existing artwork is protected." : "No songs were selected for identification."); return; }
    setError(""); setReviewItems([]); setReviewSavedIds(new Set()); setReviewSkippedIds(new Set());
    let matched = 0, review = 0, notFound = 0;
    setEnrichment({ running: true, current: 0, total: work.length, matched: 0, review: 0, notFound: 0, label: artworkOnly ? "FINDING ARTWORK" : "ANALYZING LIBRARY" });
    const reviewQueue: ReviewItem[] = [];
    try {
      for (let index = 0; index < work.length; index += 1) {
        const track = work[index];
        setEnrichment({ running: true, current: index + 1, total: work.length, matched, review, notFound, label: `${artworkOnly ? "FINDING ART" : "ANALYZING"} • ${track.title}` });
        const result = await enrichMusicTrack(track, { artworkOnly, autoApplyThreshold: 0.98 });
        if (result.status === "matched") matched += 1;
        else if (result.status === "review") { review += 1; if (result.candidates.length) reviewQueue.push({ trackId: track.id, candidates: result.candidates }); }
        else if (result.status === "not_found") notFound += 1;
        await delayMusicLookup();
      }
      await refreshTracks(); setReviewItems(reviewQueue);
      setEnrichment({ running: false, current: work.length, total: work.length, matched, review, notFound, label: artworkOnly ? "ARTWORK SCAN COMPLETE" : "LIBRARY ENRICHMENT COMPLETE" });
      setMessage(`${matched} matched${review ? ` • ${review} need review` : ""}${notFound ? ` • ${notFound} not found` : ""}. Existing artwork was not replaced.`);
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Could not finish library enrichment."); setEnrichment((current) => ({ ...current, running: false })); }
  }
    ...
    <main className="tr10-page">
      ...
      <section className="tr10-console">
        ...
        <div className="tr10-healthRail">
          ...
          <button className={`${healthFilter === "needs_info" ? "is-active " : ""}is-needs`} onClick={() => setHealthFilter("needs_info")}>NEEDS INFO <b>{needsInfoCount}</b></button>
        ...
        {enrichment.total ? <div className={`tr10-enrichment ${enrichment.running ? "is-running" : ""}`}><div><strong>{enrichment.label}</strong><span>{enrichment.current}/{enrichment.total} • {enrichment.matched} matched • {enrichment.review} review • {enrichment.notFound} not found</span></div><i style={{ transform: `scaleX(${enrichment.total ? enrichment.current / enrichment.total : 0})` }} /></div> : null}
          ...
          <div className="tr10-table">
            ...
            {pagedTracks.map((track) => {
              ...
              return <article className={`tr10-row ${current ? "is-current" : ""}`} key={track.id}>
                ...
                <button className={`tr10-energy is-${track.energy_level}`} onClick={() => void setEnergy(track, track.energy_level === "low" ? "medium" : track.energy_level === "medium" ? "high" : "low")} title="Click to change energy"><i /><span>{track.energy_level.toUpperCase()}</span><b>{track.energy_level === "high" ? "▮▮▮" : track.energy_level === "medium" ? "▮▮" : "▮"}</b></button>
      ...
      <style>{`
        ...
        .tr10-healthRail{display:grid;grid-template-columns:repeat(5,1fr);border-bottom:1px solid rgba(82,151,176,.1);background:#050d12}.tr10-healthRail button{height:40px;border:0;border-right:1px solid rgba(82,151,176,.07);background:transparent;color:#718995;font-size:7px;font-weight:1000;letter-spacing:.08em}.tr10-healthRail button b{margin-left:5px;color:#b4cbd4}.tr10-healthRail button.is-active{color:#dff8ff;background:rgba(12,55,72,.58);box-shadow:inset 0 -2px #52d3f7}.tr10-healthRail button.is-needs{color:#e99da1}.tr10-healthRail button.is-needs b{color:#ff858b}.tr10-healthRail button.is-needs.is-active{color:#ffe4e6;background:linear-gradient(180deg,rgba(105,22,28,.66),rgba(49,10,14,.76));box-shadow:inset 0 -2px #ff5862}
        ...
        .tr10-enrichment{position:relative;padding:9px 12px;border-bottom:1px solid rgba(80,155,182,.1);background:rgba(8,30,40,.72);overflow:hidden}.tr10-enrichment>div{display:flex;justify-content:space-between;gap:12px;color:#8bb1be;font-size:7px}.tr10-enrichment strong{color:#d5edf5;font-size:8px}.tr10-enrichment>i{position:absolute;left:0;right:0;bottom:0;height:2px;transform-origin:left;background:#49d3f8;transition:transform .25s ease}.tr10-message,.tr10-error{margin:9px 11px 0;padding:9px 11px;border-radius:8px;font-size:8px;font-weight:850}.tr10-message{border:1px solid rgba(74,208,151,.18);background:rgba(15,65,47,.2);color:#83dcb0}.tr10-error{border:1px solid rgba(255,82,92,.28);background:rgba(90,19,24,.25);color:#ffadb2}
        ...
        .tr10-energy{height:34px;padding:0 10px;display:grid;grid-template-columns:7px 1fr auto;gap:7px;align-items:center;border-radius:8px;font-size:7px;font-weight:1000;letter-spacing:.09em;transition:transform .12s ease}.tr10-energy:hover{transform:translateY(-1px)}.tr10-energy i{width:7px;height:7px;border-radius:2px;background:currentColor;box-shadow:0 0 8px currentColor}.tr10-energy span{text-align:left}.tr10-energy b{font-size:6px;letter-spacing:1px;opacity:.75}.tr10-energy.is-high{border:1px solid rgba(242,174,65,.46);color:#f1b45e;background:linear-gradient(180deg,rgba(72,46,14,.58),rgba(17,13,8,.92))}.tr10-energy.is-medium{border:1px solid rgba(70,200,235,.4);color:#76dff5;background:linear-gradient(180deg,rgba(9,55,70,.52),rgba(5,16,22,.92))}.tr10-energy.is-low{border:1px solid rgba(68,208,155,.38);color:#77dfae;background:linear-gradient(180deg,rgba(11,61,43,.5),rgba(5,18,14,.92))}
