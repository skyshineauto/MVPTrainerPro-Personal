...
import { MusicTabVisualRail } from "./premium/MusicTabVisualRail";
import {
  EditActionIcon,
  LikeActionIcon,
  PlaylistActionIcon,
...
function DiscoveryCard({
  ...
  return <article className={item.inLibrary ? "is-owned" : ""}>
    ...
    <footer>
      {item.previewUrl
        ? <button className={`tr10-previewButton ${previewing ? "is-playing" : ""}`} onClick={() => onPreview(item)}>{previewing ? <><InlinePlayIcon playing /><span>STOP PREVIEW</span></> : previewError ? <><InlinePlayIcon /><span>RETRY PREVIEW</span></> : <><InlinePlayIcon /><span>PREVIEW</span></>}</button>
  ...
  return <section className="tr10-collectionDetail">
    ...
    <header className="tr10-collectionDetailHero">
      ...
      <div className="tr10-collectionDetailActions">
        <button type="button" className="is-primary" disabled={!tracks.length} onClick={onPlayAll}><InlinePlayIcon /><span>PLAY ALL</span></button>
    ...
    <div className="tr10-collectionSongList">
      {tracks.map((track, index) => {
        ...
        return <article className={`tr10-collectionSong ${current ? "is-current" : ""}`} key={track.id}>
          ...
          <button type="button" className={`tr10-collectionSongPlay ${current && playing ? "is-playing" : ""}`} onClick={() => onPlayTrack(track)} aria-label={`${current && playing ? "Pause" : "Play"} ${track.title}`}><InlinePlayIcon playing={Boolean(current && playing)} /></button>
...
export function MusicPage({ navigate }: { navigate?: (to: string) => void }) {
  ...
  const [hoveredTab, setHoveredTab] = useState<MusicTab | null>(null);
    ...
    <main className={`tr10-page tr10-premiumLibrary tr10-premium-${tab}`}><MusicLibraryVisualEngine activeTab={tab} playing={Boolean(player.playing)} />
      ...
      <section className="tr10-console">
        ...
        <nav ref={tabNavRef} className="tr10-tabs" aria-label="Music library sections">
          <MusicTabVisualRail activeTab={tab} hoveredTab={hoveredTab} />
          {([ ["songs","SONGS"], ["artists","ARTISTS"], ["albums","ALBUMS"], ["playlists","PLAYLISTS"], ["smart","SMART MIX"], ["intelligence","INTELLIGENCE"], ["discover","DISCOVER"], ["audition","AUDITION"] ] as Array<[MusicTab,string]>).map(([value,label]) => (
            <motion.button
              ...
              onPointerEnter={() => setHoveredTab(value)}
              onPointerLeave={() => setHoveredTab((current) => current === value ? null : current)}
              onFocus={() => setHoveredTab(value)}
              onBlur={() => setHoveredTab((current) => current === value ? null : current)}
            ...
            {pagedTracks.map((track) => {
              ...
              return <article className={`tr10-row ${current ? "is-current" : ""}`} key={track.id}>
                <div className="tr10-orderCell" aria-label="Reorder song">
                  <button type="button" aria-label={`Move ${track.title} up`} disabled={reorderIndex <= 0} onClick={() => moveTrack(track.id,-1)}><ReorderPremiumIcon direction="up" /></button>
                  <button type="button" aria-label={`Move ${track.title} down`} disabled={reorderIndex < 0 || reorderIndex >= libraryOrderedTracks.length - 1} onClick={() => moveTrack(track.id,1)}><ReorderPremiumIcon direction="down" /></button>
                </div>
                ...
                <div className="tr10-trackCell">
                  <button className={`tr10-play ${current && player.playing ? "is-playing" : ""}`} onClick={() => void toggleTrackPlayback(track)}><TrackPlayIcon playing={Boolean(current && player.playing)} /></button>
                  <TrackArtwork track={track} />
                ...
                <div className="tr10-actions">
                  <button type="button" className={`tr10-likeAction ${track.favorite ? "is-liked" : ""}`} onClick={() => void changePreference(track, track.favorite ? "neutral" : "like")} title={track.favorite ? "Liked" : "Like"}><i className="tr10-actionGlyph" aria-hidden="true"><LikeActionIcon /></i><span>{track.favorite ? "LIKED" : "LIKE"}</span></button>
                  <button type="button" className={`tr10-lessAction ${track.play_less ? "is-down" : ""}`} onClick={() => void changePreference(track, track.play_less ? "neutral" : "play_less")} title="Play less"><i className="tr10-actionGlyph" aria-hidden="true"><PlayLessActionIcon /></i><span>PLAY LESS</span></button>
                  <button type="button" className="tr10-nextAction" onClick={() => playMusicNext(track.id)}><i className="tr10-actionGlyph" aria-hidden="true"><PlayNextActionIcon /></i><span>PLAY NEXT</span></button><button type="button" className="tr10-queueAction" onClick={() => addMusicToQueue(track.id)}><i className="tr10-actionGlyph" aria-hidden="true"><QueueActionIcon /></i><span>QUEUE</span></button><button type="button" className="tr10-playlistAction" onClick={() => openPlaylistModal([track.id])}><i className="tr10-actionGlyph" aria-hidden="true"><PlaylistActionIcon /></i><span>PLAYLIST</span></button><button type="button" className="is-edit tr10-editAction" onClick={() => openDetail(track)}><i className="tr10-actionGlyph" aria-hidden="true"><EditActionIcon /></i><span>EDIT</span></button>
                </div>
              </article>;
            })}
          </div>
        ...
        /> : <div className="tr10-cardGrid">{artistGroups.map(([artist,songs]) => <article className="tr10-collectionCard" key={artist}>
          ...
          <button type="button" className="tr10-collectionPlay" onClick={() => void playMusicAdHocQueue(artist,songs)} aria-label={`Play ${artist}`}><InlinePlayIcon /><span>PLAY</span></button>
        </article>)}</div> : null}
        ...
        /> : <div className="tr10-cardGrid">{albumGroups.map((group) => <article className="tr10-collectionCard" key={`${group.artist}-${group.album}`}>
          ...
          <button type="button" className="tr10-collectionPlay" onClick={() => void playMusicAdHocQueue(`Album • ${group.album}`,group.tracks)} aria-label={`Play ${group.album}`}><InlinePlayIcon /><span>PLAY</span></button>
        </article>)}</div> : null}
          ...
          <section className="tr10-radarPanel" aria-label="Discovery Radar">
            ...
            <div className="tr10-radarGrid">
              {discoveryRadar.map((lane) => <article key={lane.id}>
                ...
                <button type="button" disabled={!lane.tracks.length} onClick={() => void playMusicAdHocQueue(`Radar • ${lane.title}`, lane.tracks)}><InlinePlayIcon /><span>PLAY</span></button>
              </article>)}
            </div>
          </section>
      ...
      {detailTrack && typeof document !== "undefined" ? createPortal(<div className="tr10-modalBack tr10-detailPortal" onMouseDown={closeDetail}><section className="tr10-inspector" role="dialog" aria-modal="true" onMouseDown={(event: MouseEvent<HTMLElement>) => event.stopPropagation()}>
        ...
        {detailMode === "edit" ? <>
          <div className="tr10-inspectorScroll">
            <div className="tr10-inspectCommands"><button disabled={detailSaveState === "searching"} onClick={() => void findDetailMatches(detailTrack,"info_results")}>{detailSaveState === "searching" ? "SEARCHING…" : "FIND SONG INFO"}</button><button disabled={detailSaveState === "searching"} onClick={() => void findDetailMatches(detailTrack,"artwork_results")}>FIND ARTWORK</button><button onClick={() => playMusicNext(detailTrack.id)}>PLAY NEXT</button><button onClick={() => addMusicToQueue(detailTrack.id)}>ADD TO QUEUE</button><button className={detailTrack.favorite ? "is-liked" : ""} onClick={() => void changePreference(detailTrack, detailTrack.favorite ? "neutral" : "like")}><LikeActionIcon /><span>{detailTrack.favorite ? "LIKED" : "LIKE"}</span></button><button className={detailTrack.play_less ? "is-down" : ""} onClick={() => void changePreference(detailTrack, detailTrack.play_less ? "neutral" : "play_less")}><PlayLessActionIcon /><span>PLAY LESS</span></button></div>
      ...
      <style>{`
        ...
        /* MVP_TRAINER_V5_R12_5E_25_FLAGSHIP_MUSIC_FOUNDATION
           Final authority: one material system, one alignment system, one interaction language. */
        .tr10-premiumLibrary{
          --m25-blue:#0b7cff;--m25-blue-hi:#22a5ed;--m25-orange:#eb8b0f;--m25-orange-hi:#ffad35;
          --m25-ink:#020609;--m25-surface:#050b10;--m25-raised:#09131b;--m25-raised2:#0c1821;
          --m25-white:#f7fbff;--m25-muted:#91a4ad;--m25-dim:#617781;
          --m25-line:rgba(132,174,194,.11);--m25-line-hi:rgba(89,174,226,.22);
          --m25-contact:0 8px 18px rgba(0,0,0,.28);--m25-float:0 18px 42px rgba(0,0,0,.32);
        }
        .tr10-premiumLibrary,.tr10-premiumLibrary .tr10-console{background:#02070b!important}
        .tr10-premiumLibrary .tr10-console{border:0!important;border-radius:16px!important;box-shadow:0 0 0 1px rgba(91,145,169,.09),0 24px 60px rgba(0,0,0,.20)!important;overflow:visible!important}
        .tr10-premiumLibrary .tr10-console:before{display:none!important}
        .tr10-premiumLibrary .tr10-sectionHead{padding:18px 16px 14px!important;border-bottom:1px solid rgba(130,171,190,.08)!important;background:transparent!important}
        .tr10-premiumLibrary .tr10-sectionHead h2{font-size:29px!important;line-height:1!important;letter-spacing:-.045em!important}
        .tr10-premiumLibrary .tr10-directoryEyebrow{font-size:10px!important;color:#78b7ff!important}

        /* Primary controls: graphite remains graphite. Hover adds light, never darkness or washout. */
        .tr10-premiumLibrary button:not(:disabled){-webkit-tap-highlight-color:transparent}
        .tr10-premiumLibrary .tr10-headActions button,
        .tr10-premiumLibrary .tr10-bulk button,
        .tr10-premiumLibrary .tr10-pager button{
          background:#08121a!important;border:1px solid rgba(127,170,190,.14)!important;color:#e4edf1!important;
          box-shadow:inset 0 1px 0 rgba(255,255,255,.055),0 6px 14px rgba(0,0,0,.22)!important;
          filter:none!important;transition:transform .16s ease,background-color .16s ease,border-color .16s ease,box-shadow .16s ease,color .16s ease!important
        }
        @media(hover:hover){
          .tr10-premiumLibrary .tr10-headActions button:not(:disabled):hover,
          .tr10-premiumLibrary .tr10-bulk button:not(:disabled):hover,
          .tr10-premiumLibrary .tr10-pager button:not(:disabled):hover{
            background:#0c1a24!important;border-color:rgba(34,165,237,.34)!important;color:#fff!important;filter:none!important;transform:translateY(-1px)!important;
            box-shadow:inset 0 1px 0 rgba(255,255,255,.10),0 12px 24px rgba(0,0,0,.28),0 0 0 1px rgba(11,124,255,.04)!important
          }
        }
        .tr10-premiumLibrary .tr10-headActions .is-orange{background:#d97d09!important;border-color:#f39b20!important;color:#071016!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.30),0 10px 22px rgba(0,0,0,.25)!important}
        @media(hover:hover){.tr10-premiumLibrary .tr10-headActions .is-orange:not(:disabled):hover{background:#ee9018!important;border-color:#ffb143!important;color:#050b0f!important;box-shadow:inset 0 1px rgba(255,255,255,.38),0 13px 28px rgba(0,0,0,.30),0 0 18px rgba(235,139,15,.08)!important}}

        /* Status is a clean instrument strip, not five cards. */
        .tr10-premiumLibrary .tr10-statusPanel{padding:0 12px!important;background:transparent!important;border:0!important;border-bottom:1px solid rgba(128,169,189,.08)!important}
        .tr10-premiumLibrary .tr10-healthRail{gap:18px!important;display:flex!important;align-items:center!important;overflow-x:auto!important;scrollbar-width:none!important}
        .tr10-premiumLibrary .tr10-healthRail::-webkit-scrollbar{display:none}
        .tr10-premiumLibrary .tr10-healthRail button{flex:1 1 0!important;min-width:120px!important;min-height:44px!important;border:0!important;border-radius:0!important;background:transparent!important;box-shadow:none!important;color:#8498a1!important;padding:0 6px!important}
        .tr10-premiumLibrary .tr10-healthRail button:after{left:12%!important;right:12%!important;bottom:0!important;height:2px!important;background:var(--m25-blue)!important;opacity:0!important;box-shadow:0 0 10px rgba(11,124,255,.25)!important}
        .tr10-premiumLibrary .tr10-healthRail button.is-active{color:#f4faff!important;background:transparent!important;box-shadow:none!important}
        .tr10-premiumLibrary .tr10-healthRail button.is-active:after{opacity:1!important}
        @media(hover:hover){.tr10-premiumLibrary .tr10-healthRail button:hover{color:#eaf5fa!important;background:rgba(11,124,255,.025)!important;filter:none!important}}

        /* One precision nav rail. No stacked blue/orange lines, no fake glossy pills. */
        .tr10-premiumLibrary .tr10-tabs{position:relative!important;display:grid!important;grid-template-columns:repeat(8,minmax(0,1fr))!important;gap:4px!important;padding:5px!important;min-height:56px!important;background:#040a0f!important;border:1px solid rgba(124,164,183,.08)!important;border-radius:12px!important;box-shadow:inset 0 1px rgba(255,255,255,.025),inset 0 -1px rgba(0,0,0,.65)!important;overflow:visible!important}
        .tr10-premiumLibrary .mvpTabVisualRail{display:none!important}
        .tr10-premiumLibrary .tr10-tabs button{position:relative!important;min-width:0!important;height:44px!important;border:0!important;border-radius:8px!important;background:#061018!important;color:#8fa0a8!important;box-shadow:inset 0 1px rgba(255,255,255,.035),0 3px 7px rgba(0,0,0,.16)!important;overflow:hidden!important;transform:translateZ(0);transition:transform .18s cubic-bezier(.2,.8,.2,1),background-color .18s ease,color .18s ease,box-shadow .18s ease!important}
        .tr10-premiumLibrary .tr10-tabs button:before{content:""!important;position:absolute!important;inset:0!important;background:linear-gradient(118deg,transparent 0 38%,rgba(255,255,255,.055) 49%,transparent 60%)!important;transform:translateX(-125%)!important;opacity:0!important;transition:transform .42s ease,opacity .2s ease!important;pointer-events:none!important}
        .tr10-premiumLibrary .tr10-tabs button:after{content:""!important;position:absolute!important;left:16%!important;right:16%!important;bottom:3px!important;height:2px!important;border-radius:999px!important;background:var(--m25-blue)!important;opacity:0!important;filter:none!important;box-shadow:none!important}
        .tr10-premiumLibrary .tr10-tabs button .tr10-tabEnergy{display:none!important}
        .tr10-premiumLibrary .tr10-tabs button>span:last-child{position:relative!important;z-index:2!important;font-size:10px!important;font-weight:900!important;letter-spacing:.10em!important}
        @media(hover:hover){.tr10-premiumLibrary .tr10-tabs button:not(.is-active):hover{background:#0a1620!important;color:#f4fbff!important;transform:translateY(-2px)!important;box-shadow:inset 0 1px rgba(255,255,255,.075),0 10px 20px rgba(0,0,0,.26),0 0 0 1px rgba(34,165,237,.11)!important;filter:none!important}.tr10-premiumLibrary .tr10-tabs button:not(.is-active):hover:before{opacity:1!important;transform:translateX(125%)!important}}
        .tr10-premiumLibrary .tr10-tabs button.is-active{background:#0a1722!important;color:#fff!important;transform:translateY(-1px)!important;box-shadow:inset 0 1px rgba(255,255,255,.085),0 10px 22px rgba(0,0,0,.28),0 0 0 1px rgba(11,124,255,.26)!important}
        .tr10-premiumLibrary .tr10-tabs button.is-active:after{opacity:1!important;box-shadow:0 0 11px rgba(11,124,255,.34)!important}
        .tr10-premiumLibrary .tr10-tabs button.is-active:before{content:""!important;inset:0!important;height:auto!important;width:auto!important;right:auto!important;top:auto!important;background:radial-gradient(ellipse at 85% 8%,rgba(235,139,15,.11),transparent 26%),linear-gradient(180deg,rgba(34,165,237,.035),transparent 55%)!important;transform:none!important;opacity:1!important}

        /* Command bar, no box-inside-box feel. */
        .tr10-premiumLibrary .tr10-toolbar{display:grid!important;grid-template-columns:minmax(300px,1.7fr) minmax(150px,.65fr) minmax(190px,.8fr) 100px!important;gap:8px!important;padding:12px 10px!important;background:#03090d!important;border:0!important;border-bottom:1px solid rgba(128,169,189,.07)!important}
        .tr10-premiumLibrary .tr10-toolbar label:first-child{grid-column:auto!important}
        .tr10-premiumLibrary .tr10-toolbar label{background:transparent!important;border:0!important;padding:0!important}
        .tr10-premiumLibrary .tr10-toolbar input,.tr10-premiumLibrary .mvpPremiumSelectTrigger{min-height:46px!important;border-radius:9px!important;background:#071119!important;border:1px solid rgba(126,168,189,.11)!important;color:#f0f7fa!important;box-shadow:inset 0 1px rgba(255,255,255,.035),0 5px 12px rgba(0,0,0,.16)!important}
        @media(hover:hover){.tr10-premiumLibrary .tr10-toolbar input:hover,.tr10-premiumLibrary .mvpPremiumSelectTrigger:hover{background:#0a1620!important;border-color:rgba(34,165,237,.26)!important;filter:none!important}}
        .tr10-premiumLibrary .mvpPremiumSelectMenu{background:#050b10!important;border-color:rgba(34,165,237,.22)!important;box-shadow:0 24px 60px rgba(0,0,0,.62),inset 0 1px rgba(255,255,255,.04)!important}
        .tr10-premiumLibrary .mvpPremiumSelectMenu>button:hover,.tr10-premiumLibrary .mvpPremiumSelectMenu>button:focus-visible{background:#0b1821!important;color:#fff!important;box-shadow:inset 2px 0 var(--m25-blue)!important}
        .tr10-premiumLibrary .mvpPremiumSelectMenu>button.is-selected{background:#0a1720!important;color:#fff!important;box-shadow:inset 2px 0 var(--m25-blue)!important}

        /* SONGS: header and rows share the exact same six-column contract. */
        .tr10-premiumLibrary .tr10-table{background:#02070b!important;border:0!important}
        .tr10-premiumLibrary .tr10-tableHead,
        .tr10-premiumLibrary .tr10-row{display:grid!important;grid-template-columns:42px 28px minmax(280px,1fr) 72px 104px minmax(420px,auto)!important;column-gap:10px!important;align-items:center!important}
        .tr10-premiumLibrary .tr10-tableHead{min-height:38px!important;padding:0 10px!important;background:#03090d!important;border:0!important;border-bottom:1px solid rgba(124,164,183,.08)!important;color:#71858f!important;font-size:9px!important;font-weight:900!important;letter-spacing:.10em!important}
        .tr10-premiumLibrary .tr10-tableHead .tr10-orderHead{text-align:center!important}
        .tr10-premiumLibrary .tr10-tableHead>span:nth-of-type(2){text-align:right!important;padding-right:4px!important}
        .tr10-premiumLibrary .tr10-row{min-height:66px!important;padding:7px 10px!important;background:transparent!important;border:0!important;border-bottom:1px solid rgba(127,166,185,.065)!important;border-radius:0!important;box-shadow:none!important;transition:background-color .15s ease,box-shadow .15s ease,transform .15s ease!important}
        @media(hover:hover){.tr10-premiumLibrary .tr10-row:hover{background:#071118!important;box-shadow:inset 2px 0 rgba(11,124,255,.45)!important;transform:none!important}.tr10-premiumLibrary .tr10-row:hover .tr10-trackText strong{color:#fff!important}}
        .tr10-premiumLibrary .tr10-row.is-current{background:#07131c!important;box-shadow:inset 2px 0 var(--m25-blue)!important}
        .tr10-premiumLibrary .tr10-orderCell{position:static!important;display:grid!important;grid-template-rows:18px 18px!important;gap:2px!important;width:34px!important;height:40px!important;align-content:center!important;justify-self:center!important;padding:1px!important;border:1px solid rgba(120,164,184,.08)!important;border-radius:8px!important;background:#050c11!important;box-shadow:inset 0 1px rgba(255,255,255,.025)!important}
        .tr10-premiumLibrary .tr10-orderCell button{display:grid!important;place-items:center!important;width:30px!important;height:18px!important;min-height:18px!important;padding:0!important;border:0!important;border-radius:5px!important;background:transparent!important;color:#8198a3!important;font-size:10px!important;line-height:1!important;box-shadow:none!important;text-shadow:none!important}
        @media(hover:hover){.tr10-premiumLibrary .tr10-orderCell button:not(:disabled):hover{background:#0a1821!important;color:#75c8ff!important;transform:none!important;filter:none!important}}
        .tr10-premiumLibrary .tr10-check{display:grid!important;place-items:center!important;margin:0!important}
        .tr10-premiumLibrary .tr10-trackCell{display:grid!important;grid-template-columns:38px 42px minmax(0,1fr) auto!important;gap:9px!important;align-items:center!important;min-width:0!important}
        .tr10-premiumLibrary .tr10-play{width:38px!important;height:38px!important;border-radius:50%!important;border:1px solid rgba(235,139,15,.34)!important;background:radial-gradient(circle at 35% 28%,#19222a 0 18%,#0a1117 58%,#05090d 100%)!important;color:#ffad35!important;font-size:0!important;box-shadow:inset 0 1px rgba(255,255,255,.08),0 6px 14px rgba(0,0,0,.28),0 0 0 1px rgba(235,139,15,.04)!important;position:relative!important;overflow:visible!important}
        .tr10-premiumLibrary .tr10-play:before{content:none!important;display:none!important}
        .tr10-premiumLibrary .tr10-play.is-playing:before{content:"Ⅱ"!important;padding-left:0!important;color:#59b7ff!important;font-size:14px!important}
        .tr10-premiumLibrary .tr10-play.is-playing{border-color:rgba(34,165,237,.46)!important;box-shadow:inset 0 1px rgba(255,255,255,.10),0 7px 16px rgba(0,0,0,.30),0 0 15px rgba(11,124,255,.12)!important}
        @media(hover:hover){.tr10-premiumLibrary .tr10-play:hover{background:radial-gradient(circle at 35% 28%,#202d36 0 18%,#0e1820 58%,#070d12 100%)!important;border-color:rgba(255,173,53,.55)!important;transform:translateY(-1px)!important;filter:none!important}}
        .tr10-premiumLibrary .tr10-art--row{width:42px!important;height:42px!important;border-radius:9px!important;border:0!important;box-shadow:0 8px 18px rgba(0,0,0,.34),0 0 0 1px rgba(255,255,255,.055)!important}
        .tr10-premiumLibrary .tr10-trackText{min-width:0!important;align-self:center!important}
        .tr10-premiumLibrary .tr10-trackText strong{display:-webkit-box!important;-webkit-line-clamp:2!important;-webkit-box-orient:vertical!important;overflow:hidden!important;white-space:normal!important;text-overflow:clip!important;font-size:12.5px!important;line-height:1.16!important;color:#f5f9fb!important;letter-spacing:-.012em!important}
        .tr10-premiumLibrary .tr10-trackText span{display:block!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important;font-size:9.5px!important;line-height:1.25!important;color:#90a3ac!important;margin-top:3px!important}
        .tr10-premiumLibrary .tr10-trackText small{display:none!important}
        .tr10-premiumLibrary .tr10-duration{justify-self:end!important;width:100%!important;text-align:right!important;padding-right:5px!important;font-size:10.5px!important;font-weight:700!important;color:#a7b9c1!important;font-variant-numeric:tabular-nums!important;font-feature-settings:"tnum" 1!important}
        .tr10-premiumLibrary .tr10-energy{justify-self:center!important;width:94px!important;height:34px!important;min-width:94px!important;display:grid!important;grid-template-columns:8px 1fr!important;gap:7px!important;align-items:center!important;padding:0 8px!important;border:0!important;border-radius:7px!important;background:transparent!important;box-shadow:none!important;text-shadow:none!important;color:#8ba1ab!important;overflow:visible!important}
        .tr10-premiumLibrary .tr10-energy:after{content:""!important;position:absolute!important;left:8px!important;right:8px!important;bottom:2px!important;top:auto!important;height:2px!important;border-radius:999px!important;background:currentColor!important;opacity:.34!important}
        .tr10-premiumLibrary .tr10-energyLed{width:6px!important;height:6px!important;box-shadow:0 0 7px currentColor!important}.tr10-premiumLibrary .tr10-energySegments{display:none!important}.tr10-premiumLibrary .tr10-energy>span{font-size:8.5px!important;letter-spacing:.08em!important;color:currentColor!important}
        .tr10-premiumLibrary .tr10-energy.is-high{color:#f0a436!important}.tr10-premiumLibrary .tr10-energy.is-medium{color:#55b7ee!important}.tr10-premiumLibrary .tr10-energy.is-low{color:#61c995!important}
        @media(hover:hover){.tr10-premiumLibrary .tr10-energy:hover{background:rgba(255,255,255,.025)!important;filter:none!important;transform:none!important}}
        .tr10-premiumLibrary .tr10-actions{justify-self:end!important;display:flex!important;align-items:center!important;gap:0!important;padding:3px!important;border:1px solid rgba(127,169,188,.09)!important;border-radius:10px!important;background:#050c11!important;box-shadow:inset 0 1px rgba(255,255,255,.025)!important;overflow:hidden!important;min-width:0!important}
        .tr10-premiumLibrary .tr10-actions button{position:relative!important;height:32px!important;min-height:32px!important;min-width:0!important;padding:0 8px!important;border:0!important;border-left:1px solid rgba(126,166,185,.07)!important;border-radius:6px!important;background:transparent!important;color:#879aa3!important;box-shadow:none!important;font-size:7.8px!important;letter-spacing:.025em!important;filter:none!important;transform:none!important}
        .tr10-premiumLibrary .tr10-actions button:first-child{border-left:0!important}
        .tr10-premiumLibrary .tr10-actionGlyph{display:inline-grid!important;place-items:center!important;margin-right:4px!important;background:transparent!important;border:0!important;color:#738a95!important;box-shadow:none!important}
        @media(hover:hover){.tr10-premiumLibrary .tr10-actions button:not(:disabled):hover{background:#0c1921!important;color:#fff!important;box-shadow:inset 0 1px rgba(255,255,255,.05)!important;filter:none!important;transform:none!important}.tr10-premiumLibrary .tr10-actions button:not(:disabled):hover .tr10-actionGlyph{color:#78bdff!important}}
        .tr10-premiumLibrary .tr10-actions .tr10-likeAction.is-liked{background:#08140f!important;color:#a9f2c8!important;box-shadow:inset 0 -2px rgba(70,213,143,.48)!important}.tr10-premiumLibrary .tr10-actions .tr10-likeAction.is-liked .tr10-actionGlyph{color:#62dca0!important;text-shadow:0 0 9px rgba(70,213,143,.34)!important}
        .tr10-premiumLibrary .tr10-actions .tr10-lessAction.is-down{background:#150b0d!important;color:#ffbac0!important;box-shadow:inset 0 -2px rgba(241,79,91,.44)!important}.tr10-premiumLibrary .tr10-actions .tr10-lessAction.is-down .tr10-actionGlyph{color:#ff7881!important;text-shadow:0 0 9px rgba(241,79,91,.30)!important}
        .tr10-premiumLibrary .tr10-actions .tr10-playlistAction .tr10-actionGlyph{color:#d9932b!important}.tr10-premiumLibrary .tr10-actions .tr10-editAction .tr10-actionGlyph{color:#66b8f4!important}

        /* Artists / Albums: artwork-led collection rails, restrained chrome. */
        .tr10-premiumLibrary .tr10-cardGrid{grid-template-columns:repeat(3,minmax(0,1fr))!important;gap:10px!important;padding:12px!important;background:transparent!important}
        .tr10-premiumLibrary .tr10-collectionCard{min-height:104px!important;border:0!important;border-radius:13px!important;background:#050c11!important;box-shadow:inset 0 1px rgba(255,255,255,.035),0 8px 22px rgba(0,0,0,.22),0 0 0 1px rgba(126,168,188,.08)!important;overflow:hidden!important}
        @media(hover:hover){.tr10-premiumLibrary .tr10-collectionCard:hover{background:#08131a!important;transform:translateY(-2px)!important;box-shadow:inset 0 1px rgba(255,255,255,.06),0 16px 34px rgba(0,0,0,.30),0 0 0 1px rgba(34,165,237,.16)!important}}
        .tr10-premiumLibrary .tr10-collectionCard>.tr10-collectionOpen{min-height:104px!important;padding:10px!important;grid-template-columns:68px minmax(0,1fr) 18px!important;gap:10px!important}
        .tr10-premiumLibrary .tr10-collectionOpen .tr10-art--card{width:68px!important;height:68px!important;border:0!important;border-radius:10px!important;box-shadow:0 10px 22px rgba(0,0,0,.38),0 0 0 1px rgba(255,255,255,.06)!important}
        .tr10-premiumLibrary .tr10-collectionOpen h3{font-size:15px!important;line-height:1.15!important;display:-webkit-box!important;-webkit-line-clamp:2!important;-webkit-box-orient:vertical!important;white-space:normal!important;overflow:hidden!important;text-overflow:clip!important}.tr10-premiumLibrary .tr10-collectionOpen p{font-size:9px!important;color:#879ca6!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important}
        .tr10-premiumLibrary .tr10-collectionChevron{color:#66b6ea!important;font-size:22px!important;opacity:.78!important}
        .tr10-premiumLibrary .tr10-collectionCard>.tr10-collectionPlay{width:auto!important;height:40px!important;min-width:76px!important;margin-right:10px!important;padding:0 12px!important;border-radius:8px!important;border:0!important;background:#071018!important;color:#d9eaf3!important;font-size:10px!important;box-shadow:inset 0 1px rgba(255,255,255,.04),0 7px 16px rgba(0,0,0,.24)!important;position:relative!important;display:flex!important;align-items:center!important;justify-content:center!important;gap:7px!important}
        .tr10-premiumLibrary .tr10-collectionCard>.tr10-collectionPlay:before{content:none!important;display:none!important}
        @media(hover:hover){.tr10-premiumLibrary .tr10-collectionCard>.tr10-collectionPlay:hover{background:radial-gradient(circle at 35% 28%,#203340,#0b1820 68%)!important;border-color:rgba(34,165,237,.52)!important;transform:translateY(-1px)!important;box-shadow:inset 0 1px rgba(255,255,255,.10),0 10px 20px rgba(0,0,0,.31),0 0 14px rgba(11,124,255,.10)!important}}

        /* Playlists: one collection stage, track rails aligned and quiet. */
        .tr10-premiumLibrary .tr21-playlists{grid-template-columns:190px minmax(0,1fr)!important;background:transparent!important;border:0!important;gap:0!important}
        .tr10-premiumLibrary .tr21-playlistDock{background:#03090d!important;border-right:1px solid rgba(126,166,185,.07)!important;padding:12px!important}
        .tr10-premiumLibrary .tr21-playlistChoices>button{background:transparent!important;border:0!important;border-radius:7px!important;box-shadow:none!important;color:#82969f!important}.tr10-premiumLibrary .tr21-playlistChoices>button.is-active{background:#08131b!important;color:#fff!important;box-shadow:inset 2px 0 var(--m25-blue)!important}
        .tr10-premiumLibrary .tr21-playlistStage{padding:12px!important;background:transparent!important}.tr10-premiumLibrary .tr21-playlistHero{border:0!important;border-radius:14px!important;background:#061018!important;box-shadow:inset 0 1px rgba(255,255,255,.04),0 16px 36px rgba(0,0,0,.26),0 0 0 1px rgba(126,168,188,.08)!important}.tr10-premiumLibrary .tr21-playlistHero:after{height:2px!important;background:var(--m25-blue)!important;opacity:.65!important}.tr10-premiumLibrary .tr21-playlistArt{box-shadow:0 16px 34px rgba(0,0,0,.42),0 0 0 1px rgba(255,255,255,.06)!important}
        .tr10-premiumLibrary .tr21-playlistHeroActions>button{background:#071119!important;border:1px solid rgba(126,168,188,.10)!important;color:#dce8ed!important;box-shadow:inset 0 1px rgba(255,255,255,.04),0 5px 12px rgba(0,0,0,.18)!important}.tr10-premiumLibrary .tr21-playlistTracks>article{background:transparent!important;border:0!important;border-bottom:1px solid rgba(126,166,185,.06)!important;border-radius:0!important}

        /* Smart Mix: professional generator, not nested dashboard boxes. */
        .tr10-premiumLibrary .tr10-smart{padding:14px!important;gap:12px!important;background:transparent!important}.tr10-premiumLibrary .tr10-smartBuild,.tr10-premiumLibrary .tr10-savedMixes{background:#050c11!important;border:0!important;border-radius:14px!important;box-shadow:inset 0 1px rgba(255,255,255,.035),0 14px 30px rgba(0,0,0,.23),0 0 0 1px rgba(126,168,188,.08)!important}.tr10-premiumLibrary .tr10-intensity button{background:#071119!important;border:0!important;box-shadow:inset 0 1px rgba(255,255,255,.035),0 0 0 1px rgba(126,168,188,.08)!important}.tr10-premiumLibrary .tr10-intensity button.is-active{background:#0a1721!important;color:#fff!important;box-shadow:inset 0 -2px var(--m25-blue),inset 0 1px rgba(255,255,255,.07),0 0 0 1px rgba(11,124,255,.20)!important}.tr10-premiumLibrary .tr10-smartLaunch{background:#100d08!important;border:1px solid rgba(235,139,15,.38)!important;color:#ffb34a!important;box-shadow:inset 0 1px rgba(255,255,255,.055),0 10px 22px rgba(0,0,0,.24)!important}.tr10-premiumLibrary .tr10-smartLaunch:hover{background:#171108!important;border-color:rgba(255,173,53,.55)!important;color:#ffc66f!important;filter:none!important;transform:translateY(-1px)!important}

        /* Intelligence v25: clear typography, no collisions, no fake AI geometry. */
        .tr10-premiumLibrary .mvp25-intel{display:grid;gap:12px;padding:14px;background:transparent;color:#f5f9fb}
        .tr10-premiumLibrary .mvp25-intelHero,.tr10-premiumLibrary .mvp25-intelSignal,.tr10-premiumLibrary .mvp25-intelControl,.tr10-premiumLibrary .mvp25-tasteField,.tr10-premiumLibrary .mvp25-pr{background:#050c11;border:0;border-radius:14px;box-shadow:inset 0 1px rgba(255,255,255,.035),0 14px 30px rgba(0,0,0,.22),0 0 0 1px rgba(126,168,188,.08)}
        .tr10-premiumLibrary .mvp25-intelHero{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:18px;align-items:end;padding:22px}.tr10-premiumLibrary .mvp25-kicker{display:block;font-size:9px;font-weight:900;letter-spacing:.15em;color:#6baef4}.tr10-premiumLibrary .mvp25-intelIdentity h2{margin:6px 0 8px;font-size:clamp(30px,4vw,48px);line-height:.98;letter-spacing:-.05em}.tr10-premiumLibrary .mvp25-intelIdentity p{max-width:620px;margin:0;color:#889da6;font-size:11px;line-height:1.55}
        .tr10-premiumLibrary .mvp25-intelStatus{display:flex;align-items:stretch;gap:8px}.tr10-premiumLibrary .mvp25-intelStatus>div{min-width:120px;padding:9px 11px;display:grid;gap:4px;border-left:1px solid rgba(126,168,188,.09)}.tr10-premiumLibrary .mvp25-intelStatus span{font-size:8px;letter-spacing:.11em;color:#718791;font-weight:850}.tr10-premiumLibrary .mvp25-intelStatus strong{font-size:14px}.tr10-premiumLibrary .mvp25-intelStatus button{min-width:126px;padding:8px 11px;display:grid;grid-template-columns:1fr auto;grid-template-areas:"label switch" "value switch";text-align:left;border:1px solid rgba(126,168,188,.10);border-radius:9px;background:#071119;color:#dce8ed}.tr10-premiumLibrary .mvp25-intelStatus button>span{grid-area:label}.tr10-premiumLibrary .mvp25-intelStatus button>strong{grid-area:value}.tr10-premiumLibrary .mvp25-intelStatus button>i{grid-area:switch;align-self:center;width:31px;height:17px;padding:2px;border-radius:999px;background:#101b20}.tr10-premiumLibrary .mvp25-intelStatus button>i b{display:block;width:13px;height:13px;border-radius:50%;background:#6c7e86;transition:transform .16s ease,background .16s ease}.tr10-premiumLibrary .mvp25-intelStatus button.is-on{border-color:rgba(55,206,137,.24)}.tr10-premiumLibrary .mvp25-intelStatus button.is-on>i b{transform:translateX(14px);background:#56d89a;box-shadow:0 0 8px rgba(70,213,143,.32)}
        .tr10-premiumLibrary .mvp25-intelMessage{padding:9px 12px;border-left:2px solid var(--m25-blue);background:#06121a;color:#a9c6d2;font-size:10px}.tr10-premiumLibrary .mvp25-intelMessage button{float:right;border:0;background:transparent;color:#a9c6d2}
        .tr10-premiumLibrary .mvp25-intelFlow{display:grid;grid-template-columns:1fr 26px 1.15fr 26px 1fr;align-items:stretch;gap:0}.tr10-premiumLibrary .mvp25-intelFlow>article{padding:14px 16px;background:#040a0f;border-bottom:1px solid rgba(126,168,188,.06);min-width:0}.tr10-premiumLibrary .mvp25-intelFlow>article.is-current{background:#061018}.tr10-premiumLibrary .mvp25-intelFlow>i{display:grid;place-items:center;color:#456777;font-size:20px;font-style:normal}.tr10-premiumLibrary .mvp25-intelFlow span{font-size:8px;color:#6e8791;font-weight:900;letter-spacing:.10em}.tr10-premiumLibrary .mvp25-intelFlow strong{display:block;margin-top:6px;font-size:28px;line-height:1;color:#fff;font-variant-numeric:tabular-nums}.tr10-premiumLibrary .mvp25-intelFlow strong.is-title{font-size:18px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.tr10-premiumLibrary .mvp25-intelFlow b{display:block;margin-top:5px;font-size:10px;color:#a8bbc3;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.tr10-premiumLibrary .mvp25-intelFlow small{display:block;margin-top:5px;font-size:9px;color:#687d86;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
        .tr10-premiumLibrary .mvp25-intelSignal{padding:15px}.tr10-premiumLibrary .mvp25-intelSignal>header,.tr10-premiumLibrary .mvp25-tasteField>header,.tr10-premiumLibrary .mvp25-pr>header{display:flex;justify-content:space-between;align-items:end;gap:12px}.tr10-premiumLibrary .mvp25-intelSignal h3,.tr10-premiumLibrary .mvp25-tasteField h3,.tr10-premiumLibrary .mvp25-pr h3{margin:4px 0 0;font-size:19px;letter-spacing:-.025em}.tr10-premiumLibrary .mvp25-intelSignal>header>small{font-size:8px;color:#657983}.tr10-premiumLibrary .mvp25-intelSignal>header>small i{display:inline-block;width:6px;height:6px;border-radius:50%;margin-right:5px;background:#63767e}.tr10-premiumLibrary .mvp25-intelSignal>header>small.is-live{color:#82e8b3}.tr10-premiumLibrary .mvp25-intelSignal>header>small.is-live i{background:#51d994;box-shadow:0 0 7px rgba(70,213,143,.35)}
        .tr10-premiumLibrary .mvp25-intelChannels{display:grid;grid-template-columns:repeat(6,1fr);margin-top:13px;border-top:1px solid rgba(126,168,188,.06);border-bottom:1px solid rgba(126,168,188,.06)}.tr10-premiumLibrary .mvp25-intelChannels>div{padding:13px 12px;border-right:1px solid rgba(126,168,188,.06);min-width:0}.tr10-premiumLibrary .mvp25-intelChannels>div:last-child{border-right:0}.tr10-premiumLibrary .mvp25-intelChannels span{font-size:8px;color:#6f858f;font-weight:900}.tr10-premiumLibrary .mvp25-intelChannels strong{display:block;margin:6px 0 9px;font-size:25px;font-variant-numeric:tabular-nums}.tr10-premiumLibrary .mvp25-intelChannels>div>i{display:block;height:2px;background:#111a20}.tr10-premiumLibrary .mvp25-intelChannels>div>i b{display:block;width:var(--mvp25-value);height:2px;background:var(--m25-blue);box-shadow:0 0 7px rgba(11,124,255,.20)}
        .tr10-premiumLibrary .mvp25-intelControl{display:grid;grid-template-columns:.9fr 1.1fr}.tr10-premiumLibrary .mvp25-intelMemory,.tr10-premiumLibrary .mvp25-intelTarget{padding:17px}.tr10-premiumLibrary .mvp25-intelMemory{border-right:1px solid rgba(126,168,188,.07)}.tr10-premiumLibrary .mvp25-intelControl h3{margin:5px 0 5px;font-size:19px}.tr10-premiumLibrary .mvp25-intelControl p{margin:0;color:#8398a2;font-size:10px;line-height:1.45}.tr10-premiumLibrary .mvp25-confidence{display:grid;grid-template-columns:auto 1fr auto;gap:9px;align-items:center;margin-top:15px}.tr10-premiumLibrary .mvp25-confidence span,.tr10-premiumLibrary .mvp25-confidence strong{font-size:8px;color:#879aa3}.tr10-premiumLibrary .mvp25-confidence>i{height:2px;background:#121a20}.tr10-premiumLibrary .mvp25-confidence>i b{display:block;height:2px;background:var(--m25-blue)}.tr10-premiumLibrary .mvp25-intelActions{display:flex;gap:7px;margin-top:12px}.tr10-premiumLibrary .mvp25-intelActions button{height:38px;padding:0 12px;border:1px solid rgba(126,168,188,.10);border-radius:8px;background:#071119;color:#dbe8ee}.tr10-premiumLibrary .mvp25-targetRail{position:relative;margin-top:24px;height:3px;background:#121b20}.tr10-premiumLibrary .mvp25-targetRail>i{position:absolute;inset:0;background:linear-gradient(90deg,var(--m25-blue),rgba(11,124,255,.20),var(--m25-orange));opacity:.45}.tr10-premiumLibrary .mvp25-targetRail>b{position:absolute;top:50%;width:12px;height:12px;margin:-6px 0 0 -6px;border-radius:50%;background:#f6fbff;border:2px solid var(--m25-blue);box-shadow:0 0 10px rgba(11,124,255,.28)}.tr10-premiumLibrary .mvp25-targetLabels{display:flex;justify-content:space-between;margin-top:7px;color:#617780;font-size:7px;font-weight:900;letter-spacing:.08em}
        .tr10-premiumLibrary .mvp25-tasteField{padding:15px}.tr10-premiumLibrary .mvp25-tasteField>header p{margin:4px 0 0;color:#80949d;font-size:10px}.tr10-premiumLibrary .mvp25-tasteField>header>small{color:#667b84;font-size:8px}.tr10-premiumLibrary .mvp25-tasteRows{margin-top:12px}.tr10-premiumLibrary .mvp25-tasteRows button{width:100%;display:grid;grid-template-columns:34px minmax(0,1fr) 46px 120px;gap:10px;align-items:center;min-height:54px;padding:7px 10px;border:0;border-top:1px solid rgba(126,168,188,.06);background:transparent;color:#fff;text-align:left}.tr10-premiumLibrary .mvp25-tasteRows button>span{color:#607781;font-size:9px}.tr10-premiumLibrary .mvp25-tasteRows button div strong{display:block;font-size:12px}.tr10-premiumLibrary .mvp25-tasteRows button div small{display:block;margin-top:2px;color:#758b94;font-size:9px}.tr10-premiumLibrary .mvp25-tasteRows button>b{text-align:right;font-size:18px;font-variant-numeric:tabular-nums}.tr10-premiumLibrary .mvp25-tasteRows button>i{height:2px;background:#121b20}.tr10-premiumLibrary .mvp25-tasteRows button>i em{display:block;width:var(--mvp25-score);height:2px;background:var(--m25-blue)}@media(hover:hover){.tr10-premiumLibrary .mvp25-tasteRows button:hover{background:#071119!important;filter:none!important}}
        .tr10-premiumLibrary .mvp25-pr{padding:13px 15px}.tr10-premiumLibrary .mvp25-pr>header>b{font-size:24px;color:#6ebdff}.tr10-premiumLibrary .mvp25-pr.is-empty>p{margin:9px 0 2px;color:#70858e;font-size:9px}.tr10-premiumLibrary .mvp25-pr.has-records>div{display:grid;grid-template-columns:repeat(2,1fr);gap:6px;margin-top:10px}.tr10-premiumLibrary .mvp25-pr article{display:grid;grid-template-columns:1fr 34px;align-items:center;padding:8px 9px;border:1px solid rgba(126,168,188,.07);border-radius:8px;background:#071017}.tr10-premiumLibrary .mvp25-pr article strong,.tr10-premiumLibrary .mvp25-pr article span,.tr10-premiumLibrary .mvp25-pr article small{display:block}.tr10-premiumLibrary .mvp25-pr article span{font-size:9px;color:#889ca5}.tr10-premiumLibrary .mvp25-pr article small{font-size:8px;color:#647982}.tr10-premiumLibrary .mvp25-pr article button{width:30px;height:30px;border-radius:50%;border:1px solid rgba(34,165,237,.18);background:#08131b;color:#79c5ff}

        /* Audition: compact media console and restored list actions. */
        .tr10-premiumLibrary .mvp-audition{background:transparent!important}.tr10-premiumLibrary .mvp-auditionHero{padding:17px 14px!important;background:#040b10!important;border:0!important;border-bottom:1px solid rgba(126,168,188,.07)!important}.tr10-premiumLibrary .mvp-auditionNav{min-height:48px!important;padding:4px 8px!important;background:#03090d!important;border:0!important;border-bottom:1px solid rgba(126,168,188,.07)!important}.tr10-premiumLibrary .mvp-auditionNav button{height:38px!important;min-height:38px!important;background:transparent!important;border:0!important;border-radius:7px!important;color:#8498a1!important;box-shadow:none!important}.tr10-premiumLibrary .mvp-auditionNav button.is-active{background:#08141d!important;color:#fff!important;box-shadow:inset 0 -2px var(--m25-blue)!important}.tr10-premiumLibrary .mvp-auditionNav button.is-import{margin-left:auto!important;border:1px solid rgba(235,139,15,.24)!important;color:#ffb24a!important;background:#100d08!important}
        .tr10-premiumLibrary .mvp-auditionToolbar{display:grid!important;grid-template-columns:1fr 180px!important;gap:12px!important;align-items:end!important;padding:11px 12px!important}.tr10-premiumLibrary .mvp-auditionSortSelect{width:180px!important}
        .tr10-premiumLibrary .mvp-auditionLists{display:grid!important;grid-template-columns:1fr!important;gap:7px!important;padding:0 12px 12px!important}.tr10-premiumLibrary .mvp-auditionListCard{display:grid!important;grid-template-columns:minmax(240px,1.2fr) 150px 290px!important;grid-template-areas:"head stats foot" "progress progress progress" "pre pre pre"!important;align-items:center!important;gap:8px 14px!important;min-height:92px!important;padding:10px!important;border:0!important;border-radius:11px!important;background:#050c11!important;box-shadow:inset 0 1px rgba(255,255,255,.035),0 8px 22px rgba(0,0,0,.20),0 0 0 1px rgba(126,168,188,.07)!important;overflow:visible!important}.tr10-premiumLibrary .mvp-auditionListCard>header{grid-area:head!important;display:grid!important;grid-template-columns:34px minmax(0,1fr) auto!important;gap:9px!important;align-items:center!important}.tr10-premiumLibrary .mvp-auditionListIcon{width:34px!important;height:34px!important;border-radius:9px!important}.tr10-premiumLibrary .mvp-auditionListTitle h3{font-size:13px!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important}.tr10-premiumLibrary .mvp-auditionPercent{font-size:18px!important;color:#67c3ff!important}.tr10-premiumLibrary .mvp-auditionListStats{grid-area:stats!important;display:grid!important;grid-template-columns:repeat(4,1fr)!important;gap:4px!important}.tr10-premiumLibrary .mvp-auditionListStats span{padding:5px!important;background:transparent!important;border:0!important;text-align:center!important}.tr10-premiumLibrary .mvp-auditionListStats b{font-size:13px!important}.tr10-premiumLibrary .mvp-auditionListStats small{font-size:7px!important}.tr10-premiumLibrary .mvp-auditionListCard>footer{grid-area:foot!important;display:flex!important;justify-content:flex-end!important;align-items:center!important;gap:4px!important;overflow:visible!important;opacity:1!important;visibility:visible!important}.tr10-premiumLibrary .mvp-auditionListCard>footer button{display:inline-flex!important;align-items:center!important;justify-content:center!important;height:34px!important;min-height:34px!important;padding:0 8px!important;border:0!important;border-radius:7px!important;background:#071119!important;color:#8498a1!important;font-size:7px!important;box-shadow:inset 0 0 0 1px rgba(126,168,188,.07)!important}.tr10-premiumLibrary .mvp-auditionListCard>footer .is-open{min-width:94px!important;background:#081722!important;color:#dff4ff!important;box-shadow:inset 0 0 0 1px rgba(34,165,237,.22)!important}.tr10-premiumLibrary .mvp-auditionListCard>footer .is-danger{color:#ff9ba2!important;background:#12090b!important}.tr10-premiumLibrary .mvp-auditionProgress{grid-area:progress!important;height:2px!important;background:#10181d!important}.tr10-premiumLibrary .mvp-auditionProgress i{background:var(--m25-blue)!important;box-shadow:0 0 8px rgba(11,124,255,.20)!important}.tr10-premiumLibrary .mvp-auditionPreflight{grid-area:pre!important;padding:3px 0!important}
        @media(hover:hover){.tr10-premiumLibrary .mvp-auditionListCard>footer button:not(:disabled):hover{background:#0c1921!important;color:#fff!important;filter:none!important;transform:translateY(-1px)!important}.tr10-premiumLibrary .mvp-auditionListCard>footer .is-open:hover{background:#0b1d29!important;color:#fff!important}}
        .tr10-premiumLibrary .mvp-auditionStage{padding:0 10px 12px!important}.tr10-premiumLibrary .mvp-auditionSongCard{grid-template-columns:minmax(170px,215px) minmax(0,1fr)!important;gap:18px!important;padding:14px!important;min-height:0!important;border:0!important;border-radius:14px!important;background:#050c11!important;box-shadow:inset 0 1px rgba(255,255,255,.035),0 18px 38px rgba(0,0,0,.25),0 0 0 1px rgba(126,168,188,.08)!important;overflow:hidden!important}.tr10-premiumLibrary .mvp-auditionAmbient{opacity:.035!important;filter:blur(70px)!important}.tr10-premiumLibrary .mvp-auditionArtwork{width:100%!important;max-width:215px!important;aspect-ratio:1!important;align-self:start!important}.tr10-premiumLibrary .mvp-auditionArtwork img{border-radius:12px!important;box-shadow:0 16px 34px rgba(0,0,0,.42),0 0 0 1px rgba(255,255,255,.06)!important}.tr10-premiumLibrary .mvp-auditionSongInfo h2{font-size:clamp(30px,4vw,48px)!important;line-height:.98!important;letter-spacing:-.05em!important;margin:5px 0 6px!important}.tr10-premiumLibrary .mvp-auditionMeta{font-size:9px!important;color:#748991!important}.tr10-premiumLibrary .mvp-auditionActionLabel{display:none!important}
        .tr10-premiumLibrary .mvp-auditionListen{display:grid!important;grid-template-columns:1fr 1fr!important;gap:6px!important;margin-top:14px!important;border:0!important;background:transparent!important}.tr10-premiumLibrary .mvp-auditionListen button{height:46px!important;min-height:46px!important;padding:0 12px!important;border:1px solid rgba(126,168,188,.10)!important;border-radius:9px!important;background:#071119!important;color:#dce8ed!important;box-shadow:inset 0 1px rgba(255,255,255,.04),0 5px 12px rgba(0,0,0,.18)!important}.tr10-premiumLibrary .mvp-auditionListen button.is-preview{border-color:rgba(34,165,237,.20)!important}.tr10-premiumLibrary .mvp-auditionListen button.is-youtube{border-color:rgba(230,65,78,.17)!important}.tr10-premiumLibrary .mvp-auditionListen button.is-playing{background:#08141d!important;border-color:rgba(34,165,237,.40)!important;box-shadow:inset 0 -2px var(--m25-blue),inset 0 1px rgba(255,255,255,.06)!important}.tr10-premiumLibrary .mvp-auditionControlIcon{width:22px!important;height:22px!important}.tr10-premiumLibrary .mvp-auditionControlCopy strong{font-size:9.5px!important}.tr10-premiumLibrary .mvp-auditionControlCopy small{font-size:7px!important}
        .tr10-premiumLibrary .mvp-auditionDecision{display:grid!important;grid-template-columns:repeat(3,1fr)!important;gap:3px!important;margin-top:6px!important;padding:3px!important;border:1px solid rgba(126,168,188,.08)!important;border-radius:10px!important;background:#040a0f!important}.tr10-premiumLibrary .mvp-auditionDecision button{height:44px!important;min-height:44px!important;padding:0 10px!important;border:0!important;border-radius:7px!important;background:transparent!important;color:#a5b4ba!important;box-shadow:none!important}.tr10-premiumLibrary .mvp-auditionDecisionIcon{width:18px!important;height:18px!important}.tr10-premiumLibrary .mvp-auditionDecision strong{font-size:9px!important}.tr10-premiumLibrary .mvp-auditionDecision small{display:none!important}.tr10-premiumLibrary .mvp-auditionDecision button.is-active{background:#081119!important}.tr10-premiumLibrary .mvp-auditionDecision button.is-keep.is-active{box-shadow:inset 0 -2px rgba(70,213,143,.56)!important;color:#aef2ca!important}.tr10-premiumLibrary .mvp-auditionDecision button.is-maybe.is-active{box-shadow:inset 0 -2px rgba(235,169,63,.56)!important;color:#ffd89b!important}.tr10-premiumLibrary .mvp-auditionDecision button.is-pass.is-active{box-shadow:inset 0 -2px rgba(241,79,91,.56)!important;color:#ffc0c5!important}
        @media(hover:hover){.tr10-premiumLibrary .mvp-auditionListen button:hover,.tr10-premiumLibrary .mvp-auditionDecision button:not(:disabled):hover{background:#0b1821!important;color:#fff!important;filter:none!important;transform:translateY(-1px)!important;box-shadow:inset 0 1px rgba(255,255,255,.07),0 8px 16px rgba(0,0,0,.22)!important}.tr10-premiumLibrary .mvp-auditionDecision .is-keep:hover{box-shadow:inset 0 -2px rgba(70,213,143,.38)!important}.tr10-premiumLibrary .mvp-auditionDecision .is-maybe:hover{box-shadow:inset 0 -2px rgba(235,169,63,.38)!important}.tr10-premiumLibrary .mvp-auditionDecision .is-pass:hover{box-shadow:inset 0 -2px rgba(241,79,91,.38)!important}}
        .tr10-premiumLibrary .mvp-auditionPager{display:grid!important;grid-template-columns:auto 1fr auto!important;gap:8px!important;align-items:center!important;margin-top:8px!important;border-top:1px solid rgba(126,168,188,.06)!important;padding-top:8px!important}.tr10-premiumLibrary .mvp-auditionPager button{height:34px!important;min-height:34px!important;border:0!important;background:transparent!important;color:#7e929b!important;font-size:8px!important}.tr10-premiumLibrary .mvp-auditionPager .is-prev{grid-column:1!important}.tr10-premiumLibrary .mvp-auditionPager .is-next-unreviewed{grid-column:3!important}

        /* Song Control: fixed artwork footprint, clear dirty/save transaction. */
        .tr10-detailPortal .tr10-inspector{background:#050c11!important;border:0!important;box-shadow:0 40px 110px rgba(0,0,0,.72),0 0 0 1px rgba(34,165,237,.18)!important}.tr10-detailPortal .tr10-inspector>header{background:#071119!important}.tr10-detailPortal .tr10-detailPreviewArt{width:72px!important;height:72px!important;min-width:72px!important;max-width:72px!important;min-height:72px!important;max-height:72px!important;object-fit:cover!important;border-radius:10px!important;box-shadow:0 10px 24px rgba(0,0,0,.38),0 0 0 1px rgba(255,255,255,.06)!important}.tr10-detailPortal .tr10-detailCandidates img,.tr10-detailPortal .tr10-candidates img,.tr10-detailPortal .tr10-candidateArt{width:58px!important;height:58px!important;max-width:58px!important;max-height:58px!important;object-fit:cover!important;border-radius:8px!important}.tr10-detailPortal .tr10-saveButton:not(:disabled){background:#081a25!important;border-color:rgba(34,165,237,.32)!important;color:#eef9ff!important}.tr10-detailPortal .tr10-saveButton.is-changed{background:#08150f!important;border-color:rgba(70,213,143,.30)!important;color:#b6f4ce!important}

        /* Discover: reduce box density while keeping artwork authoritative. */
        .tr10-premiumLibrary .tr10-discover{background:transparent!important}.tr10-premiumLibrary .tr10-discoverSeed,.tr10-premiumLibrary .tr10-discoverCategory{background:#050c11!important;border:0!important;box-shadow:inset 0 1px rgba(255,255,255,.03),0 9px 22px rgba(0,0,0,.20),0 0 0 1px rgba(126,168,188,.07)!important}.tr10-premiumLibrary .tr10-discoverGrid article{background:#061018!important;border:0!important;border-radius:10px!important;box-shadow:inset 0 1px rgba(255,255,255,.03),0 9px 20px rgba(0,0,0,.22),0 0 0 1px rgba(126,168,188,.07)!important}.tr10-premiumLibrary .tr10-discoverGrid article footer button{background:transparent!important;border:1px solid rgba(126,168,188,.09)!important;color:#8ea3ad!important;box-shadow:none!important}

        /* Responsive: quality stays, composition changes. */
        @media(max-width:1180px){
          .tr10-premiumLibrary .tr10-tableHead,.tr10-premiumLibrary .tr10-row{grid-template-columns:38px 26px minmax(230px,1fr) 64px 90px minmax(340px,auto)!important;column-gap:7px!important}.tr10-premiumLibrary .tr10-actions button{padding:0 6px!important;font-size:7px!important}.tr10-premiumLibrary .tr10-cardGrid{grid-template-columns:repeat(2,minmax(0,1fr))!important}
        }
        @media(max-width:900px){
          .tr10-premiumLibrary .tr10-toolbar{grid-template-columns:1fr 1fr!important}.tr10-premiumLibrary .tr10-toolbar label:first-child{grid-column:1/-1!important}.tr10-premiumLibrary .tr10-tableHead{display:none!important}.tr10-premiumLibrary .tr10-row{grid-template-columns:34px 24px minmax(0,1fr) 72px!important;grid-template-areas:"order check track energy" "order check track time" "actions actions actions actions"!important;row-gap:5px!important;padding:9px!important}.tr10-premiumLibrary .tr10-orderCell{grid-area:order!important}.tr10-premiumLibrary .tr10-check{grid-area:check!important}.tr10-premiumLibrary .tr10-trackCell{grid-area:track!important}.tr10-premiumLibrary .tr10-duration{grid-area:time!important;justify-self:end!important}.tr10-premiumLibrary .tr10-energy{grid-area:energy!important;width:72px!important;min-width:72px!important}.tr10-premiumLibrary .tr10-actions{grid-area:actions!important;justify-self:stretch!important;display:grid!important;grid-template-columns:repeat(6,minmax(0,1fr))!important}.tr10-premiumLibrary .tr10-actions button{border-left:1px solid rgba(126,166,185,.06)!important}.tr10-premiumLibrary .tr21-playlists{grid-template-columns:1fr!important}.tr10-premiumLibrary .tr21-playlistDock{border-right:0!important;border-bottom:1px solid rgba(126,166,185,.07)!important}.tr10-premiumLibrary .tr21-playlistChoices{display:flex!important;overflow-x:auto!important}.tr10-premiumLibrary .mvp-auditionListCard{grid-template-columns:1fr!important;grid-template-areas:"head" "stats" "progress" "pre" "foot"!important}.tr10-premiumLibrary .mvp-auditionListCard>footer{justify-content:flex-start!important;flex-wrap:wrap!important}.tr10-premiumLibrary .mvp25-intelChannels{grid-template-columns:repeat(3,1fr)!important}.tr10-premiumLibrary .mvp25-intelChannels>div:nth-child(3){border-right:0}.tr10-premiumLibrary .mvp25-intelControl{grid-template-columns:1fr!important}.tr10-premiumLibrary .mvp25-intelMemory{border-right:0!important;border-bottom:1px solid rgba(126,168,188,.07)!important}
        }
        @media(max-width:760px){
          .tr10-premiumLibrary .tr10-tabs{display:flex!important;grid-template-columns:none!important;gap:6px!important;overflow-x:auto!important;overflow-y:hidden!important;padding:5px max(10px,calc(env(safe-area-inset-left) + 8px)) 7px!important;scroll-snap-type:x mandatory!important;scroll-padding-inline:calc(50vw - 56px)!important;-webkit-overflow-scrolling:touch!important;scrollbar-width:none!important}.tr10-premiumLibrary .tr10-tabs::-webkit-scrollbar{display:none}.tr10-premiumLibrary .tr10-tabs button{flex:0 0 112px!important;min-width:112px!important;width:112px!important;height:46px!important;scroll-snap-align:center!important}.tr10-premiumLibrary .tr10-tabs button>span:last-child{font-size:9px!important}
          .tr10-premiumLibrary .tr10-healthRail button{min-width:116px!important}.tr10-premiumLibrary .tr10-toolbar{grid-template-columns:1fr!important;padding:9px!important}.tr10-premiumLibrary .tr10-toolbar label:first-child{grid-column:auto!important}.tr10-premiumLibrary .tr10-toolbar input,.tr10-premiumLibrary .mvpPremiumSelectTrigger{min-height:50px!important;font-size:12px!important}.tr10-premiumLibrary .tr10-row{grid-template-columns:30px minmax(0,1fr) 72px!important;grid-template-areas:"order track energy" "order track time" "actions actions actions"!important}.tr10-premiumLibrary .tr10-check{display:none!important}.tr10-premiumLibrary .tr10-trackCell{grid-template-columns:38px 44px minmax(0,1fr)!important}.tr10-premiumLibrary .tr10-healthBadge{grid-column:3!important;justify-self:start!important}.tr10-premiumLibrary .tr10-trackText strong{font-size:13px!important}.tr10-premiumLibrary .tr10-trackText span{font-size:9.5px!important}.tr10-premiumLibrary .tr10-actions{grid-template-columns:repeat(3,1fr)!important;gap:2px!important}.tr10-premiumLibrary .tr10-actions button{height:40px!important;min-height:40px!important;font-size:8px!important}.tr10-premiumLibrary .tr10-cardGrid{grid-template-columns:1fr!important;padding:9px!important}.tr10-premiumLibrary .tr10-collectionCard>.tr10-collectionOpen{min-height:94px!important}.tr10-premiumLibrary .tr10-collectionOpen .tr10-art--card{width:64px!important;height:64px!important}.tr10-premiumLibrary .tr10-smart{grid-template-columns:1fr!important;padding:9px!important}.tr10-premiumLibrary .mvp25-intelHero{grid-template-columns:1fr!important;padding:16px!important}.tr10-premiumLibrary .mvp25-intelStatus{display:grid!important;grid-template-columns:1fr 1fr!important}.tr10-premiumLibrary .mvp25-intelStatus>div{border-left:0!important}.tr10-premiumLibrary .mvp25-intelFlow{grid-template-columns:1fr!important}.tr10-premiumLibrary .mvp25-intelFlow>i{display:none!important}.tr10-premiumLibrary .mvp25-intelFlow>article{border-bottom:1px solid rgba(126,168,188,.07)!important}.tr10-premiumLibrary .mvp25-intelChannels{grid-template-columns:repeat(2,1fr)!important}.tr10-premiumLibrary .mvp25-intelChannels>div{border-right:1px solid rgba(126,168,188,.06)!important;border-bottom:1px solid rgba(126,168,188,.06)!important}.tr10-premiumLibrary .mvp25-tasteRows button{grid-template-columns:28px minmax(0,1fr) 38px!important}.tr10-premiumLibrary .mvp25-tasteRows button>i{grid-column:2/-1!important}.tr10-premiumLibrary .mvp-auditionHero{flex-direction:column!important}.tr10-premiumLibrary .mvp-auditionToolbar{grid-template-columns:1fr!important}.tr10-premiumLibrary .mvp-auditionSortSelect{width:100%!important}.tr10-premiumLibrary .mvp-auditionSongCard{grid-template-columns:1fr!important}.tr10-premiumLibrary .mvp-auditionArtwork{max-width:min(70vw,260px)!important;justify-self:center!important}.tr10-premiumLibrary .mvp-auditionSongInfo h2{font-size:34px!important}.tr10-premiumLibrary .mvp-auditionListen button{height:48px!important}.tr10-premiumLibrary .mvp-auditionDecision button{height:46px!important}.tr10-detailPortal .tr10-detailPreviewArt{width:64px!important;height:64px!important;min-width:64px!important;max-width:64px!important;min-height:64px!important;max-height:64px!important}
        }
        @media(max-width:430px){
          .tr10-premiumLibrary .tr10-sectionHead{padding:15px 12px 12px!important}.tr10-premiumLibrary .tr10-sectionHead h2{font-size:25px!important}.tr10-premiumLibrary .tr10-tabs button{flex-basis:106px!important;min-width:106px!important;width:106px!important}.tr10-premiumLibrary .tr10-row{grid-template-columns:28px minmax(0,1fr) 64px!important;column-gap:6px!important}.tr10-premiumLibrary .tr10-orderCell{width:28px!important}.tr10-premiumLibrary .tr10-orderCell button{width:24px!important}.tr10-premiumLibrary .tr10-trackCell{grid-template-columns:36px 40px minmax(0,1fr)!important;gap:6px!important}.tr10-premiumLibrary .tr10-play{width:36px!important;height:36px!important}.tr10-premiumLibrary .tr10-art--row{width:40px!important;height:40px!important}.tr10-premiumLibrary .tr10-energy{width:64px!important;min-width:64px!important;padding:0 4px!important}.tr10-premiumLibrary .tr10-energy>span{font-size:7px!important}.tr10-premiumLibrary .tr10-actions button{font-size:7.5px!important;padding:0 4px!important}.tr10-premiumLibrary .mvp25-intelIdentity h2{font-size:32px!important}.tr10-premiumLibrary .mvp25-intelStatus{grid-template-columns:1fr!important}.tr10-premiumLibrary .mvp25-intelChannels strong{font-size:22px!important}.tr10-premiumLibrary .mvp-auditionListen{grid-template-columns:1fr 1fr!important}.tr10-premiumLibrary .mvp-auditionControlCopy small{display:none!important}.tr10-premiumLibrary .mvp-auditionDecision{grid-template-columns:repeat(3,1fr)!important}.tr10-premiumLibrary .mvp-auditionDecision button{padding:0 5px!important}.tr10-premiumLibrary .mvp-auditionDecision strong{font-size:8px!important}
        }
