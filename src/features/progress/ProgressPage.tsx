...
// optional:
// import icoStar from "../../assets/progress-icons/star.png";
...
/** ===== Icon capsules (PNG -> “embedded HUD”) ===== */
function HudIcon({
  src,
  alt,
  size = 18,
  ...
  return (
    <span className={`tr-icocap ${size >= 22 ? "is-big" : ""} tone-${tone}`} aria-hidden title={alt}>
      <img src={src} alt={alt} style={{ width: size, height: size }} />
    </span>
  );
}

function MetricLabel({ icon, text, tone }: { icon: string; text: string; tone?: any }) {
  return (
...
function MetricValue({ icon, value, tone }: { icon: string; value: React.ReactNode; tone?: any }) {
  return (
...
function KpiTile({
  ...
  return (
    <div className={`tr-kpiTile tone-${tone} ${hero ? "is-hero" : ""}`}>
      ...
      <MetricValue icon={icon} value={value} tone={tone} />
...
export function ProgressPage() {
  ...
  const [painRows, setPainRows] = useState<Array<{ name: string; avg: number; max: number; count: number }>>([]);
  ...
  async function countWorkoutsSince(params: { userId: string; sinceISO: string; activeSessionIds: string[] | null }) {
  ...
  async function loadAll() {
    ...
    try {
      ...
      // Active block goal (used for protein)
      ...
      // scope active -> get session ids in active block
      ...
      // Time buckets (top row)
      const [wk, mo, yr] = await Promise.all([
        countWorkoutsSince({ userId: u.user.id, sinceISO: startOfWeekISO(), activeSessionIds }),
        countWorkoutsSince({ userId: u.user.id, sinceISO: startOfMonthISO(), activeSessionIds }),
        countWorkoutsSince({ userId: u.user.id, sinceISO: startOfYearISO(), activeSessionIds }),
      ]);
      setWkCount(wk);
      setMoCount(mo);
      setYrCount(yr);

      // workouts in range (completed only)
      let wQuery = supabase
        .from("workouts")
        .select(
      ...
      if (scope === "active" && activeSessionIds) wQuery = wQuery.in("scheduled_session_id", activeSessionIds);
      ...
      // Completion rate (range-based, active program only)
      if (scope === "active" && ab?.id) {
      ...
      for (const w of wRows) {
        ...
        if (asec > 0) {
          ...
          const e = w.ended_at ? new Date(w.ended_at).getTime() : w.completed_at ? new Date(w.completed_at).getTime() : null;
          if (s && e && e > s) {
      ...
      setAvgDurationSeconds(durations.length ? Math.round(totalSec / durations.length) : 0);

      // Latest BW and change
      const bwSeries = wRows
        .filter((w: any) => w.bodyweight_lb != null)
        .map((w: any) => ({ t: w.completed_at || w.started_at, bw: Number(w.bodyweight_lb) }))
      ...
      if (bwSeries.length) {
        ...
        const { data: lastBW } = await supabase
          ...
          .limit(1)
          .maybeSingle();
        setLatestBW(lastBW?.bodyweight_lb != null ? Number(lastBW.bodyweight_lb) : null);
        setBwChange(null);
      }

      // Pain flags (7d) — keep your rule
      ...
      if (scope === "active" && activeSessionIds) w7Query = w7Query.in("scheduled_session_id", activeSessionIds);
      ...
      if (w7Ids.length) {
        const { data: pf, error: pfErr } = await supabase.from("workout_exercises").select("id").in("workout_id", w7Ids).gte("pain", 3);
      ...
      // Pain table by exercise + range pain KPIs
      if (wIds.length) {
        ...
        const allPain = (wes ?? []).map((x: any) => Number(x.pain)).filter((p: number) => Number.isFinite(p));
        if (allPain.length) {
        ...
        const exIds = Array.from(new Set(list.map((x: any) => x.exercise_id).filter(Boolean)));

        const nameMap = new Map<string, string>();
        if (exIds.length) {
          const { data: ex, error: exNameErr } = await supabase.from("exercises").select("id,name").in("id", exIds);
          if (exNameErr) throw exNameErr;
          for (const e of ex ?? []) nameMap.set((e as any).id, (e as any).name);
        }

        const agg = new Map<string, { sum: number; max: number; count: number; name: string }>();
        for (const r of list) {
        ...
        const rowsOut = Array.from(agg.values())
          .map((a) => ({ name: a.name, avg: a.count ? a.sum / a.count : 0, max: a.max, count: a.count }))
      ...
      // Volume total + Sets Logged KPI
      if (wIds.length) {
        const { data: weids, error: weidErr } = await supabase.from("workout_exercises").select("id, workout_id").in("workout_id", wIds);
        if (weidErr) throw weidErr;

        const weIds = (weids ?? []).map((x: any) => x.id);
        if (weIds.length) {
          const { data: sets, error: setsErr } = await supabase.from("workout_sets").select("reps,weight,workout_exercise_id").in("workout_exercise_id", weIds);
      ...
      if (wRows.length) {
        ...
        const sessIds = Array.from(new Set(top.map((w: any) => w.scheduled_session_id).filter(Boolean)));

        const sessMap = new Map<string, any>();
        if (sessIds.length) {
          const { data: sess } = await supabase.from("scheduled_sessions").select("id, template_id").in("id", sessIds);
          for (const s of sess ?? []) sessMap.set((s as any).id, s);
        }
        const tmplIds = Array.from(new Set(Array.from(sessMap.values()).map((s: any) => s.template_id).filter(Boolean)));
        const tmplMap = new Map<string, string>();
        if (tmplIds.length) {
          const { data: tmpls } = await supabase.from("workout_templates").select("id,name").in("id", tmplIds);
          for (const t of tmpls ?? []) tmplMap.set((t as any).id, (t as any).name);
        }

        const topIds = top.map((w: any) => w.id);

        // Pain agg per workout
        const painAgg = new Map<string, { sum: number; max: number; count: number }>();
        if (topIds.length) {
          const { data: wes } = await supabase.from("workout_exercises").select("workout_id,pain").in("workout_id", topIds).not("pain", "is", null);
          for (const r of wes ?? []) {
        ...
        // Volume agg per workout
        const volAgg = new Map<string, number>();
        if (topIds.length) {
          const { data: weids2 } = await supabase.from("workout_exercises").select("id, workout_id").in("workout_id", topIds);
          const weIds2 = (weids2 ?? []).map((x: any) => x.id);
          const weToW = new Map<string, string>();
          for (const w of weids2 ?? []) weToW.set((w as any).id, (w as any).workout_id);

          if (weIds2.length) {
            const { data: sets } = await supabase.from("workout_sets").select("reps,weight,workout_exercise_id").in("workout_exercise_id", weIds2);
            for (const s of sets ?? []) {
              ...
              const reps = Number((s as any).reps ?? 0);
              const wt = Number((s as any).weight ?? 0);
              if (reps > 0 && wt > 0) volAgg.set(wid, (volAgg.get(wid) ?? 0) + reps * wt);
            }
          }
        }
        ...
        const out = top.map((w: any) => {
          ...
          const tmplName = sess?.template_id ? tmplMap.get(sess.template_id) ?? "Session" : "Session";
          ...
          return {
            ...
            bodyweight_lb: w.bodyweight_lb != null ? Number(w.bodyweight_lb) : null,
            protein_target_g: w.protein_target_g != null ? Number(w.protein_target_g) : null,
            pain_max: pa.max,
            pain_avg: painAvg,
            volume_total: volAgg.get(w.id) ?? 0,

            post_difficulty: (w.post_difficulty as string) ?? null,
            session_rating: w.session_rating != null ? Number(w.session_rating) : null,
  ...
  async function onClearLogs() {
    ...
    try {
      ...
      const { data: active, error: aErr } = await supabase.from("workouts").select("id").eq("user_id", u.user.id).is("completed_at", null).limit(1).maybeSingle();
      if (aErr) throw aErr;
  ...
  // Hero metric choice: Completion Rate (feels most “dashboard”)
  ...
  return (
    <div className="tr-progressStage">
      <div className="tr-progressStack">
        ...
        <Card
          ...
          right={
            <div className="tr-progressControls">
              <button className={`tr-seg ${scope === "active" ? "is-active" : ""}`} onClick={() => setScope("active")}>
                Active Program
              </button>
              <button className={`tr-seg ${scope === "all" ? "is-active" : ""}`} onClick={() => setScope("all")}>
                All Time
              </button>

              <span className="tr-progressDivider" />

              <button className={`tr-seg ${range === 7 ? "is-active" : ""}`} onClick={() => setRange(7)}>
                7d
              </button>
              <button className={`tr-seg ${range === 14 ? "is-active" : ""}`} onClick={() => setRange(14)}>
                14d
              </button>
              <button className={`tr-seg ${range === 30 ? "is-active" : ""}`} onClick={() => setRange(30)}>
                30d
              </button>
            </div>
          }
        >
          {toast ? (
            <div className="tr-rowbox" style={{ borderColor: "rgba(0,170,255,.35)", background: "rgba(0,170,255,.10)", fontWeight: 900 }}>
              {toast}
            </div>
          ) : null}
          ...
          <div className="tr-moduleFrame">
            <div className="tr-kpiGrid">
              {/* Row 1 — Time buckets */}
              <KpiTile icon={icoSchedule} label="WORKOUTS (WEEK)" value={loading ? "—" : wkCount} tone="blue" sub="Mon → Sun" />
              <KpiTile icon={icoSchedule} label="WORKOUTS (MONTH)" value={loading ? "—" : moCount} tone="blue" sub="This month" />
              <KpiTile icon={icoSchedule} label="WORKOUTS (YEAR)" value={loading ? "—" : yrCount} tone="blue" sub="This year" />

              {/* Row 2 — Hero + consistency */}
              <KpiTile icon={icoChecked} label={`COMPLETED (${range}d)`} value={loading ? "—" : completedCount} tone="base" />
              <KpiTile
                icon={icoRate}
                ...
                hero
              />
              <KpiTile icon={icoFlames} label="STREAK" value={loading ? "—" : `${streakDays}`} tone={streakDays >= 7 ? "green" : "base"} sub="days" />

              {/* Row 3 — Work + density */}
              <KpiTile icon={icoInProcess} label={`TRAINING TIME (${range}d)`} value={loading ? "—" : fmtHMS(totalTimeSeconds)} tone="base" />
              <KpiTile icon={icoInProcess} label={`AVG DURATION (${range}d)`} value={loading ? "—" : fmtHMS(avgDurationSeconds)} tone="base" />
              <KpiTile icon={icoWorkout} label={`SETS LOGGED (${range}d)`} value={loading ? "—" : setsLogged} tone="blue" sub="reps+weight logged" />
            </div>
          </div>

          {/* Tabs */}
          <div className="tr-progressTabs">
            <button className={`tr-seg ${tab === "protein" ? "is-active" : ""}`} onClick={() => setTab("protein")}>
              Protein
            </button>
            <button className={`tr-seg ${tab === "weight" ? "is-active" : ""}`} onClick={() => setTab("weight")}>
              Weight
            </button>
            <button className={`tr-seg ${tab === "volume" ? "is-active" : ""}`} onClick={() => setTab("volume")}>
              Volume
            </button>
            <button className={`tr-seg ${tab === "pain" ? "is-active" : ""}`} onClick={() => setTab("pain")}>
              Pain
            </button>
          </div>
        </Card>

        {/* ===== PAIN MODULE (separate, “pops” + tone glow) ===== */}
        <Card title="Pain" tone="blue">
          <div className={`tr-moduleFrame tr-moduleFrame--pain tone-${painTone}`}>
            <div className="tr-kpiGrid tr-kpiGrid--3">
              <KpiTile icon={icoFlag} label="PAIN FLAGS (7D)" value={loading ? "—" : painFlags7d} tone={painFlags7d > 0 ? "orange" : "green"} sub="pain ≥ 3" />
              <KpiTile icon={icoPain} label={`AVG PAIN (${range}D)`} value={loading ? "—" : painAvgRange == null ? "—" : painAvgRange.toFixed(1)} tone={painTone} sub="all logged pain" />
              <KpiTile icon={icoWarning} label={`MAX PAIN (${range}D)`} value={loading ? "—" : painMaxRange == null ? "—" : painMaxRange} tone={painTone} sub="peak in range" />
            </div>
          </div>
        </Card>
            ...
            <div className="tr-moduleFrame tr-moduleFrame--protein">
              ...
              <div className="tr-proteinHero">
                <div className="tr-proteinHeroLeft">
                  ...
                  <MetricValue icon={icoProtein} value={proteinTargetToday != null ? `${proteinTargetToday}g` : "—"} tone="blue" />
                ...
                <div className="tr-proteinHeroRight">
                  ...
                  <div className="tr-rowbox tr-rowbox--row">
                    <MetricLabel icon={icoScale} text="CURRENT WEIGHT (LB)" tone="base" />
                    <MetricValue icon={icoScale} value={latestBW != null ? latestBW : "—"} tone="base" />
                  </div>
                </div>
              </div>
            </div>
          ...
          <Card title="Bodyweight">
            <div className="tr-moduleFrame">
              <div className="tr-kpiGrid tr-kpiGrid--2">
                <KpiTile icon={icoScale} label="LATEST" value={latestBW != null ? `${latestBW} lb` : "—"} tone="blue" />
                <KpiTile
                  icon={icoRate}
                  label={`CHANGE (${range}d)`}
                  value={bwChange == null ? "—" : `${bwChange >= 0 ? "+" : ""}${bwChange.toFixed(1)} lb`}
        ...
        {tab === "volume" && (
          <Card title="Volume">
            <div className="tr-moduleFrame">
              <KpiTile icon={icoReport} label={`TOTAL VOLUME (${range}d)`} value={volumeTotal == null ? "—" : Math.round(volumeTotal).toLocaleString()} tone="blue" sub="sum(reps × weight)" hero />
            </div>
          </Card>
        )}
          ...
          <Card title="Pain">
            <div className="tr-moduleFrame">
              <div style={{ marginTop: 6, display: "grid", gap: 10 }}>
                {painRows.length ? (
                  painRows.map((r) => (
                    <div key={r.name} className="tr-rowbox tr-rowbox--float" style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 10, alignItems: "center" }}>
                      <div style={{ display: "grid", gap: 2 }}>
                        <div style={{ fontWeight: 950, display: "flex", gap: 10, alignItems: "center" }}>
                          <HudIcon src={icoPain} alt="" size={18} tone={r.max >= 5 ? "orange" : "blue"} />
                          {r.name}
                        </div>
                        <div className="tr-sub">avg {r.avg.toFixed(1)} • max {r.max} • n={r.count}</div>
                      </div>
                      <Chip tone={r.max >= 5 ? "orange" : "blue"}>max {r.max}</Chip>
                    </div>
                  ))
                ) : (
        ...
        <Card
          title="History"
          right={
            <button className="tr-seg" style={{ minWidth: 160, borderColor: "rgba(255,80,80,.35)" }} onClick={() => setClearOpen(true)}>
              Clear Logs
            </button>
          }
        >
          {loading ? (
            ...
            <div style={{ display: "grid", gap: 10 }}>
              {history.map((h) => {
                ...
                return (
                  <div key={h.id} className="tr-rowbox tr-rowbox--float" style={{ display: "grid", gap: 8 }}>
                    ...
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      ...
                      <Chip tone="blue">Weight {h.bodyweight_lb != null ? `${h.bodyweight_lb} lb` : "—"}</Chip>
                      <Chip tone="blue">Protein {h.protein_target_g != null ? `${h.protein_target_g}g` : "—"}</Chip>
                      <Chip tone={h.pain_max >= 5 ? "orange" : "blue"}>Pain max {h.pain_max}</Chip>
                      <Chip tone="blue">Vol {Math.round(h.volume_total).toLocaleString()}</Chip>
            ...
            <div className="tr-modalOverlay">
              <div className="tr-modal">
                <div className="tr-modalHead">
                  ...
                  <button className="tr-btn" onClick={() => setClearOpen(false)} disabled={clearBusy}>
                    Close
                  </button>
                </div>
                ...
                <div style={{ marginTop: 10, display: "grid", gap: 10 }}>
                  <div className="tr-rowbox" style={{ borderColor: "rgba(255,80,80,.35)", background: "rgba(255,80,80,.10)", fontWeight: 900 }}>
                    This permanently deletes all completed workout logs (History + KPIs). Schedule/templates stay intact.
                    <div style={{ marginTop: 8 }}>
                      Type <b>CLEAR</b> to confirm.
                    </div>
                  </div>

                  <input value={clearText} onChange={(e) => setClearText(e.target.value)} placeholder="Type CLEAR" style={{ height: 44 }} disabled={clearBusy} />

                  <button
                    className="tr-btn tr-btn--primary"
                    style={{
                      borderColor: clearText.trim().toUpperCase() === "CLEAR" ? "rgba(255,80,80,.65)" : undefined,
                      background: clearText.trim().toUpperCase() === "CLEAR" ? "linear-gradient(180deg, rgba(255,80,80,.22), rgba(255,80,80,.12))" : undefined,
                    }}
        ...
        <Card title="Actions">
          <div style={{ display: "grid", gap: 10 }}>
            <Button onClick={() => (window.location.pathname = "/")}>Back to Workouts</Button>
            <Button variant="secondary" onClick={() => (window.location.pathname = "/coach")}>
              Go to Coach
            </Button>
      ...
      {/* ===== PAGE-SCOPED REDESIGN STYLES ===== */}
      <style>{`
        /* Foreground stage: big separation background -> UI */
        .tr-progressStage{
          position: relative;
          isolation: isolate;
        }
        ...
        .tr-progressStack{ position: relative; z-index: 1; display: grid; gap: 14px; }
        ...
        /* Make cards feel like floating hardware (page-scoped) */
        ...
        /* Inner module frame (bezel within the card body) */
        .tr-moduleFrame{
          margin-top: 12px;
          border-radius: 18px;
        ...
        /* KPI tile: floating instrument tile */
        .tr-kpiTile{
          border-radius: 16px;
        ...
        /* Step 2 — bigger label icons (Progress page only) */
         .tr-kpiTile .tr-ico{
...
.tr-kpiTile .tr-icocap{
  ...
  height:34px !important;
        ...
        /* Hero tile: spans visually, bigger readout */
        .tr-kpiTile.is-hero{
          background:
