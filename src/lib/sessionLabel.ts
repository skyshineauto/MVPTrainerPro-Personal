...
export function friendlyBaseName(sessionType: string | null | undefined) {
  ...
  if (s === "Upper 1") return "Upper A — Foundation";
  if (s === "Upper 2") return "Upper B — Foundation";
  if (s === "Lower 1") return "Lower A — Foundation";
  if (s === "Lower 2") return "Lower B — Foundation";
  return s || "Session";
}
...
export function isSymptomMode(goalMode: string | null | undefined) {
  return String(goalMode || "").toLowerCase().includes("symptom");
}
...
export function formatSessionLabel(params: {
  ...
  // Option B: infer symptom tag when in symptom mode; otherwise use goal tag.
